...
**Envelope + hash-chain rules for the Identity binding-lifecycle events (added v5.4 patch 2026-05-18 per Codex SC6 PR #10 R4 round-4 HIGH finding closure — the unchanged audit envelope rules elsewhere in this doc require `target_patient_id` + `resource_type` + `resource_id` + a hash-chain partition keyed by `target_patient_id`, which cannot hold for non-patient-scoped governance events; these three identity actions are scoped to the platform identity layer, not to patient PHI surfaces, so the envelope rules below apply specifically and supersede the patient-default rules):**
- `target_patient_id` is **NULL** for all three identity binding-lifecycle events. Identity-layer events are platform-scope governance evidence, not patient-scope records.
- `resource_type` = `'actor_context'` for `identity.actor_context_bound` and `identity.actor_context_unbound_rejected`; `resource_type` = `'session'` for `identity.session_liveness_check_failed`.
- `resource_id` = `session_id` when present (always for `actor_context_bound` and `session_liveness_check_failed` when the JWT was successfully parsed); when `session_id` is unknown (e.g., `session_liveness_check_failed` where JWT verify failed before liveness check, or `actor_context_unbound_rejected` where no bound context exists), `resource_id` = `pg_backend_pid::text || ':' || txid::text` composite string. The composite-string form preserves uniqueness within the audit chain and keeps the row schema NOT NULL on `resource_id`.
- **Hash-chain partition:** identity binding-lifecycle events partition into a separate, dedicated `identity_lifecycle` hash chain (NOT the per-patient chain) keyed by `tenant_id_claimed` when present, otherwise the platform-scope `_PLATFORM_` sentinel partition. This preserves the per-patient hash-chain integrity rule for PHI-scoped events while giving identity-layer events their own append-only chain for forensic walks. The `_PLATFORM_` sentinel is reserved exclusively for events with no tenant attribution (e.g., pre-JWT-verify failures); concrete tenants always partition under their `tenant_id_claimed` even when the actor's claimed identity turns out to be invalid (a revoked-session check is still tenant-scoped at the JWT level).
- I-027 attribution discipline is preserved by the actor/account/session fields **inside** the `detail` payload (for events that carry them), NOT by the envelope's `target_patient_id`. The detail payload is the authoritative attribution surface for these Cat B governance events.
