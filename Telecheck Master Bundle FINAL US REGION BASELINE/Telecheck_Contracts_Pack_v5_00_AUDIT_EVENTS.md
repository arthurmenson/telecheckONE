...
- `resource_id`: for events 1-3 (actor_context_bound, session_liveness_check_failed, actor_context_unbound_rejected), use `session_id` when present (always for `actor_context_bound` and `session_liveness_check_failed` when the JWT was successfully parsed); when `session_id` is unknown, use `pg_backend_pid::text || ':' || txid::text` composite string. For `identity.audit_recovery_link`, use the deterministic composite `backstop_partition_id::text || ':' || outage_window_start::text` (so each outage window has exactly one resource_id; replay or duplicate-emission attempts are idempotency-safe via this stable resource_id). All composite forms preserve uniqueness within the audit chain and keep the row schema NOT NULL on `resource_id`.
