...
- **Hash-chain partition:** identity binding-lifecycle events partition into a separate, dedicated `identity_lifecycle` hash chain (NOT the per-patient chain). All FOUR events (including `identity.audit_recovery_link`) land in the PRIMARY `identity_lifecycle` partition when ingested into the primary `audit_events` table — the recovery_link checkpoint specifically lands in primary `identity_lifecycle` after `audit_backstop_ingest` drain completes, NOT in the backstop's `identity_lifecycle_backstop` chain (the backstop chain only holds events 1-3 that were emitted during the outage; the recovery_link bridges back to primary). Partition key for all four events = `tenant_id_claimed` when present, otherwise `_PLATFORM_` sentinel; for `identity.audit_recovery_link` the tenancy is the outage window's first-event tenancy (carries forward across the bridge so forensic walks stay in one tenant partition where applicable; `_PLATFORM_` if all events in the outage window were untenancied). The `_PLATFORM_` sentinel is reserved exclusively for events with no tenant attribution. The dedicated audit-backstop instance maintains its own `identity_lifecycle_backstop` partition with the same key + append-only discipline during outages.
