...
- **I-012 closure rule (added v5.2 patch 2026-05-02 per Codex Scope 1 HIGH-1 finding):** For any audit record whose action class is governed by I-012 — including `refill.approved`, `prescribing.approved`, `protocol_authorized_prescribing`, `protocol_authorized_refill_renewal`, and any future I-012-class action — the fields `ai_workload_type` and `autonomy_level` are **required regardless of `actor_type`**. The previous nullability carve-out for non-AI actors does NOT apply to I-012-class actions. The legacy `protocol_engine` actor_type, when emitting an I-012-class action, MUST be mapped at emission time to `actor_type = ai_workload, ai_workload_type = protocol_execution` (this is the canonical mapping per AI_LAYERING v5.2 §10.2). Schema-driven implementations that retain `protocol_engine` as the literal actor_type for I-012 events are non-compliant. This closure prevents an I-012 prescription/refill from passing schema validation with null workload/autonomy fields, defeating the §13.7 three-clause evidence requirement.
