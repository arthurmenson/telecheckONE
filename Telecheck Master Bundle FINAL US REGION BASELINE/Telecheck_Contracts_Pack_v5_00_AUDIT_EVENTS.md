...
- `resource_id`: for events 1-3 (actor_context_bound, session_liveness_check_failed, actor_context_unbound_rejected), use `session_id` when present (always for `actor_context_bound` and `session_liveness_check_failed` when the JWT was successfully parsed); when `session_id` is unknown, use `pg_backend_pid::text || ':' || txid::text` composite string. For `identity.audit_recovery_link`, use the **3-part deterministic composite `backstop_partition_id::text || ':' || outage_window_start::text || ':' || partition_key` where `partition_key` is the per-checkpoint tenant discriminator (a concrete `account_tenant_id` value, or the literal `_PLATFORM_` sentinel for the multi-tenant aggregate checkpoint)** — extended from 2-part to 3-part composite per Codex SC6 PR #10 R19 round-19 HIGH closure 2026-05-18 because the prior 2-part composite collided across the per-distinct-tenant checkpoint set that the R17 round-17 multi-tenant rule requires (a multi-tenant outage window emits one checkpoint per distinct tenant + one aggregate `_PLATFORM_` checkpoint, all sharing the prior 2-part resource_id and triggering the idempotency-on-resource_id check incorrectly). The 3-part composite gives each tenant-discriminator checkpoint a unique stable resource_id while preserving idempotency on retry within the same checkpoint. All composite forms preserve uniqueness within the audit chain and keep the row schema NOT NULL on `resource_id`.
