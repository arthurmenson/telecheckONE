...
**v5.4 amendments (2026-05-18, P-023 / SC6 SI-010 canonical-content-port landing; refined across Codex PR #10 rounds 1–6):** **4 net-new Category B action IDs** added under a new "Identity binding-lifecycle events" subsection of §Category-B (the 3 originally-ratified + 1 added per Codex round-6 R1 HIGH closure to bridge primary + backstop hash-chained partitions during audit-infrastructure recovery): `identity.actor_context_bound` (emitted by the authContextPlugin via the application-side `auditWriterPool` with fallback to the dedicated `auditBackstopPool` on primary failure — NOT by the `bind_actor_context()` procedure itself, which performs only the binding-table UPSERT; clarified per Codex R28 round-28 MEDIUM closure 2026-05-18), `identity.session_liveness_check_failed` (emitted on revoked / missing / expired session check at the authContextPlugin Step 2 fail-closed point per SI-010 R3 HIGH-2 closure 2026-05-15), `identity.actor_context_unbound_rejected` (emitted by dependent SECURITY DEFINER procedures when `assert_request_nonce_bound()` raises `actor_context_unbound` or `request_nonce_unbound_or_expired`), plus `identity.audit_recovery_link` (emitted by `audit_backstop_ingest` worker once per outage window during drain of the dedicated audit-backstop instance back into primary; bridges primary `identity_lifecycle` and backstop `identity_lifecycle_backstop` hash-chained partitions for forensic walks). **Per-event mandatory-minimum fields (event-specific by design; correlation invariant explained below) — corrected v5.4 patch 2026-05-18 per Codex SC6 PR #10 R1 HIGH finding closure to remove the prior "All 3 carry actor_account_id + session_id + nonce_hash" claim, which was inconsistent with the detail-payload schemas below:** (a) `identity.actor_context_bound` carries the full bound identity (`actor_account_id`, `account_tenant_id`, `role`, `session_id`, `nonce_hash`, `pg_backend_pid`, `txid`); (b) `identity.session_liveness_check_failed` carries `session_id` + best-effort `account_id` from JWT claims + `tenant_id_claimed` + `failure_reason` + `checked_at` + `pg_backend_pid` + `txid` (the last two captured server-side at liveness-check time so the correlation invariant holds even though no bind row exists yet — added Codex round 2 closure 2026-05-18) (it fires BEFORE `bind_actor_context()` succeeds, so there is no `nonce_hash` and the JWT-claimed identity may not match an active actor — `account_id` is nullable when the JWT verify itself failed before liveness check); (c) `identity.actor_context_unbound_rejected` carries `procedure_name` + `rejection_code` + `pg_backend_pid` + `txid` + `nonce_hash` (nullable when the GUC was missing or the dependent procedure could not extract one) — actor/account/session fields are intentionally absent because by definition no bound context resolved. **Forensic correlation invariant (scoped to the three binding-path events; the recovery_link checkpoint uses a different correlation model — see below):** `pg_backend_pid` + `txid` are present on all three binding-path events (`identity.actor_context_bound`, `identity.session_liveness_check_failed`, `identity.actor_context_unbound_rejected`) as the cross-event correlation key; any `identity.session_liveness_check_failed` or `identity.actor_context_unbound_rejected` in the audit chain can be tied back to its enclosing request transaction without requiring `actor_account_id` or `session_id`. **All three binding-path events use the rollback-independent commit path documented in Identity Spec v1.1 §3.6 step 4** (application-side `auditWriterPool` for plugin-layer emissions; `dblink_exec` loopback for SECURITY DEFINER procedure-side emissions); the durable backstop on the dedicated `audit_backstop` PostgreSQL instance covers the failure mode. **The fourth event, `identity.audit_recovery_link`, uses its own correlation model and writer:** it is emitted by the `audit_backstop_ingest` worker (system actor) once per outage window during drain of the backstop instance back into primary; its correlation key is the resource_id, which takes **two variant forms** (per Codex R19 + R23 closures 2026-05-18): **per-tenant checkpoint** uses 3-part deterministic composite `(backstop_partition_id, outage_window_start, partition_key)` with `partition_key` = concrete `account_tenant_id`; **aggregate `_PLATFORM_` checkpoint** (multi-tenant outage windows only) uses distinct namespace `aggregate:_PLATFORM_:<outage_window_start>` carrying NO scalar `backstop_partition_id` (the aggregate references per-tenant checkpoints via its `per_tenant_checkpoints[]` manifest). The correlation key for both variants is NOT a per-request `(pg_backend_pid, txid)` tuple. The recovery_link's payload references the backstop chain head/tail audit_ids, outage window timestamps, and recovered event count — providing the bridge a forensic walk traverses to reach the backstop chain segment that holds the binding-path events emitted during the outage. All four events share the durable backstop guarantee (recovery_link emission is itself fail-closed + retried via exponential backoff + idempotent on resource_id collision per regression test #7 sub-assertion (d)). The forensic anchor survives request-transaction rollback for the three binding-path events; for the recovery_link, durability is provided by the worker's commit-on-success + retry-on-failure semantics outside any request transaction. This entire model addresses the prior `pg_notify`-based transport's transactional-suppression vulnerability (Codex round-3 closure 2026-05-18) and the original 3-event scope ambiguity (Codex round-9 + round-10 closures 2026-05-18). The full bound identity (when known) lives on the corresponding `identity.actor_context_bound` row; the absence of that row in the same `(pg_backend_pid, txid)` tuple IS the forensic signal. Closes Phase 2 F-3 (JWT session-liveness check) by virtue of folding the liveness check into the binding path; unblocks SI-005 + SI-008 + SI-009 SECURITY DEFINER procedures' IMPL-readiness gate. Lockstep paired with Identity & Authentication Spec v1.0 §3.6 NEW "Server-side actor context (per SI-010)" section landing in this same PR-A2/A3 commit. CDM-exempt (Identity-slice procedure-only; no new entity rows). DOMAIN_EVENTS-exempt (binding lifecycle is audit-only Cat B; no domain events). All carry-forward enums from v5.3 preserved unchanged.
...
| Action | Actor types | Detail payload | audit_sensitivity_level |
|---|---|---|---|
| `identity.actor_context_bound` | system (emitted by the **authContextPlugin** (application-side, NOT by `bind_actor_context()` SECURITY DEFINER procedure itself — clarified per Codex SC6 PR #10 R28 round-28 MEDIUM closure 2026-05-18; the prior wording implied procedure-side emission which contradicted Identity Spec §3.6 step 4's application-side `auditWriterPool` writer + fallback to `auditBackstopPool` on primary failure; `bind_actor_context()` performs ONLY the binding-table UPSERT, NOT the audit emit) after `bind_actor_context()` returns successfully; actor identity derived from JWT-verified context, not caller-supplied; durable-backstop-first fallback chain per Identity Spec §3.6 step 4 — primary `auditWriterPool` → backstop `auditBackstopPool` → 503 only if both fail) | actor_account_id, account_tenant_id, role, admin_home_tenant_id (null except for platform_admin actors), session_id, nonce_hash (SHA-256 of the binding nonce; never plaintext), bound_at, expires_at, pg_backend_pid, txid | standard |
| `identity.session_liveness_check_failed` | system (emitted on revoked / missing / expired session check at the authContextPlugin Step 2 fail-closed point per SI-010 R3 HIGH-2 closure 2026-05-15; paired with `throw UnauthenticatedError()` → Fastify error-envelope plugin maps to tenant-blind 401 per I-025) | session_id, account_id (best-effort from JWT; may be null if JWT verify itself failed first), tenant_id_claimed (from JWT; may differ from actor's actual tenant_id if revocation occurred mid-session), failure_reason (`revoked` | `missing` | `expired`), checked_at, pg_backend_pid, txid (both captured server-side at liveness-check time even though no bind row exists yet — preserves the cross-event correlation invariant per Codex SC6 PR #10 R2 round-2 HIGH finding closure 2026-05-18) | standard |
...
| `identity.audit_recovery_link` | system (emitted by `audit_backstop_ingest` worker per outage window during drain of the dedicated `audit_backstop` PostgreSQL instance's `identity_lifecycle_backstop` partition back into primary `identity_lifecycle` partition; bridges the two hash-chained partitions for forensic walks; added v5.4 patch per Codex SC6 PR #10 R6 round-6 HIGH closure 2026-05-18 + refined per R17/R19/R20/R21/R22 closures for multi-tenant outage windows) | **detail-payload shape is per-checkpoint-variant** (added v5.4 patch per Codex R22 round-22 HIGH closure 2026-05-18 — the prior scalar-only fields could not faithfully represent multi-tenant aggregate checkpoints because backstop partitions are per-tenant too): for a **per-tenant checkpoint** (partition_key = concrete `account_tenant_id`): `backstop_partition_id`, `backstop_chain_head_audit_id`, `backstop_chain_tail_audit_id`, `outage_window_start`, `outage_window_end`, `recovered_event_count`, `drain_completed_at`, `primary_partition = 'identity_lifecycle'`, `partition_key`, `recovery_link_protocol_version`. For the **aggregate `_PLATFORM_` checkpoint** (partition_key = `_PLATFORM_`; emitted only for multi-tenant outage windows): `outage_window_start`, `outage_window_end`, `drain_completed_at`, `primary_partition = 'identity_lifecycle'`, `partition_key = '_PLATFORM_'`, `recovery_link_protocol_version`, AND a NEW `per_tenant_checkpoints[]` array carrying one entry per tenant `{partition_key: account_tenant_id, backstop_partition_id, head_audit_id, tail_audit_id, recovered_event_count, recovery_link_audit_id}` (the `recovery_link_audit_id` cross-references the per-tenant checkpoint's `audit_id` so forensic walks from the aggregate land on the per-tenant checkpoint which then bridges to the backstop chain segment). The aggregate's `recovered_event_count` is the sum of per-tenant counts; the aggregate carries NO scalar backstop_partition_id/head/tail because no single backstop partition represents the multi-tenant window (a scalar value here would be a misrepresentation). Test 7B gate asserts the aggregate covers exactly the per-tenant checkpoint set (no missing tenants; no duplicates). | standard |

**Envelope + hash-chain rules for the Identity binding-lifecycle events (added v5.4 patch 2026-05-18 per Codex SC6 PR #10 R4 round-4 HIGH finding closure + extended per Codex R6 round-6 HIGH-1 closure 2026-05-18 to explicitly cover the `identity.audit_recovery_link` event — the unchanged audit envelope rules elsewhere in this doc require `target_patient_id` + `resource_type` + `resource_id` + a hash-chain partition keyed by `target_patient_id`, which cannot hold for non-patient-scoped governance events; these FOUR identity actions are scoped to the platform identity layer, not to patient PHI surfaces, so the envelope rules below apply specifically and supersede the patient-default rules):**
- `target_patient_id` is **NULL** for all four identity binding-lifecycle events (`identity.actor_context_bound`, `identity.session_liveness_check_failed`, `identity.actor_context_unbound_rejected`, `identity.audit_recovery_link`). Identity-layer events are platform-scope governance evidence, not patient-scope records.
- `resource_type`: `'actor_context'` for `identity.actor_context_bound` and `identity.actor_context_unbound_rejected`; `'session'` for `identity.session_liveness_check_failed`; `'audit_recovery_link'` for `identity.audit_recovery_link`.
- `resource_id`: for events 1-3 (actor_context_bound, session_liveness_check_failed, actor_context_unbound_rejected), use `session_id` when present (always for `actor_context_bound` and `session_liveness_check_failed` when the JWT was successfully parsed); when `session_id` is unknown, use `pg_backend_pid::text || ':' || txid::text` composite string. For `identity.audit_recovery_link`, the resource_id form depends on the checkpoint variant (per Codex SC6 PR #10 R23 round-23 HIGH-1 closure 2026-05-18 — the prior uniform 3-part `backstop_partition_id::outage_window_start::partition_key` composite required a scalar `backstop_partition_id` that the aggregate `_PLATFORM_` variant carries no value for, because no single backstop partition represents a multi-tenant outage; using a fake partition id would misrepresent the chain endpoint relationship). **Per-tenant checkpoint resource_id** (partition_key = concrete `account_tenant_id`): **3-part deterministic composite `backstop_partition_id::text || ':' || outage_window_start::text || ':' || partition_key`** — each per-tenant checkpoint has a unique stable resource_id keyed to its backstop partition. **Aggregate `_PLATFORM_` checkpoint resource_id**: **deterministic composite `'aggregate:_PLATFORM_:' || outage_window_start::text`** (NO `backstop_partition_id` because the aggregate references the per-tenant checkpoint set via its `per_tenant_checkpoints[]` manifest, not via any single scalar partition; the `aggregate:` prefix discriminator distinguishes this resource_id namespace from the per-tenant form and avoids collision). Both forms are idempotency-safe on retry within the same checkpoint and emit-set-safe across the per-tenant + aggregate set. The 3-part per-tenant composite was extended from a 2-part form per Codex R19 round-19 HIGH closure because the prior 2-part composite collided across the per-distinct-tenant checkpoint set that the R17 round-17 multi-tenant rule requires. The aggregate's separate `aggregate:` namespace was added per R23 because the 3-part form is undefined when the aggregate carries no scalar `backstop_partition_id`. All composite forms preserve uniqueness within the audit chain and keep the row schema NOT NULL on `resource_id`.
- **Hash-chain partition:** identity binding-lifecycle events partition into a separate, dedicated `identity_lifecycle` hash chain (NOT the per-patient chain). All FOUR events land in the PRIMARY `identity_lifecycle` partition when ingested into the primary `audit_events` table — the recovery_link checkpoint specifically lands in primary `identity_lifecycle` after `audit_backstop_ingest` drain completes, NOT in the backstop's `identity_lifecycle_backstop` chain (the backstop chain only holds events 1-3 emitted during the outage; the recovery_link bridges back to primary). **Per-event partition-key mapping (added v5.4 patch per Codex SC6 PR #10 R17 round-17 HIGH finding closure 2026-05-18 — the prior "tenant_id_claimed when present" rule was wrong because `tenant_id_claimed` is a payload field on `identity.session_liveness_check_failed` only; the other three events carry different tenant fields or none, so the wrong rule would have mispartitioned tenant-attributed bindings into `_PLATFORM_`):**
  - `identity.actor_context_bound` → partition key = `account_tenant_id` (from the detail payload; always present for successfully-bound contexts). The audit envelope's `tenant_id` field MUST equal `account_tenant_id`.
  - `identity.session_liveness_check_failed` → partition key = `tenant_id_claimed` from JWT claims when present; `_PLATFORM_` sentinel ONLY when JWT verify itself failed before any claim could be extracted (rare; recorded in `failure_reason = missing` cases without claims).
  - `identity.actor_context_unbound_rejected` → partition key = best-effort tenant resolution chain: (i) if a prior `identity.actor_context_bound` exists in the same `(pg_backend_pid, txid)` correlation tuple and is still readable, use its `account_tenant_id`; (ii) otherwise `_PLATFORM_` sentinel. The procedure SHOULD emit the audit AFTER attempting to read the bound row's tenancy (best-effort enrichment); when no bound context resolves at all, `_PLATFORM_` is the correct attribution (no tenant identity exists).
  - `identity.audit_recovery_link` → partition key = the outage window's set-of-tenants discriminator: if all backstop chain events in the outage window share a single `account_tenant_id` (or all are `_PLATFORM_`), partition under that single value; if the backstop chain spans multiple tenants, emit ONE recovery_link checkpoint per distinct tenant partition AND ONE additional checkpoint in `_PLATFORM_` referencing the multi-tenant aggregate (so per-tenant forensic walks can cross into the backstop chain segment for their tenant without traversing other tenants' events).
- The `_PLATFORM_` sentinel is reserved exclusively for events with NO tenant attribution resolvable at emission time. The dedicated audit-backstop instance maintains its own `identity_lifecycle_backstop` partition with the same per-event key mapping + append-only discipline during outages. Regression assertion (added to test 7A gate): a tenant-attributed `identity.actor_context_bound` event MUST NEVER land in `_PLATFORM_`; any such event in `_PLATFORM_` indicates a partition-key derivation bug.
- I-027 attribution discipline is preserved by the actor/account/session fields **inside** the `detail` payload (for events that carry them), NOT by the envelope's `target_patient_id`. The detail payload is the authoritative attribution surface for these Cat B governance events. For `identity.audit_recovery_link`, attribution is to the `audit_backstop_ingest` worker (system actor) writing the checkpoint; the bridged actor identities live on the backstop chain segment that the checkpoint references.

**Cross-references for the Identity binding-lifecycle events:**

- **Identity & Authentication Spec v1.1 §3.6** — "Server-side actor context (per SI-010)" — defines the `_session_actor_context` table + `bind_actor_context()` SECURITY DEFINER procedure + `current_actor_*()` helpers + `assert_request_nonce_bound()` + authContextPlugin wiring + GRANT model + cleanup mechanism + rollback-independent audit-writer pattern + fail-closed semantics on audit-writer failure. Landed in lockstep with this AUDIT_EVENTS v5.4 amendment per the SC6 P-023 SI-010 canonical-content-port commit (2026-05-18).
...
- **PHI guarantee:** all **4 Identity binding-lifecycle events** reference identifiers by ID or hash (`session_id` ULID, `account_id` ULID, `nonce_hash` SHA-256, `pg_backend_pid` numeric, `txid` numeric on the three binding-path events; `backstop_partition_id` opaque-string + `backstop_chain_head_audit_id`/`backstop_chain_tail_audit_id` ULIDs + ISO-8601 outage-window timestamps + numeric `recovered_event_count` on the `identity.audit_recovery_link` checkpoint — no PHI payload, no actor identity, no session secrets); never plaintext PHI per the AUDIT_EVENTS Cat B governance discipline.
