...
> **⚠️ SUPERSEDED 2026-05-22 — POST-P-034 CANONICAL BODIES TAKE PRECEDENCE.** The procedure body below was the SI-019 Option B SECDEF wrapper design at original ratification (P-026 OQ-A 2026-05-20, per this file's header; subsequent P-033 ratified the SI-019 PRD v2.0 source 2026-05-21, and P-034 then ratified the CDM follow-on landing 2026-05-21). The P-034 CDM v1.6 → v1.7 follow-on amendment ratified the canonical wrapper-body shape using the SI-024.1 JWT-binding trust anchor (`verify_session_jwt_and_extract_claims()` + `tenant_account_membership` LAYER B authorization + LAYER C tenant-scope match per the P-038 R6 dissolution pattern). The pre-JWT design preserved here (caller-supplied `p_tenant_id` + `p_clinician_id` + raw `current_setting('app.tenant_id')` predicate check + no LAYER A/B/C structure + EXECUTE granted directly to application role `medication_interaction_clinician`) is **NOT the canonical implementation target** — engineers MUST implement against the post-P-034 canonical wrapper bodies in `Telecheck Master Bundle FINAL US REGION BASELINE/Telecheck_CDM_v1_6_to_v1_7_Amendment.md`. Specifically: **§3 New SECURITY DEFINER procedures** → §6.NEW1 raw lifecycle writer `record_interaction_signal_lifecycle_transition()`; §6.NEW2-NEW6 five reason-specific wrappers (`record_signal_emission/activation/supersession/resolution/expiry`); §6.NEW7 override wrapper `record_interaction_signal_override()`. **§8 RBAC v1.1 → v1.2 amendment** → 4 application roles + 6 wrapper-owner roles + 2 service-level owner roles + the wrapper EXECUTE-grant matrix. The post-P-034 wrappers preserve the same business contract (state-transition triples + CHECK enforcement + clinical-decision-notes mandate + idempotency-key semantics + lifecycle-transition append-only via raw writer) but route authorization through the canonical JWT-binding helper. **Closure trigger:** P-042 2026-05-22 cross-slice consistency audit Finding 1 HIGH; closed as part of the post-P-042 hygiene cycle (Phase 1) via supersession marker rather than wholesale prose rewrite to preserve the audit trail of the original ratifier-approved P-033 design + the migration path to the P-034 canonical bodies. **For new SI-019 reads, the post-P-034 bodies are authoritative; the prose below is preserved for traceability only.**
