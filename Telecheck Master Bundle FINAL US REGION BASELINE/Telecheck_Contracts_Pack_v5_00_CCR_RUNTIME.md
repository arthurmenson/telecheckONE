...
- **I-029** — research data export 6-condition reject-unless gate (canonical full statement at INVARIANTS v5.2 I-029; *updated 2026-05-02 per Codex Round-12 Scope 1 HIGH-1 finding to clean up the prior conjunction that conflated a Stage 2 precondition with a completion-time gate condition; expanded to 6-condition gate 2026-05-02 per Codex Round-12 Scope 3 HIGH-1 + post-merge adversarial review HIGH-1 finding adding the per-export grant-artifact gate*).

  **Stage 2 activation PRECONDITION (checked at `research.export_initiated` time, not part of the 6-condition completion gate):** `research_data_partnership_active = active` for the cohort's `country_of_care` (per MARKET_LAUNCH v5.1 Stage 2 activation gate). Initiations attempted while the country is `inactive` or `consent_only` are rejected at `/research/exports/initiate` per OpenAPI v0.2 + STATE_MACHINES v1.1 `queued → processing` guard.

  **Canonical 6-condition completion-time gate (matches INVARIANTS / TYPES / AUDIT_EVENTS / OpenAPI / STATE_MACHINES / GOVERNANCE_CONTROLS exactly; expanded 2026-05-02 per Codex Round-12 Scope 3 HIGH-1 finding from prior 5-condition gate to add per-export grant artifact re-validation):** (1) `dsa_status_at_export = active` (DSA still active at completion-time check; expired/suspended/retired during the export window rejects); (2) `k_threshold_actual >= k_min_required` (per-DSA k_min_required ≥ CCR `k_min_default = 11`; per-DSA increases permitted, decreases below `k_min_default` prohibited); (3) `permitted_data_domains_at_export` matches the `research.export_initiated` snapshot (no CCR drift mid-export; per-DSA domains MUST be subset of country-level CCR `research_permitted_data_domains`); (4) `consent_cohort_snapshot_hash_completed = consent_cohort_snapshot_hash_initiated` (cohort unchanged mid-export); (5) every contributing patient has active `ResearchConsent` at completion-time evaluation (`granted_at` non-null, `revoked_at` null); (6) per-export grant artifact (PolicyAuthorization OR named-equivalent per `research_export_authorized_signers` attestation evidence-locker) exists, unexpired, matches export by ID/hash binding, and still attests the multi-party signer chain.

  Failed delivery MUST emit `research.export_completed(status=invalidated)` with canonical `invalidation_reason` enum value (one of `dsa_inactive | k_anonymity_violation | permitted_domain_drift | consent_cohort_change | consent_revocation_mid_export | grant_artifact_invalidated`) paired with `signal_enforcement_trigger` Category B per I-003. The Stage 2 precondition does not have a corresponding `invalidation_reason` because failures at the precondition stage reject before initiation; no export entity is created.
