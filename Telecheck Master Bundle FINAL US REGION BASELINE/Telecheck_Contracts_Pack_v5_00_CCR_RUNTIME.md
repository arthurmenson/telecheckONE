...
**Runtime validator rule (added v5.2 patch 2026-05-02 per Codex Round-2 Scope 3 HIGH-1 finding):** The CCR runtime validator MUST reject any state transition setting `research_data_partnership_active = consent_only` when `research_ethics_review_body` is null (i.e., its `approval_reference_id`, `approval_validity_from`, `approval_validity_to`, `approval_scope` sub-fields are null/missing). This closes the failure mode where a tenant could collect 5th-tier consent (and emit `research.consent_granted` audit events) before an approved REC artifact exists. The `inactive → consent_only` gate per MARKET_LAUNCH v5.1 is the canonical activation review for this transition; the CCR runtime validator is the runtime backstop.
