...
**Plus claim-wrapper SECURITY DEFINER procedure discipline (mirrors SI-024.1 advisory-lock pattern):** the `claim_consult_for_review()` SECURITY DEFINER wrapper MUST acquire `pg_try_advisory_xact_lock` on `(tenant_id, consult_id)` BEFORE attempting INSERT into `consult_review_claim`. This serializes concurrent `/claim` requests so the partial unique index conflict is observable as a structured `claim_already_held` rejection rather than a generic unique_violation race + Allow follow-on workflow (claim takeover via reassignment SECURITY DEFINER procedure that atomically releases prior + inserts new under same advisory lock).
