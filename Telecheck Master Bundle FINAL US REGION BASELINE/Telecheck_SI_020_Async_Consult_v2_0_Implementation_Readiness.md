...
**Predecessor reconciliation (2026-05-24, per cockpit Addendum 95 lever).** This Sub-decision 1 entity model **supersedes** the earlier Consult/ConsultEvent schema proposed in `Telecheck_SI_Closure_Cycle_2026-05-11/Telecheck_SI_005_Consult_ConsultEvent_Schema_DRAFT.md` (a 2-entity **mutable-state-column** model — `consults` with a mutable `state` column + denormalized lifecycle timestamps, plus a generic `consult_events` log; `VARCHAR(26)` PKs; patient FK to `accounts`; v0.2 2026-05-11). That is the exact "mutable-state-column model from v1.0 §12" this Sub-decision's append-only-only (I-035) model is designed to supersede. SI-005 has been annotated **SUPERSEDED-FOR-RATIFICATION** with a full divergence table + SI-005→SI-020 entity/column crosswalk; its closure now routes through **this** SI's P-037/P-038 ratification, NOT a standalone SI-005 CDM amendment. **The two schema models must NOT both be ratified.** SI-005 is preserved as the historical record of the Sprint-9 placeholder that shipped in `telecheck-app/migrations/020` + `021`. **One load-bearing item carried in from SI-005 for the ratifier to resolve at the ceremony:** migrations 020/021 shipped the SI-005-shaped mutable-state placeholder, so the P-038 follow-on amendment must include a **schema-model-replacement migration** (drop mutable-state `consults`/`consult_events`; create these 7 entities + view) rather than a forward-ALTER — plus a ratifier decision on whether any Sprint-9 rows data-migrate or 020/021 is greenfield-replaceable (no production data pre-launch). Two minor SI-005 gaps also flagged: no explicit `modality` column on `consult` (ADR-012 async↔sync handoff modeled as the `escalated_to_sync` transition/audit only), and no denormalized `country_of_care` on `consult`. Not authored here (would be net-new beyond ratified sub-decision scope per hard-floor item 6).
...
**Proposed action IDs (R7 MED-1 closure 2026-05-21: 17 new under `async_consult.*` namespace; **4 Cat A + 3 Cat B + 10 Cat C** — earlier draft incorrectly stated 4+6+7; recounted from the actual table: Cat A IDs 9, 10, 11, 16; Cat B IDs 6, 13, 17; Cat C IDs 1, 2, 3, 4, 5, 7, 8, 12, 14, 15):**
...
**Recommendation:** APPROVE (17 events under `async_consult.*` namespace per actual table: 4 Cat A + 3 Cat B + 10 Cat C; partition routing per SI-018; Cat A on clinician decision + prescribing + decision-rationale-disagreement + invariant violations aligns with SI-005 P-021 + Mode 1 P-035 patterns; Cat B on AI-preparation failure + cross-mode escalation + expired-claim auto-release per audit-pipeline + reassignment-takeover discipline; Cat C high-volume sampled on 8 per-step lifecycle observations + non-sampled on intake_abandoned + additional_data_requested + follow_up_message_sent per their volume / PHI-relevance characteristics).

**Predecessor reconciliation (2026-05-24, per cockpit Addendum 94 lever).** This Sub-decision 2 table **supersedes** the earlier audit taxonomy proposed in `Telecheck_SI_Closure_Cycle_2026-05-11/Telecheck_SI_004_Async_Consult_Audit_Events_Ratification_DRAFT.md` (a `consult.*` 20-event, one-per-state-transition taxonomy, v0.1 2026-05-11, derived from State Machines v1.1 §3). SI-004 has been annotated SUPERSEDED-FOR-RATIFICATION with a full `consult.*` → `async_consult.*` crosswalk; its closure now routes through **this** table's P-037/P-038 ratification, NOT a standalone SI-004 amendment. **The two taxonomies must NOT both be ratified.** One open item carried in from SI-004 for the ratifier to resolve at the ceremony: SI-004's `consult.expired` (consult-level age-out before review) has no equivalent in this 17-event table — ratifier decides (a) add an 18th event, (b) fold into `async_consult.intake_abandoned`, or (c) treat as non-audited internal scheduler state. Not authored here (would be net-new beyond ratified sub-decision scope per hard-floor item 6).
...
**Proposed RBAC roles (9 new; R11 HIGH-1 closure 2026-05-23 adds `consult_claim_wrapper_owner`):**
...
| `consult_claim_wrapper_owner` (non-BYPASSRLS) | Owns `claim_consult_for_review()` + `reassign_consult_claim()` SECURITY DEFINER procedures (claim-lifecycle family per Sub-decision 1 R3/R4/R5 closures); owner-only EXECUTE; no human grant (R11 HIGH-1 closure 2026-05-23 — without this owner role the claim procedures default to migration-superuser definer/BYPASSRLS, the P-034 R4 raw-writer-broadly-granted tenant-isolation hazard on a clinical-review claim-write path) | service-account-owner pattern |

**Promotion class:** content-change; RBAC v1.2 → v1.3 (9 new role definitions).
...
**Proposed tenant-threading rules (per SI-024.1 v0.8 RATIFIED canonical pattern + I-035 + I-026; R9 HIGH-1 closure 2026-05-21 expanded to **7 RLS-bearing entities** to match R1 closure addition of consult_review_claim):**

- All **7 new RLS-bearing entities** carry `tenant_id` (Telecheck-{country}) per I-023 layer 2:
  1. `consult`
  2. `consult_intake_submission`
  3. `consult_clinical_summary`
  4. **`consult_review_claim`** (R1 HIGH-1 closure entity; was previously omitted from the Sub-decision 7 count — fixed here)
  5. `consult_clinician_decision`
  6. `consult_lifecycle_transition`
  7. `consult_follow_up_message`
- RLS policies on all 7 entities key on `current_tenant_id_strict('<entity_name>')` per the SI-024.1 v0.8 canonical pattern (reads from `session_jwt_admission` cryptographically bound to current backend; Phase B fallback to GUC ONLY when entity's `phase_4_cutover_eligible=FALSE` AND `raw_guc_fallback_audited=TRUE`).
- Per-tenant KMS keys for `consult_intake_submission.intake_payload_kms_envelope` + `consult_clinical_summary.summary_kms_envelope` + `consult_clinician_decision.decision_rationale_kms_envelope` + `consult_follow_up_message.message_kms_envelope` per I-026 (PHI columns). `consult_review_claim` carries no PHI columns (clinician_account_id + timestamps + enum values only) so no KMS encryption required.
...
- **`jwt_migration_entity_status` seed scope: 7 RLS-bearing entity names** (all 7 tables enumerated above) + **1 derived view trust-anchor name** (`consult_outcome_summary_view`) = **8 total seed entries** with `phase_4_cutover_eligible=FALSE` AND `raw_guc_fallback_audited=TRUE` defaults (Phase B fail-closed-with-audit posture per P-036 R6 closure pattern). cdm_owner sequencing guidance: flip per-table `phase_4_cutover_eligible=TRUE` first (tables before view) as middleware migration to JWT-required posture completes; the derived view's trust-anchor flip happens last so all upstream writers are JWT-cutover before downstream readers per P-036 R6 sequencing discipline.
- Composite tenant-scoped FKs everywhere (patient + program + consult + admission + claim identity propagation per P-036 R2/R4 closures + R1 HIGH-1 + R3 HIGH-1 + R4 HIGH-1 claim ownership closures from this cycle).
...
**Proposed procedures (9 new; R11 HIGH-1 closure 2026-05-23 reconciles the 2 claim-lifecycle procedures from Sub-decision 1 into this enumeration — was ~7; mirrors SI-019 8.5 raw-writer-plus-wrappers pattern):**
...
- **`claim_consult_for_review()`** (claim-lifecycle SECURITY DEFINER procedure; canonical body in Sub-decision 1 per R3/R5 closures) — clinician claims a consult for review; advisory-lock-then-expired-claim-auto-release-then-INSERT under `(tenant_id, consult_id)`; emits `async_consult.claim_expired_auto_released` (Cat B) in the same transaction on STEP-2 auto-release; structured `claim_already_held` rejection on the partial-UNIQUE-INDEX conflict. Owned by `consult_claim_wrapper_owner` (Sub-decision 6); owner-only EXECUTE.
- **`reassign_consult_claim()`** (claim-lifecycle SECURITY DEFINER procedure; canonical body in Sub-decision 1 per R4 closure) — clinician handoff/takeover; advisory-lock-then-UPDATE-release-prior-then-INSERT-new in a single transaction; one-way `released_at`/`release_reason` mutation via the `consult_review_claim_one_way_released_at` trigger. Owned by `consult_claim_wrapper_owner` (Sub-decision 6); owner-only EXECUTE.

Per-reason wrappers prevent the raw-writer-broadly-granted defect (per P-034 R4 closure). Raw transition writer EXECUTE restricted to wrapper owner roles ONLY. **The 2 claim-lifecycle SECURITY DEFINER procedures (`claim_consult_for_review()` + `reassign_consult_claim()`) carry the same discipline (R11 HIGH-1 closure 2026-05-23): both are owned by the dedicated non-BYPASSRLS `consult_claim_wrapper_owner` with owner-only EXECUTE — without a dedicated owner role they would default to the migration-superuser definer (BYPASSRLS) and/or broad EXECUTE, the exact P-034 R4 hazard on a clinical-review claim-write path. This makes the procedure enumeration here, the Sub-decision 6 RBAC owner-role set, and the §3/§4 procedure count mutually consistent at 9.**
...
- **CDM:** +**7 new entities** (consult, consult_intake_submission, consult_clinical_summary, **consult_review_claim** [R1 HIGH-1 closure 2026-05-21 — durable claim/admission identity], consult_clinician_decision, consult_lifecycle_transition, consult_follow_up_message) + 1 plain data-minimization view (consult_outcome_summary_view) + 1 OPTIONAL materialized view (consult_current_state_mv) + **9 SECURITY DEFINER procedures** (7 lifecycle/decision writers — including extended `record_consult_clinician_decision` with claim-FK validation per R1 HIGH-1 closure — plus the 2 claim-lifecycle procedures `claim_consult_for_review()` + `reassign_consult_claim()` per R3/R4/R5 closures, reconciled into the enumeration + RBAC owner-role set at R11 HIGH-1 closure 2026-05-23).
- **AUDIT_EVENTS:** +**17 new action IDs** (4 Cat A + 3 Cat B + 10 Cat C per R7 MED-1 closure 2026-05-21 recount against actual table; R6 HIGH-1 closure added `claim_expired_auto_released` as required Cat B event for canonical observability of expired-claim auto-release; explicit Sampling column per P-036 pattern).
...
- **RBAC:** +9 new role definitions (v1.2 → v1.3; R11 HIGH-1 closure 2026-05-23 adds `consult_claim_wrapper_owner` — was 8).
- **Slice PRD:** v1.0 → v2.0 with the above codified.
- **Registry:** R8 MED-1 closure 2026-05-21 — **two-stage bump per established post-P-035/P-036 pattern**: P-037 SI ratification = Registry v2.23 → v2.24 (SI artifact lands at canonical bundle path; lockstep bump); P-038 follow-on amendment = Registry v2.24 → v2.25 (CDM/AUDIT/CCR/etc entities mechanically consolidated into named bundle files). The SI itself doesn't land the CDM entities into named CDM files — that happens at P-038 per the established post-P-029 SI-spec-first promotion pattern.
- **Promotion Ledger:** 2 new entries — **P-037** (this SI ratification + Registry v2.23 → v2.24) + **P-038** (follow-on CDM v1.8 → v1.9 + AUDIT_EVENTS v5.10 → v5.11 + DOMAIN_EVENTS additive + OpenAPI v0.3 → v0.4 + State Machines v1.2 → v1.3 + RBAC v1.2 → v1.3 mechanical consolidation + Registry v2.24 → v2.25).

**`jwt_migration_entity_status` seed scope at follow-on amendment (R9 + R10 HIGH-1 closures 2026-05-21):** **8 entity names** = **7 RLS-bearing tables** (`consult`, `consult_intake_submission`, `consult_clinical_summary`, **`consult_review_claim`** [explicit per R10], `consult_clinician_decision`, `consult_lifecycle_transition`, `consult_follow_up_message`) + **1 derived view trust-anchor surface** (`consult_outcome_summary_view`) with `phase_4_cutover_eligible=FALSE` AND `raw_guc_fallback_audited=TRUE` defaults (Phase B fail-closed-with-audit posture per P-036). cdm_owner sequencing guidance: flip per-table `phase_4_cutover_eligible=TRUE` first (all 7 tables), then the derived view's trust-anchor last so all upstream writers are JWT-cutover before downstream readers per P-036 R6 closure precedent.

---

## 4. Codex pre-ratification target

**Recommendation:** 6-10 rounds + 1 ship-it verification = 7-11 total. Async-Consult is a large slice (10 sub-decisions, 7 new entities, 9 SECURITY DEFINER procedures, 17 audit events, 7 domain events, 11 endpoints, 1 state machine with 22 transition triples, 9 RBAC roles, 1 optional MV + data-minimization view; counts reconciled at R11 MED-1 closure 2026-05-23 — §4 parenthetical was stale at 6 entities / 16 audit events / 7 procedures / 8 roles). Codex convergence likely 6-10 rounds based on SI-019 (7 rounds total post-OQ7) + Mode 1 (8 rounds; fresh-review-post-major-cycles) + CDM v1.7 follow-on (8 rounds) precedents. Expect similar HIGH-then-MED long-tail asymptote pattern.
...
Authored on `spec/si-020-async-consult-v2-0-implementation-readiness-2026-05-21` branch off main at `3b10b4c` (post-P-036 + Addendum 64). v0.2 commit `b34e7b0`. v0.3 commit `fca9e35`. v0.4 commit `79a4e24`. v0.5 commit `5cc2f1f`. v0.6 commit `3fca49c`. v0.7 commit `feb7cf0`. v0.8 commit `14a39e8`. v0.9 commit `391b4e7`. v0.10 commit `b2a93b9`. v0.11 commit landed on main (cockpit Addendum trail). v0.12 R11 author-side closure committed directly to telecheckONE main per cockpit Addendum 94 (docs-not-Codex-gated; the Codex pre-ratification ship-it cycle per §7 still gates ratification — Codex remains unavailable in the remote-cron env per Addenda 75–93).

**v0.12 DRAFT 2026-05-23 — R11 author-side readiness pass (1 HIGH + 1 MED closed inline; Track-6 row-shaping, NOT a Codex round — Codex unavailable in remote-cron env):**
- **R11 HIGH-1 closed:** the 2 claim-lifecycle SECURITY DEFINER procedures introduced across the R3/R4/R5 closures (`claim_consult_for_review()` + `reassign_consult_claim()`, with canonical bodies in Sub-decision 1) were never reconciled into (a) Sub-decision 8's procedure enumeration (raw writer + 6 per-reason wrappers = 7), (b) Sub-decision 6's RBAC owner-role set (8 roles; none owned the claim procedures), or (c) the §3/§4 "7 SECURITY DEFINER procedures" count. Per this SI's own discipline (Sub-decision 8: "Raw transition writer EXECUTE restricted to wrapper owner roles ONLY" per P-034 R4 closure), a SECURITY DEFINER procedure with no dedicated non-BYPASSRLS owner role + owner-only EXECUTE grant defaults to the migration-superuser definer (BYPASSRLS) and/or broad EXECUTE — the exact P-034 R4 raw-writer-broadly-granted tenant-isolation hazard, here on a clinical-review claim-write path that touches consult/patient identity. This is the direct procedure-count analogue of the R9/R10 entity-count lockstep closures (same R1/R3/R4/R5 claim additions; the enumeration/RBAC/count sections weren't updated when the procedures were added). Within-scope reconciliation — the procedures are already ratification-scoped, so no net-new architecture — hence closeable inline per hard-floor item 6's explicit "prose-consistency / within-scope clarification" exemption. Fix: added `consult_claim_wrapper_owner` non-BYPASSRLS role (Sub-decision 6: 8 → 9 roles) owning both claim procedures with owner-only EXECUTE; enumerated both procedures in Sub-decision 8 (~7 → 9) with the owner-role-restriction discipline restated; corrected §3 (7 → 9 procedures; 8 → 9 RBAC roles) + §4 (7 → 9 procedures; 8 → 9 roles). Procedure enumeration + RBAC owner-role set + §3/§4 counts now mutually consistent at 9.
- **R11 MED-1 closed:** §4's Codex-pre-ratification-target parenthetical inventory still said "6 new entities" + "16 audit events" — stale vs the R1 closure (7 entities incl. `consult_review_claim`) + R6 closure (17 audit events incl. `claim_expired_auto_released`) already applied to Sub-decision 1/2 + §3. The §4 parenthetical is the round-estimation rationale; a downstream reader sizing the Codex cycle off it would undercount the slice. Fix: §4 corrected to 7 entities + 17 audit events (+ the HIGH-1 procedure/role corrections) for full lockstep with Sub-decisions 1/2/6/8 + §3.

**v0.11 DRAFT 2026-05-21 — R10 closure applied (1 HIGH):**
- **R10 HIGH-1 closed:** §3 cross-artifact follow-on summary's seed-scope sentence said "7 entity names (6 RLS-bearing tables + 1 derived view trust-anchor surface)" — still reflecting pre-R9 entity count. Sub-decision 7 had been correctly expanded at R9 to 7 RLS-bearing tables + 1 derived view = 8 entries, but the §3 mechanical handoff sentence for P-038 wasn't updated in lockstep → P-038 follow-on amendment ratifier/implementer would have seeded only 6 table trust-anchors per the §3 instruction, leaving `consult_review_claim` unseeded → reintroducing the R9 tenant-isolation hazard. Fix: §3 follow-on summary explicitly enumerates all 8 entries with explicit table list including `consult_review_claim` + cdm_owner sequencing guidance restated; §3 + Sub-decision 7 now mutually consistent.

**v0.10 DRAFT 2026-05-21 — R9 closure applied (1 HIGH):**
- **R9 HIGH-1 closed:** Sub-decision 7 tenant-threading scope said "6 new RLS-bearing entities" but R1 closure added `consult_review_claim` as the 7th entity in Sub-decision 1. P-038 follow-on mechanically seeding 6 surfaces would leave `consult_review_claim` outside SI-024.1 fail-closed/audited fallback posture → tenant-isolation implementation hazard on a clinical-review claim entity. Fix: Sub-decision 7 expanded to explicit 7-entity enumeration with `consult_review_claim` properly tenant-scoped + `jwt_migration_entity_status` seed scope updated to 8 total (7 RLS-bearing tables + 1 derived view trust-anchor `consult_outcome_summary_view`) + cdm_owner sequencing guidance per P-036 R6 (tables first, view last). KMS-encryption clarification: `consult_review_claim` carries no PHI columns so no KMS envelope required (unlike the 4 other PHI-bearing tables).

**v0.9 DRAFT 2026-05-21 — R8 closure applied (1 MED):**
- **R8 MED-1 closed:** §3 cross-artifact summary said "Registry v2.23 → v2.24 single bump" but §7 ratification sequence said "Registry v2.24 → v2.25" for P-038 follow-on amendment. Two incompatible promotion paths in the same readiness artifact → P-037 / P-038 ratifier or implementer could skip or double-count the v2.24 registry state → ledger/registry drift. Fix: §3 corrected to two-stage bump per established post-P-035/P-036 pattern — P-037 SI ratification = Registry v2.23 → v2.24 (SI artifact lands at canonical bundle path; lockstep bump); P-038 follow-on amendment = Registry v2.24 → v2.25 (CDM/AUDIT/CCR/etc mechanical consolidation into named bundle files); 2 Promotion Ledger entries expected, not 1. §3 + §7 now mutually consistent.

**v0.8 DRAFT 2026-05-21 — R7 closure applied (1 MED):**
- **R7 MED-1 closed:** audit category counts in Sub-decision 2 prose + §3 cross-artifact summary said "4 Cat A + 6 Cat B + 7 Cat C" but actual table had 4 Cat A (IDs 9, 10, 11, 16) + 3 Cat B (IDs 6, 13, 17) + 10 Cat C (IDs 1-5, 7, 8, 12, 14, 15) = 17. Cat B/C controls differ on failure visibility + sampling posture; downstream P-038 follow-on amendment would have ratified wrong category split affecting retention/partition routing of 7 events. Fix: recounted prose against actual table; §3 cross-artifact updated; Sub-decision 2 recommendation paragraph clarified per-category rationale.
