...
### I-029 · Research data export 5-condition reject-unless gate

*(Originally stated as a 3-condition gate. Expanded to the canonical 5-condition gate v5.2 patch 2026-05-02 per Codex Round-4 Scope 2 HIGH-2 finding to align with AUDIT_EVENTS v5.2 §I-029 binding, STATE_MACHINES v1.1 ResearchExportRequest reject-unless rule, OpenAPI v0.2 `/research/exports/{export_id}/complete` endpoint contract, TYPES v5.2 `ResearchDataExport.invalidation_reason` enum, and GOVERNANCE_CONTROLS v5.2 §7.2 incident matrix. The 3-condition framing was a partial statement; the 5-condition gate is the canonical normative form.)*

Every research data export from the platform to a research partner requires, at minimum, ALL of the following 5 conditions to be present and valid at completion-time evaluation:
...
State machine validation MUST reject any **successful** export — i.e., the domain event `research_export.delivered` MUST NOT emit, the export artifact MUST NOT leave the platform, and the partner MUST NOT receive data — when ANY of the 5 conditions is unmet. The validator returns a structured error identifying which condition failed (mapping deterministically to one of the 5 `invalidation_reason` enum values; no "fallthrough other" bucket).
