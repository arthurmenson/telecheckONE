...
**Companion documents:** ADR Set v1.0, ADR Addendum 016–019, ADR Addendum 020–025 (with ADR-025 superseded), System Architecture v1.2, Master Platform PRD v1.10, Operational Readiness To-Do v1.5

> **C3 brand-structure + canonical-BAA-chain note (added 2026-05-02 per Codex Round-9 Scope 4 HIGH-1 + Round-12 Scope 4 HIGH-1 findings — bilateral with Registry v2.10 §3 row 10a):** This ADR was authored 2026-04-26 before the v1.10 cycle's C3 brand-structure cleanup and before the canonical 3-party BAA chain was articulated in System Architecture v1.2 §11.4. **Any prose in this ADR that uses bare `Heros` as a tenant/operator identifier OR states the BAA chain as `Heros → Telecheck → AWS US` is superseded in interpretation by:** (a) Master PRD v1.10 §17 brand-structure rule (operating-tenant identifiers are `Telecheck-{country}` — Telecheck-US, Telecheck-Ghana; consumer DBA is `Heros Health` country-instanced via subdomains, sourced from `tenant.consumer_dba` not `tenant.id`); (b) Glossary v5.2 §Brand and tenant terms (forbidden-alias rule against bare `Heros` outside §17 carve-outs); (c) System Architecture v1.2 §11.4 canonical 3-party BAA chain `Telecheck Health LLC (Telecheck-US tenant operator; Heros Health DBA consumer surface) → Telecheck parent/platform (separate business associate; data-plane operator + per-tenant KMS / RLS per ADR-023) → AWS US (subprocessor)`. Counsel review, subprocessor documentation, and launch-readiness evidence (per OR-303 canonical 3-party form per Round-3 Scope 4 HIGH-2 + Round-5 Scope 4 HIGH-3 patches) MUST treat all three parties separately and use `Telecheck-US` as the operating-tenant identifier, never bare `Heros`. The substantive regional-hosting decision (us-east-1 primary, us-west-2 cold DR; supersedes ADR-025) is not affected by this supersession-in-interpretation; only the tenant/operator identifier vocabulary and the BAA-chain articulation are. Any future re-issue of this ADR will physically rewrite the prose.
