...
- Canonical Data Model: **v1.2** (41 entities)
- State Machines: **v1.1** (14 state machines)
- OpenAPI: **v0.2** (178 endpoints, 21 modules)
- RBAC: **v1.1** (dual hierarchy)
- System Architecture: **v1.2** (15 modules; us-east-1 primary, us-west-2 cold DR per ADR-026)
- Engineering Handoff: **v1.3**
- OR Tracker: **v1.5**
- Contracts Pack: **v5.1** (filenames retain `v5_00` legacy pattern; headers govern)
...
- Design System: **v1.1**
- Design Implementation Contract: **v1.0 PROVISIONAL**
- Artifact Registry: **v2.9**
- Active Document Index: **v1.0** (refreshed 2026-04-26 for US Region Migration Cycle U-002)

**Architecture decision set (canonical):** ADR Set v1.0 + ADR Addendum 016–019 + ADR Addendum 020–025 (with **ADR-025 superseded by ADR-026**) + **ADR Addendum 026** (single-region us-east-1 primary, us-west-2 cold DR; supersedes ADR-025).
...
- `Telecheck_Canonical_Data_Model_v1_2.md` §3, §4, §4-bis
- `Telecheck_State_Machines_v1_1.md` §1–§16
- `Telecheck_OpenAPI_v0_2.md` (endpoint shapes)
- `Telecheck_Contracts_Pack_v5_00_*.md` (cross-cutting invariants, types, error model)
...
- Cross-tenant access requires break-glass per RBAC v1.1 + audit per AUDIT_EVENTS v5.1.
...
## 7. Design Implementation Contract status (HIGH-11 product decision)

- DIC v1.0 is **PROVISIONAL** until design files are delivered.
- During provisional period: build frontend per Design System v1.1 + IA documents only.
- Pixel-exact-match requirement does NOT apply during provisional period.
- When design files arrive: DIC bumps to v1.1 "Canonical for development"; reimplementation cycles begin.
...
| Filename `v5_00` vs header `v5.1` (Contracts Pack) | Header wins (v5.1 is canonical). |
...
6. Read relevant Contracts Pack v5.1 invariants (especially I-001 audit, I-024 break-glass, I-025 information-leak prevention, I-027 tenant isolation)
7. Check Operational Readiness Tracker v1.4 for any open Tier-1/Tier-2 items in scope
...
- It does NOT cover product decisions. Those live in Master PRD v1.9 and the ADR Set (including ADR Addendum 026).
