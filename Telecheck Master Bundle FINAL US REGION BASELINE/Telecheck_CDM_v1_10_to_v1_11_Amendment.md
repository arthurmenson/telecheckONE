...
4. `POST /v1/admin/templates/{template_id}/submit-for-review` — calls `submit_forms_template_for_admin_review` wrapper; requires admin_basic_operator role ONLY at LAYER B (R6 HIGH-3 closure 2026-05-22; builder-role direct-submit deferred to post-pilot Admin Backend v1.1); emits `admin.template_submitted_for_review` Cat A audit + creates forms_template_admin_review row + initial `none → pending_review` lifecycle_transition row co-transactionally. **No `Idempotency-Key` HTTP header is required for this endpoint** (P-042 R3 HIGH-2 + R4 HIGH-1 closures 2026-05-22 — the canonical SI-023 `submit_forms_template_for_admin_review(tenant_id_t, UUID)` wrapper signature does not accept an idempotency key + there is no submit-side idempotency-key table; advertising the header would have been misleading because a lost-response retry with same advertised key cannot replay the original review_id — it would hit the active-review guard and return `admin-template-submit-already-in-flight` error). **Retry semantics + lost-response recovery (P-042 R4 HIGH-1 closure 2026-05-22 — recovery path scoped to pilot-acceptable operational handling rather than promising a lookup endpoint outside this amendment's OpenAPI surface):** the initial-submission path is NOT idempotent at the API layer in pilot scope; a lost-response retry returns `admin-template-submit-already-in-flight` (40001 serialization_failure) which the HTTP layer surfaces to the client as 409 Conflict. Pilot-scope lost-response recovery is **operational rather than API-driven**: the operator (or platform support) recovers the original review_id via the canonical admin SQL surface (direct SELECT against `forms_template_admin_review` for the affected `(tenant_id, forms_template_id)` with `latest_state IN ('pending_review','revision_requested')` derived via the §4.NEW3 lifecycle_transition table). No new `GET /v1/admin/template-reviews` lookup endpoint is added to this amendment's OpenAPI surface (P-042's 5-endpoint scope matches SI-023 §5 verbatim; adding a 6th endpoint would silently fork from the ratified SI). The revision-resubmission path is naturally idempotent at the wrapper layer (re-submits reuse the existing review_id; concurrent re-submits serialize at the parent-template FOR UPDATE lock per R8 HIGH-1 + R11 HIGH-1 closures). **Pilot-acceptability rationale:** the day-1 pilot tenants (Telecheck-US greenfield + Telecheck-Ghana) operate with a small admin-operator team; lost-response retries on initial-submission are expected to be infrequent + the operational SQL-console recovery path is acceptable. **Post-pilot v1.1 hardening:** if observed lost-response retry frequency justifies API-driven recovery, v1.1 will add either (a) a `GET /v1/admin/template-reviews` lookup endpoint with admin_basic_operator RBAC + tenant isolation + Cat A audit emission per dashboard-read pattern, OR (b) submit-side idempotency-key support extending the canonical wrapper signature + a new `admin_submit_idempotency_key` entity. The choice between (a) and (b) is a v1.1 ratifier decision based on which pattern better fits the observed retry-storm profile.
