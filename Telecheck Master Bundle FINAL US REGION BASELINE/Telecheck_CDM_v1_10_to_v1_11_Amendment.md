...
1. **Phase 1 — RBAC + ownership setup (P-042 R7 HIGH-2 + R8 MED-1 closures 2026-05-22 — 12 roles, not 11):** create the **12 net-new RBAC roles** per §6 (2 application + 3 dashboard-wrapper-owner + 2 template-wrapper-owner + 1 raw-writer-owner + 4 view-owner including the new `forms_template_admin_review_pending_view_owner` per §4.NEW9); set role passwords via KMS-bound credential vault. Then execute the §4.NEW8g `GRANT` statements assigning each wrapper-owner role its exact least-privilege DML/EXECUTE set (per the P-042 R7 HIGH-1 + R8 HIGH-1 closures — without these grants, the wrappers fail at runtime with permission_denied under SECURITY DEFINER semantics; UPDATE grants on `forms_template` + `forms_template_admin_review` are required by `SELECT ... FOR UPDATE` even though append-only triggers prevent actual mutation).
...
6. **Phase 6 — Views FIRST** (per SI-023 R6 HIGH-1 closure — wrapper-only design forces view-before-wrapper deployment; P-042 R8 MED-1 closure 2026-05-22 — pending view added to this phase): create the 3 admin dashboard views (`admin_crisis_operational_health_v` + `admin_consult_queue_health_v` + `admin_mode1_volume_health_v`) with `security_invoker=true + security_barrier=true`; set ownership to corresponding view-owner role; REVOKE ALL FROM PUBLIC; GRANT SELECT ONLY to corresponding dashboard read-wrapper-owner role. **Plus create the §4.NEW9 reviewer pending-only view (`forms_template_admin_review_pending_v`) with `security_barrier=true` + `security_invoker=false`** (so the LATERAL JOIN to `forms_template_admin_review_lifecycle_transition` runs under view-owner privileges, preserving wrapper-only read discipline); set ownership to `forms_template_admin_review_pending_view_owner`; REVOKE ALL FROM PUBLIC; GRANT SELECT on the view to `admin_template_reviewer`; GRANT SELECT on `forms_template_admin_review` + `forms_template_admin_review_lifecycle_transition` to the pending-view-owner. Phase 6.1 CTAS provenance event trigger REUSED from P-040 (admin views — including the pending view — become new dependents in class K allowlist).
