...
**Scheduling guidance:** the canonical platform scheduler (pg_cron, AWS EventBridge → Lambda, or operational equivalent — established at platform-floor level outside SI-023 scope) registers a job to call `monitor_admin_dashboard_audit_completeness()` hourly. The scheduler service is the SOLE caller (EXECUTE grant restricted to `platform_scheduler_service` only); operators do not invoke the monitor directly. **Scheduler configuration lives in operational runbooks (not the SQL spec corpus);** the SQL-side preflight class N.0 verifies the FUNCTION exists + the MONITOR-OWNER role exists + the SCHEDULER SERVICE has EXECUTE, which is the strongest assertion possible from within the database before the scheduler is configured. The operational runbook for cutover MUST register the hourly job in the platform scheduler as part of the Phase 11 deployment checklist (added below).
...
11. **Phase 11 — OpenAPI endpoint deployment + operational scheduler registration (P-042 R17 HIGH-1 closure 2026-05-22 — scheduler registration added):** (a) deploy 5 net-new endpoints under `/v1/admin/*` per §4; verify caller-class role requirement enforced at endpoint layer (defense-in-depth alongside SECDEF wrapper LAYER A+B+C). (b) **Register the class N.1 hourly job in the canonical platform scheduler** (pg_cron, AWS EventBridge → Lambda calling `SELECT monitor_admin_dashboard_audit_completeness()`, or operational equivalent established at platform-floor level outside SI-023 scope). The runbook step MUST verify the scheduler service-account has the required EXECUTE grant on the monitor function (granted at Phase 7 step (c)) + that the hourly cadence matches the canonical Cat C drift-detection window. Scheduler registration is operational config (lives in deployment runbooks, NOT the SQL spec corpus); the SQL-side N.0 preflight assertion confirms the deployable artifact + owner role + EXECUTE grant exist, which is the strongest SQL-side guarantee possible before the scheduler is configured.
