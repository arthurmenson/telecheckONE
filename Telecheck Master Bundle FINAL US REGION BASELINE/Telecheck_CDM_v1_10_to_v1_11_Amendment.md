...
5. **RBAC v1.4 → v1.5:** **+13 net-new role definitions** (P-042 R7 HIGH-2 + R17 HIGH-1 closures 2026-05-22 — +1 view-owner for pending view + +1 monitor-owner for class N.1 deployable function). Application roles (2): `admin_basic_operator`, `admin_template_reviewer`. Dashboard-wrapper-owner roles (3): `read_admin_crisis_operational_health_wrapper_owner`, `read_admin_consult_queue_health_wrapper_owner`, `read_admin_mode1_volume_health_wrapper_owner`. Template-wrapper-owner roles (2): `forms_template_admin_review_submit_wrapper_owner`, `forms_template_admin_review_decision_wrapper_owner`. Raw-writer-owner role (1): `forms_template_admin_review_transition_writer_owner`. View-owner roles (4): `admin_crisis_operational_health_view_owner`, `admin_consult_queue_health_view_owner`, `admin_mode1_volume_health_view_owner`, `forms_template_admin_review_pending_view_owner`. **Monitor-owner role (1, NEW at R17 HIGH-1 closure): `admin_dashboard_audit_completeness_monitor_owner`** (owns the class N.1 deployable function `monitor_admin_dashboard_audit_completeness()`).
...
| 13 | `admin_dashboard_audit_completeness_monitor_owner` | monitor-owner | n/a (**BYPASSRLS** NOLOGIN; P-042 R17 HIGH-1 + R18 HIGH-1 closures 2026-05-22 — BYPASSRLS is REQUIRED because the cross-tenant aggregation in the N.1 monitor reads `admin_dashboard_query_execution` rows across all tenants, but the table is FORCE RLS-protected via `tenant_id = current_tenant_id_strict(...)`; without BYPASSRLS the monitor would either RAISE (no tenant GUC set in scheduler context) or filter to zero rows and never detect drift; this is the canonical cross-tenant operational monitor pattern matching the break-glass framework from P-040 §8.1 class H — BYPASSRLS scoped to a single read-only aggregation function with EXECUTE restricted to `platform_scheduler_service` only, NOT a general application-runtime path) | OWNS `monitor_admin_dashboard_audit_completeness()` SECDEF function deployed at Phase 7 for class N.1 hourly drift detection; SELECT on `admin_dashboard_request_log_v` + `admin_dashboard_query_execution`; EXECUTE on `emit_audit_event_co_transactional()`; the function's EXECUTE is granted ONLY to `platform_scheduler_service` (canonical platform-floor scheduler role). **BYPASSRLS rationale documented in §6 narrative below + class L is REVERSED for this owner-role: monitor-owner is the only canonical exception to the non-BYPASSRLS discipline for owner roles, justified by cross-tenant audit-completeness scope.** |
...
**Class A — Verify the 13 net-new RBAC roles exist** (P-042 R7 HIGH-2 + R17 HIGH-1 closures 2026-05-22 — extended by +1 to include `forms_template_admin_review_pending_view_owner` then +1 more for `admin_dashboard_audit_completeness_monitor_owner`):
...
BEGIN
    FOR v_role_missing IN
        SELECT unnest(ARRAY[
            ...
            -- Monitor-owner role (1; P-042 R17 HIGH-1: class N.1 deployable function owner)
            'admin_dashboard_audit_completeness_monitor_owner'
        ])
...
**Class N — Admin dashboard audit completeness tripwire** (P-042 R14 + R15 + R16 closures 2026-05-22 — split into TWO sub-classes: N.0 **BLOCKING** input-contract assertion + N.1 **NON-BLOCKING** post-deploy drift monitor):

Rationale for split: v0.5-v0.13 class N was a single blocking gate that produced noisy false-positives from global APM-vs-DB count comparison (R14). v0.15 demoted entirely to non-blocking which surfaced two new problems: (a) `current_tenant_id_strict` GUC dependency made the monitor unreliable in post-deploy contexts (R15); (b) soft-skip on missing input columns meant deploys with older APM view shapes got ZERO audit-completeness coverage with no signal — a real observability regression (R16). v0.17 final design: **class N.0 (blocking input-contract assertion)** verifies `admin_dashboard_request_log_v` exists + carries `tenant_id` + `dashboard_name` columns at Phase 10 cutover gate — if missing, BLOCK cutover with explicit error pointing at the required APM-schema-propagation work. **Class N.1 (non-blocking operational monitor)** runs post-deploy, per-(tenant, dashboard) drift detection, emits Cat C audit on detected drift without blocking. This split gives the right operational properties: monitor coverage is GUARANTEED to exist at deploy time (N.0); spurious count-comparison noise doesn't block ships (N.1); silent observability regression eliminated.

**Class N.0 — Admin dashboard audit completeness input-contract assertion (BLOCKING; runs at Phase 10):**
...
BEGIN
    ...
    IF NOT v_log_view_exists THEN
        RAISE EXCEPTION 'si-023-admin-dashboard-request-log-view-missing: admin_dashboard_request_log_v view does NOT exist — required by class N.1 operational monitor + the §3 row 5 Cat C tripwire contract; deploy MUST provision the APM-sourced view before cutover';
    END IF;

    SELECT
        EXISTS(SELECT 1 FROM information_schema.columns
                WHERE table_name = 'admin_dashboard_request_log_v' AND column_name = 'tenant_id'),
        EXISTS(SELECT 1 FROM information_schema.columns
                WHERE table_name = 'admin_dashboard_request_log_v' AND column_name = 'dashboard_name'),
        EXISTS(SELECT 1 FROM information_schema.columns
                WHERE table_name = 'admin_dashboard_request_log_v' AND column_name = 'response_status'),
        EXISTS(SELECT 1 FROM information_schema.columns
                WHERE table_name = 'admin_dashboard_request_log_v' AND column_name = 'request_at')
    ...
    IF NOT v_log_view_has_tenant_id OR NOT v_log_view_has_dashboard_name
       OR NOT v_log_view_has_response_status OR NOT v_log_view_has_request_at THEN
        RAISE EXCEPTION 'si-023-admin-dashboard-request-log-schema-violation: admin_dashboard_request_log_v MUST carry tenant_id + dashboard_name + response_status + request_at columns (has tenant_id=%, dashboard_name=%, response_status=%, request_at=%); deploy MUST propagate JWT-verified tenant_id + endpoint dashboard_name + HTTP response_status + request timestamp from the APM/HTTP layer into this view before cutover',
            v_log_view_has_tenant_id, v_log_view_has_dashboard_name, v_log_view_has_response_status, v_log_view_has_request_at;
    END IF;

    -- P-042 R17 HIGH-1 closure 2026-05-22: verify N.1 monitor deployable artifact + owner role exist
    IF NOT EXISTS(SELECT 1 FROM pg_proc p
                   JOIN pg_namespace n ON n.oid = p.pronamespace
                  WHERE p.proname = 'monitor_admin_dashboard_audit_completeness'
                    AND n.nspname = 'public') THEN
        RAISE EXCEPTION 'si-023-admin-dashboard-completeness-monitor-function-missing: monitor_admin_dashboard_audit_completeness() function MUST exist before cutover passes; Phase 7 deploys it';
    END IF;
    IF NOT EXISTS(SELECT 1 FROM pg_roles WHERE rolname = 'admin_dashboard_audit_completeness_monitor_owner') THEN
        RAISE EXCEPTION 'si-023-admin-dashboard-completeness-monitor-owner-missing: admin_dashboard_audit_completeness_monitor_owner role MUST exist (13th net-new RBAC role per §6); Phase 1 creates it';
    END IF;
    -- Verify monitor function is OWNED by the canonical monitor-owner role (anti-drift)
    IF NOT EXISTS(SELECT 1 FROM pg_proc p
                   JOIN pg_namespace n ON n.oid = p.pronamespace
                   JOIN pg_roles r ON r.oid = p.proowner
                  WHERE p.proname = 'monitor_admin_dashboard_audit_completeness'
                    AND n.nspname = 'public'
                    AND r.rolname = 'admin_dashboard_audit_completeness_monitor_owner') THEN
        RAISE EXCEPTION 'si-023-admin-dashboard-completeness-monitor-owner-mismatch: monitor_admin_dashboard_audit_completeness() ownership MUST be admin_dashboard_audit_completeness_monitor_owner';
    END IF;

    -- P-042 R18 HIGH-1 closure 2026-05-22: monitor-owner MUST have BYPASSRLS so cross-tenant
    -- aggregation in the SECDEF function body can read RLS-FORCED admin_dashboard_query_execution
    -- without raising on missing tenant GUC or filtering to zero rows. BYPASSRLS is the canonical
    -- exception for cross-tenant operational monitors per the P-040 §8.1 class H break-glass
    -- framework pattern; scoped to EXECUTE by platform_scheduler_service only.
    IF NOT EXISTS(SELECT 1 FROM pg_roles
                   WHERE rolname = 'admin_dashboard_audit_completeness_monitor_owner'
                     AND rolbypassrls = TRUE) THEN
        RAISE EXCEPTION 'si-023-admin-dashboard-completeness-monitor-owner-bypassrls-missing: admin_dashboard_audit_completeness_monitor_owner MUST hold BYPASSRLS attribute per P-042 R18 HIGH-1 (cross-tenant aggregation in the N.1 monitor function requires bypassing RLS); set via ALTER ROLE ... BYPASSRLS at Phase 1 role creation';
    END IF;

    -- P-042 R18 HIGH-2 closure 2026-05-22: verify platform_scheduler_service role exists +
    -- holds EXECUTE on the monitor function. Without these, the scheduler cannot call the
    -- monitor + the Cat C tripwire is silently inactive.
    IF NOT EXISTS(SELECT 1 FROM pg_roles WHERE rolname = 'platform_scheduler_service') THEN
        RAISE EXCEPTION 'si-023-platform-scheduler-service-role-missing: platform_scheduler_service role MUST exist (canonical platform-floor scheduler role; pre-deployed); without it the N.1 monitor cannot be invoked';
    END IF;
    IF NOT has_function_privilege('platform_scheduler_service',
                                  'monitor_admin_dashboard_audit_completeness()',
                                  'EXECUTE') THEN
        RAISE EXCEPTION 'si-023-scheduler-monitor-execute-grant-missing: platform_scheduler_service MUST hold EXECUTE on monitor_admin_dashboard_audit_completeness() per Phase 7 step (c); without this grant the scheduler cannot invoke the N.1 monitor + Cat C tripwire is silently inactive';
    END IF;

    -- Anti-bypass: verify NO other role besides platform_scheduler_service holds EXECUTE on the monitor
    -- (the monitor should not be invokable by application roles; SECDEF call chains might otherwise
    -- propagate BYPASSRLS effects through wrappers)
    DECLARE
    ...
    BEGIN
        FOR v_unauthorized_grantee IN
            SELECT g.grantee
             ...
             WHERE g.routine_name = 'monitor_admin_dashboard_audit_completeness'
               AND g.privilege_type = 'EXECUTE'
               AND g.grantee NOT IN ('platform_scheduler_service',
                                     'admin_dashboard_audit_completeness_monitor_owner')
        LOOP
            RAISE EXCEPTION 'si-023-monitor-execute-grant-violation: monitor_admin_dashboard_audit_completeness() EXECUTE granted to non-canonical role %; canonical grantee set = {platform_scheduler_service, monitor-owner (self)}', v_unauthorized_grantee;
        END LOOP;
    END;
END $$;
```

**Class N.1 — Admin dashboard audit completeness drift monitor (NON-BLOCKING; deployable as a SECURITY DEFINER function scheduled to run hourly post-deploy):**

P-042 R17 HIGH-1 closure 2026-05-22: v0.16-v0.17 documented N.1 as an inline DO block, which would have left N.1 a "spec only" control — operators reading the cutover sequence have no concrete deployment artifact, no owner, no schedule, and no preflight assertion that the monitor is enabled. That recreates the R16 failure mode in a different form: N.0 passes because the input contract exists, but if no operator manually runs the DO block, zero drift checks happen and Cat C never fires. Fix: convert N.1 into a deployable SECURITY DEFINER function `monitor_admin_dashboard_audit_completeness()` owned by a new RBAC role `admin_dashboard_audit_completeness_monitor_owner` (**13th net-new RBAC role**), deployed in Phase 7 alongside the other SECDEF procedures, with explicit scheduling guidance (hourly cadence via the canonical platform scheduler). Class N.0 extended to verify the function exists + the monitor-owner role exists.

**Deployable function** (Phase 7 — SECURITY DEFINER + locked search_path + owned by monitor-owner role):

```sql
CREATE OR REPLACE FUNCTION monitor_admin_dashboard_audit_completeness()
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public
...
DECLARE
    v_row RECORD;
BEGIN
    -- Per-(tenant, dashboard) drift detection. N.0 has already validated the input contract
    -- at cutover gate, so this monitor can assume admin_dashboard_request_log_v exists +
    -- carries the required columns. One Cat C event emitted per (tenant, dashboard) with
    -- non-zero drift; tenant_id passed explicitly so no GUC dependency.
    FOR v_row IN
    ...
    LOOP
        PERFORM emit_audit_event_co_transactional(
            ...
            jsonb_build_object(
                ...
                'drift', v_row.drift,
                'monitor_kind', 'operational_post_deploy_hourly'
            )
        );
        RAISE NOTICE 'si-023-admin-dashboard-audit-completeness-drift: tenant=% dashboard=% drift=% (expected=%, actual=%); Cat C audit emitted; not blocking',
            v_row.tenant_id, v_row.dashboard_name, v_row.drift, v_row.expected_count, v_row.actual_count;
    END LOOP;
END;
$$;

ALTER FUNCTION monitor_admin_dashboard_audit_completeness()
    OWNER TO admin_dashboard_audit_completeness_monitor_owner;
REVOKE EXECUTE ON FUNCTION monitor_admin_dashboard_audit_completeness() FROM PUBLIC;
-- EXECUTE granted ONLY to the canonical platform scheduler role (pg_cron-equivalent or
-- the operational scheduler service-account; established at platform-floor level outside
-- SI-023 scope) — anti-bypass: the monitor should not be invokable by application roles.
GRANT EXECUTE ON FUNCTION monitor_admin_dashboard_audit_completeness()
    TO platform_scheduler_service;  -- pre-existing canonical scheduler role (platform-floor)
```

**Function grants for the monitor-owner** (added in Phase 7 alongside the function deployment):

```sql
GRANT SELECT ON admin_dashboard_request_log_v TO admin_dashboard_audit_completeness_monitor_owner;
GRANT SELECT ON admin_dashboard_query_execution TO admin_dashboard_audit_completeness_monitor_owner;
GRANT EXECUTE ON FUNCTION emit_audit_event_co_transactional(tenant_id_t, TEXT, JSONB)
    TO admin_dashboard_audit_completeness_monitor_owner;
```

**Scheduling guidance + heartbeat-based residual coverage (P-042 R19 HIGH-1 closure 2026-05-22):** the canonical platform scheduler registers a job calling `monitor_admin_dashboard_audit_completeness()` hourly. Scheduler service is the SOLE caller. **Scheduler-side registration cannot be mechanically verified at SQL preflight time** — the scheduler backend is platform-floor (pg_cron / AWS EventBridge / equivalent) + outside the SQL spec corpus. R19 HIGH-1 flagged this as a residual silent-coverage risk: a deploy could pass Phase 10's blocking N.0 assertions (function exists + callable + correctly scoped) AND complete Phase 11 OpenAPI deployment, but if the runbook step (c) "register hourly job in scheduler" is skipped, no drift checks run.

**Residual scheduler-registration gap (P-042 R19 HIGH-1 closure 2026-05-22 — documented as known-residual + routed through existing canonical observability infrastructure rather than expanding SI-023's audit-event roster):**

Scheduler-side registration cannot be mechanically verified at SQL preflight time. SQL preflight can verify everything the spec needs UP TO callable state: function exists + correctly owned + executable by `platform_scheduler_service` + anti-bypass grants verified (R18 HIGH-2 closure). What it CANNOT verify without forking SI-023's scope is whether the platform scheduler backend (pg_cron / AWS EventBridge / equivalent) has actually registered the hourly job. The chicken-and-egg property is inherent: the scheduler backend is platform-floor, outside SI-023's SQL surface; introducing scheduler-backend assertions in this amendment would silently fork from SI-023's 5-endpoint + 6-audit-event scope.

**The R19 residual gap is closed via two existing canonical mechanisms** (no new SI-023 entities + no new SI-023 audit-event action_ids):

1. **Operational runbook gate (Phase 11 step (c), see §8.2 below):** the canonical cutover runbook adds a manual sign-off step verifying that (i) the scheduler job is registered + (ii) the first scheduled invocation completes within 2× the canonical cadence (2h) post-cutover. Failure to verify is a deployment-incomplete signal and triggers cutover rollback per §8.3. This is an OPERATIONAL gate, not a SQL preflight gate; same kind of human sign-off step the P-040 cutover runbook uses for the CTAS provenance event trigger registration.

2. **Platform-observability-side detection (outside SI-023 scope):** the canonical platform observability infrastructure (Datadog / Honeycomb / equivalent — established at platform-floor level) monitors expected audit-event emission rates per slice. If `admin.dashboard_query_executed` Cat A emission rate drops to zero for a tenant for >2h (no successful dashboard reads happening), the platform observability tripwire fires regardless of whether the SI-023-specific drift tripwire (`admin.dashboard_query_audit_completeness_violation`) does. Operators receive an out-of-band alert; the scheduler-registration omission is detected within hours, not weeks. This is the same belt-and-suspenders pattern the P-040 break-glass framework uses for its own monitor-freshness checks.

**Post-pilot v1.1 hardening path** (documented for ratifier awareness; deferred until pilot operational data justifies the work): if observed scheduler-registration failures occur during pilot, v1.1 will introduce a canonical platform-level `scheduler_registration` entity at platform-floor scope (NOT in SI-023) with a generic preflight assertion API that all slices' monitors can plug into. SI-023 v1.1 then references the canonical platform assertion rather than re-implementing per-slice. This is the right scope for the work — SI-023 should not single-handedly invent a scheduler-registration verification framework.

**R19 HIGH-1 acceptance:** P-042 closes R19 as **documented-residual-with-canonical-mitigation** rather than introducing a NEW deployable artifact (e.g., heartbeat table + 7th audit-event-id) that would silently fork SI-023's scope. The two-pronged operational mitigation (runbook sign-off + platform observability detection) closes the practical observability gap; the SQL-side N.0 assertions guarantee everything achievable before the scheduler is configured. This matches the canonical P-042 closure pattern from R4 HIGH-1 (lost-response recovery scoped to operational runbook rather than API endpoint addition) and R14 HIGH-1 (class N demoted to operational monitor rather than blocking gate) — the right scope-discipline answer for cross-slice operational concerns at pilot scale.

**Inline DO block usage (deprecated post-Phase 7):** the inline DO-block version of N.1 below is preserved ONLY as a deployment-time manual override for ad-hoc operator-initiated drift checks (e.g., during incident response). The canonical N.1 mechanism is the scheduled function above.

N.1 input contract is guaranteed by N.0's blocking assertion (above); the monitor can therefore assume `admin_dashboard_request_log_v` exists + carries `tenant_id` + `dashboard_name` + `response_status` + `request_at`. Per-(tenant, dashboard) drift comparison with one Cat C event per drifting pair:
...
DECLARE
    v_row RECORD;
BEGIN
    -- N.0 has already verified the input contract; this monitor assumes the columns exist.
    -- Per-(tenant, dashboard) drift comparison. expected_count = successful (2xx) HTTP requests
    -- against the 3 dashboard endpoints aggregated per (tenant, dashboard); actual_count =
    -- admin_dashboard_query_execution rows aggregated per (tenant, dashboard). One Cat C audit
    -- emitted per (tenant, dashboard) with non-zero drift; tenant_id passed explicitly so no
    -- GUC dependency.
    FOR v_row IN
        WITH expected_per_tenant AS (
            SELECT log.tenant_id, log.dashboard_name, COUNT(*) AS expected_count
    ...
    LOOP
        PERFORM emit_audit_event_co_transactional(
            ...
            jsonb_build_object(
                ...
                'actual_count', v_row.actual_count,
                'drift', v_row.drift,
                'monitor_kind', 'operational_post_deploy'
            )
        );
        RAISE NOTICE 'si-023-admin-dashboard-audit-completeness-drift: tenant=% dashboard=% drift=% (expected=%, actual=%); Cat C audit emitted; not blocking cutover',
            v_row.tenant_id, v_row.dashboard_name, v_row.drift, v_row.expected_count, v_row.actual_count;
    END LOOP;
END $$;
```

**Post-pilot v1.1 hardening:** if observed drift frequency in production justifies tighter correlation (e.g., per-request identity matching rather than per-(tenant, dashboard) count comparison), v1.1 will add a `request_id UUID` column propagated from the HTTP layer through to `admin_dashboard_query_execution` so class N can do exact correlation. This is a v1.1 ratifier decision based on observed-drift profile.
...
1. **Phase 1 — RBAC role creation only (P-042 R9 HIGH-1 + R17 HIGH-1 + R18 HIGH-1 closures 2026-05-22):** create the **13 net-new RBAC roles** per §6 (2 application + 3 dashboard-wrapper-owner + 2 template-wrapper-owner + 1 raw-writer-owner + 4 view-owner + 1 monitor-owner); set role passwords via KMS-bound credential vault. **`admin_dashboard_audit_completeness_monitor_owner` MUST be created with `BYPASSRLS` attribute** (per R18 HIGH-1 — required for cross-tenant aggregation in the N.1 monitor; canonical exception to non-BYPASSRLS owner-role discipline; scoped to EXECUTE by `platform_scheduler_service` only). All other 12 net-new roles are NOLOGIN + non-BYPASSRLS per §6 standard discipline. **Pre-existing-dependency GRANTs that target already-deployed canonical entities** (from P-027/P-031): `SELECT ON tenant_account_membership` to all 6 SECDEF wrapper-owners + `EXECUTE ON verify_session_jwt_and_extract_claims()` to all 6 SECDEF wrapper-owners + `EXECUTE ON emit_audit_event_co_transactional(tenant_id_t, text, jsonb)` to 5 wrapper-owners (excluding the raw lifecycle writer which doesn't emit audit). NO grants on SI-023 entities (admin tables, admin views, raw writer function) in this phase — those are deferred to Phase 2 / Phase 6 / Phase 7 below where the target objects exist.
...
7. **Phase 7 — Procedures + function-level GRANTs + class N.1 monitor function (P-042 R9 HIGH-1 + R17 HIGH-1 closures 2026-05-22):** (a) deploy the 6 SECDEF procedures per §4.NEW8a-f (1 raw + 3 dashboard read-wrappers + 2 template wrappers); each SECURITY DEFINER + locked search_path; ownership set per §6; REVOKE EXECUTE FROM PUBLIC + GRANT EXECUTE to application roles per the inline DDL in §4.NEW8b-f. (b) **Execute the function EXECUTE GRANTs from §4.NEW8g**: `GRANT EXECUTE ON FUNCTION record_forms_template_admin_review_transition(...) TO forms_template_admin_review_submit_wrapper_owner, forms_template_admin_review_decision_wrapper_owner` (anti-bypass). (c) **Deploy the class N.1 operational monitor function `monitor_admin_dashboard_audit_completeness()`** per §8.1 class N.1 (SECURITY DEFINER + locked search_path + owned by `admin_dashboard_audit_completeness_monitor_owner` + REVOKE EXECUTE FROM PUBLIC + GRANT EXECUTE to `platform_scheduler_service`). (d) Execute monitor-owner grants: SELECT on `admin_dashboard_request_log_v` + SELECT on `admin_dashboard_query_execution` + EXECUTE on `emit_audit_event_co_transactional()` to `admin_dashboard_audit_completeness_monitor_owner`.
...
10. **Phase 10 — Deployment preflight gate (P-042 R8 MED-1 + R14 HIGH-1 + R16 HIGH-1 closures 2026-05-22):** run §8.1 DO block — **blocking classes A-M + N.0 + O + P + Q**. Class N.0 (admin dashboard request log input-contract assertion per R16 HIGH-1) verifies `admin_dashboard_request_log_v` exists + carries `tenant_id` + `dashboard_name` + `response_status` + `request_at` columns; any missing column BLOCKS cutover (prevents silent observability regression — operators learn at deploy time that HTTP-layer/APM propagation work is required). Class Q (per R7 HIGH-1 + R8 HIGH-1) verifies wrapper-owner DML/EXECUTE privileges. Any FAILED assertion BLOCKS cutover. **Class N.1 (drift detection) moved to post-deploy operational monitor** (per R14 HIGH-1 closure — global count comparison is too coarse for blocking preflight; runs post-deploy as Cat C tripwire emission without blocking ships; N.0 guarantees the monitor's input contract is met at deploy time so N.1 cannot silently skip).
11. **Phase 11 — OpenAPI endpoint deployment + operational scheduler registration + post-deploy verification (P-042 R17 HIGH-1 + R19 HIGH-1 closures 2026-05-22):** (a) deploy 5 net-new endpoints under `/v1/admin/*` per §4; verify caller-class role requirement enforced at endpoint layer. (b) **Register the class N.1 hourly job in the canonical platform scheduler** (pg_cron, AWS EventBridge → Lambda calling `SELECT monitor_admin_dashboard_audit_completeness()`, or operational equivalent established at platform-floor level outside SI-023 scope). (c) **R19 HIGH-1 closure — post-deploy operational sign-off gate:** within 2h of cutover completing, the cutover operator MUST verify (i) the scheduler job is registered + active in the platform scheduler backend; (ii) the first scheduled invocation completes successfully (visible via the canonical platform observability infrastructure as an `admin.dashboard_query_audit_completeness_violation` Cat C emission OR a successful no-drift no-op execution, depending on whether tenant operators are using the dashboards yet). **Failure to verify within 2h = deployment-incomplete signal → cutover rollback per §8.3.** This operational gate is the canonical close to R19 HIGH-1 (the residual scheduler-registration gap that SQL preflight cannot mechanically verify); the SQL-side N.0 + this Phase 11 step (c) sign-off + the platform observability infrastructure together close the practical gap without forking SI-023's 6-audit-event + 5-endpoint scope.
