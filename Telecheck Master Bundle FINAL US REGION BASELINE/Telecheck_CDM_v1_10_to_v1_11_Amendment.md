...
**Class N — Admin dashboard audit completeness tripwire** (P-042 R14 HIGH-1 closure 2026-05-22 — demoted from blocking Phase 10 preflight to non-blocking post-deploy operational tripwire; emits Cat C audit on detected drift but does NOT block cutover):

Rationale for demotion: v0.5-v0.13 class N compared `admin_dashboard_request_log_v` (APM-sourced) row counts vs `admin_dashboard_query_execution` (DB) row counts globally over 24h. This comparison was fragile under real deployment conditions: APM logs include failed/unauthorized requests (correctly NOT producing DB rows); APM tenant scope may differ from the cutover tenant scope; retention/filtering differ between APM and DB; multi-tenant cutover scenarios produce expected drift. A blocking preflight on this comparison would have produced noisy false-positives at cutover time, blocking ships for non-defect reasons. Since the SECDEF wrapper-only canonical read path makes the audit-row INSERT co-transactional with the dashboard view SELECT (per FLOOR-020 fail-closed), genuine drift can ONLY come from database-integrity defects (e.g., manual DELETE on `admin_dashboard_query_execution` outside canonical APIs) — operational monitoring with Cat C emission is sufficient; blocking preflight is over-engineered. Codex R14 HIGH-1 recommendation accepted.

Class N implementation (post-deploy operational monitor; per-(tenant, dashboard) drift emission; no tenant GUC dependency; P-042 R15 HIGH-1 closure 2026-05-22 — rewritten to be tenant-scoped end-to-end via GROUP BY on tenant_id + dashboard_name, eliminating the v0.15 `current_tenant_id_strict('monitor_*')` GUC dependency that would have raised before emission in post-deploy contexts):

**Precondition for the monitor:** `admin_dashboard_request_log_v` MUST carry per-row `tenant_id` + `dashboard_name` columns sourced from APM logs (the HTTP layer extracts the tenant from the JWT-verified claims + the dashboard surface from the endpoint path; both populated at request time + flowed into the APM logs). Without these columns, the per-tenant correlation is not possible, and class N MUST NOT claim audit-completeness coverage; in that case the monitor block below is skipped + a one-time deploy-time advisory is logged.
...
BEGIN
    -- Verify the operational monitor's input contract: admin_dashboard_request_log_v MUST carry
    -- tenant_id + dashboard_name columns. If not, skip the monitor entirely + log advisory.
    SELECT
    ...
    INTO v_log_view_has_tenant_id, v_log_view_has_dashboard_name;

    IF NOT v_log_view_has_tenant_id OR NOT v_log_view_has_dashboard_name THEN
        RAISE NOTICE 'si-023-admin-dashboard-audit-completeness-monitor-skipped: admin_dashboard_request_log_v lacks required tenant_id + dashboard_name columns; per-tenant correlation not possible; v1.1 hardening item to add HTTP-layer request_id propagation + APM tenant attribution';
        RETURN;
    END IF;
...
10. **Phase 10 — Deployment preflight gate (P-042 R8 MED-1 + R14 HIGH-1 closures 2026-05-22):** run §8.1 DO block — **blocking classes A-M + O + P + Q** (wrapper-owner DML/EXECUTE privilege verification per R7 HIGH-1 + R8 HIGH-1; any FAILED assertion BLOCKS cutover). **Class N moved to post-deploy operational monitor** (per R14 HIGH-1 closure 2026-05-22 — global APM-vs-DB count comparison is too coarse for blocking preflight; runs post-deploy as Cat C tripwire emission without blocking ships).
