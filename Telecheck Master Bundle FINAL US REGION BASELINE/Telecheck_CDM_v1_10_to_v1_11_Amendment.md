...
**Class N — Admin dashboard audit completeness tripwire (BLOCKING; P-042 R20 Path-1 revert 2026-05-22 — restored SI-023's literal blocking-preflight design):**

Per Evans's Path-1 ratifier decision 2026-05-22: class N runs synchronously as a blocking preflight at Phase 10 (matches SI-023 §8.1 class N "Any drift > 0 raises ... + blocks cutover"). The v0.14-v0.20 path that demoted N to an operational scheduled monitor + back to a split N.0/N.1 design + back to an inline-DO operational-residual closure was an over-engineering wrong turn that the ratifier reverted in one move. The blocking design implements the v0.16-style tightened comparison (per-(tenant, dashboard) FOR-loop + response_status 2xx filter) which addresses the R14 noise concern that originally triggered the demotion. Folds in the input-contract precondition (was N.0 in v0.17-v0.20) at the top so the DO block fails fast if `admin_dashboard_request_log_v` isn't correctly provisioned. No scheduler. No 13th role. No heartbeat. No ongoing monitor process. Cat C audit emission preserved per-drifting-pair (matches SI-023 §3 row 5 contract).
...
BEGIN
    ...
    -- tenant_id + dashboard_name + response_status + request_at columns. If missing,
    -- BLOCK cutover with explicit error (deploy must propagate JWT-verified tenant_id +
    -- endpoint dashboard_name + HTTP response_status + request timestamp from the APM/HTTP
    -- layer into this view before cutover).
    ...
    -- Per-(tenant, dashboard) drift comparison (R14-noise-resistant: tenant-attributed + 2xx-filtered).
    -- Tenant_id passed explicitly to emit_audit (no GUC dependency). One Cat C event emitted per
    -- drifting pair; total absolute drift > 0 BLOCKS cutover.
    FOR v_row IN
        WITH expected_per_tenant AS (
            SELECT log.tenant_id, log.dashboard_name, COUNT(*) AS expected_count
             ...
             WHERE log.request_at > now() - INTERVAL '24 hours'
               AND log.response_status BETWEEN 200 AND 299  -- successful wrapper-backed requests only
    ...
    IF v_drift_total > 0 THEN
        RAISE EXCEPTION 'si-023-admin-dashboard-audit-completeness-violation: total absolute drift=% across (tenant, dashboard) pairs (Cat C audit events emitted per-pair); BLOCKS cutover per FLOOR-020 fail-closed discipline',
            v_drift_total;
    END IF;
    -- At first cutover (greenfield deploy) the comparison is vacuous (zero rows = zero rows) so
    -- no false-positives from initial deploy; on subsequent re-cutover/redeploy the comparison
    -- catches database-integrity defects (manual DELETE on admin_dashboard_query_execution outside
    -- canonical APIs). The wrapper-only canonical read path makes audit-row INSERT co-transactional
    -- with view SELECT (FLOOR-020 fail-closed) — there is NO legitimate path where a successful
    -- 2xx HTTP response exists without a corresponding DB row.
END $$;
```
...
10. **Phase 10 — Deployment preflight gate (P-042 R20 Path-1 revert 2026-05-22 — class N reunified as blocking; N.0/N.1 split removed):** run §8.1 DO block — **blocking classes A-M + N + O + P + Q**. Class N (per the Path-1 revert) runs the full audit-completeness preflight inline: input-contract verification on `admin_dashboard_request_log_v` + per-(tenant, dashboard) drift comparison + total-drift > 0 BLOCKS cutover. Class Q (per R7 HIGH-1 + R8 HIGH-1) verifies wrapper-owner DML/EXECUTE privileges. Any FAILED assertion BLOCKS cutover.
