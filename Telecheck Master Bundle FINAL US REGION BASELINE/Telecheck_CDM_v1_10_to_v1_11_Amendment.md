...
**Class N Block 1 — detect + emit + record state (commits)**:
...
BEGIN
    ...
    -- Record sentinel row for Block 2 to consume. cutover_preflight_state is a canonical
    -- platform-floor table (established outside SI-023 scope) used by all slice preflights
    -- to pass state between blocking DO blocks across explicit COMMIT boundaries.
...
**Class N Block 2 — read sentinel + raise on drift (durable evidence already committed by Block 1)**:
...
BEGIN
    ...
    IF v_drift_total IS NULL THEN
        RAISE EXCEPTION 'si-023-admin-dashboard-audit-completeness-block2-missing-sentinel: class N Block 1 did not write its sentinel row; Block 1 may have failed before COMMIT — check earlier cutover output for Block 1 error';
    END IF;

    IF v_drift_total > 0 THEN
        RAISE EXCEPTION 'si-023-admin-dashboard-audit-completeness-violation: total absolute drift=% across % (tenant, dashboard) pairs (Cat C audit events emitted per-pair by Block 1 + durably committed before this raise); BLOCKS cutover per FLOOR-020 fail-closed discipline',
            v_drift_total, v_drift_pair_count;
    END IF;
