...
Class N implementation (operational monitor; runs post-deploy at the same cadence as other class-N tripwires across slices; emits Cat C audit but uses RAISE NOTICE not RAISE EXCEPTION):
...
BEGIN
    -- Operational tripwire (NOT a cutover gate). Compares last-24h counts as a coarse signal;
    -- correlation by tenant + dashboard + request_id is a v1.1 post-pilot hardening item if drift
    -- frequency in operations justifies tighter correlation. Failure modes that produce noisy
    -- positives (APM-filtered failed/unauthorized requests, multi-tenant scope mismatches, retention
    -- differences) are accepted as known-noise at v1.0; the tripwire's value is detecting database-
    -- integrity defects (manual DELETE on admin_dashboard_query_execution outside canonical APIs).
    SELECT COUNT(*) INTO v_expected_count
      FROM admin_dashboard_request_log_v
     WHERE request_at > now() - INTERVAL '24 hours'
       AND response_status BETWEEN 200 AND 299;  -- filter to successful wrapper-backed requests only
    ...
    IF v_drift <> 0 THEN
        PERFORM emit_audit_event_co_transactional(
            current_tenant_id_strict('monitor_admin_dashboard_audit_completeness'),
        ...
        RAISE NOTICE 'si-023-admin-dashboard-audit-completeness-drift: 24h drift=% (expected=%, actual=%); Cat C audit emitted; not blocking cutover',
            v_drift, v_expected_count, v_actual_count;
    END IF;
END $$;
```

**Post-pilot v1.1 hardening:** if observed drift frequency in production justifies tighter correlation, v1.1 will add per-request correlation between APM logs + DB audit rows (e.g., propagating a request_id from the HTTP layer through to `admin_dashboard_query_execution`) so class N can do exact correlation rather than count comparison. This is a v1.1 ratifier decision based on observed-drift profile.
