# Program Porting Checklist — Telecheck-US (Heros Health DBA) GLP-1 → Telecheck-Ghana (Heros Health Ghana DBA) GLP-1

**Scope:** Concrete worked example of what has to happen — by layer, by owner, by approval chain — to make the Telecheck-US (Heros Health DBA) existing US GLP-1 program available as a Telecheck-Ghana (Heros Health Ghana DBA) program under the current platform architecture (ProgramMarketPolicy + Pattern A form versioning + four-layer Forms Engine + CCR Runtime).
...
- **Inherit / Author / Activate** — whether this is starting-point material from the Telecheck-US (Heros Health DBA) version, new authoring work, or an activation step
- **Owner** — who signs off
...
|---|---|---|---|---|
| A.1 | Platform-level program record exists in catalog | 🟢 | Product Lead | Already exists; was created when Telecheck-US (Heros Health DBA) launched US GLP-1. No new record needed. |
...
Per **ADR-004 Pattern A**, the Telecheck-Ghana market gets its own `intake_form_version_id` (`frv_*`) for GLP-1. The Telecheck-US (Heros Health DBA) version is the authoring template. Every layer below produces work; the four-layer architecture controls who reviews what.
...
| B.1.4 | Testimonials and social proof | 🔴 | Content Author + Marketing Lead | Must be Ghana-sourced. Cannot reuse Telecheck-US (Heros Health DBA) testimonials (data protection + authenticity). |
...
- Adding a new tenant (e.g., a future Telecheck-Nigeria, a future US tenant beyond Telecheck-US / Heros Health DBA). That's tenant-onboarding work — RBAC scoping, tenant config, brand assets, legal entity, payment processor account.
...
**RPM Diabetes → Telecheck-US (Heros Health DBA).** Reverse direction. Telecheck-US starts from Telecheck-Ghana's intake template. L1 layer rewrites for US voice, regulatory disclaimers swap to FDA, pricing in USD via Stripe Billing, US clinician licensure verification, US formulary (insulin types, glucometer brands available in US). Mode 2 pathway and protocol library currently scoped for Ghana — would require US versions. **Note:** No active RPM ProgramMarketPolicy for the US currently exists, so this is also a tenant-strategy decision (does the Telecheck-US tenant operator add chronic care to its catalog?), not just a port.
...
- **Related artifacts:** Master Platform PRD v1.10 §10 (operating model), §10.5 (program catalog architecture — invokes this checklist), §11 (architecture), §11.7 (slice index); Forms Engine contract; CCR Runtime contract; Market Launch contract; Protocol Library Ghana v1.0; Guardrail Templates v1.0; ADR-004; ADR-005; ADR-024.
