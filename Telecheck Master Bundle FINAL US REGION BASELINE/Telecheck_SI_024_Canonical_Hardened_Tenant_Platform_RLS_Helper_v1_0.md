...
**PRODUCTION USE BLOCKED UNTIL SI-024.1 (Pass-2 B+ synthesis condition 2026-05-20):** SI-024 v1.0 authors the canonical schema (`break_glass_approval` table + RLS policies + helper functions + audit triggers + INSERT path) so Phase 1 implementation can scaffold the infrastructure. **However, production target-tenant break-glass operations MUST NOT be exercised before SI-024.1 ratification + integration** because v1.0 carries two acknowledged gaps that compound on the highest-risk access path: (a) dual-control approval-creation is self-attested + out-of-band (simplification #9; not two-phase workflow), and (b) per-invocation break-glass access has no canonical audit (simplification #8; deferred to SI-024.1's `begin_target_tenant_break_glass_session()` procedure). Combined, these mean a valid approval can be used repeatedly inside its window without per-access audit evidence proving each access. Phase 1 scaffolding is permitted; production use blocks on SI-024.1.
...
9. **OQ-NEW2 — Phase 4 cutover gating on SI-024.1 readiness (R1 CRITICAL-2 closure 2026-05-20; STRENGTHENED at Pass-2 B+ synthesis 2026-05-20).** Recommendation: Phase 4 cutover of SI-024 v1.0 (removing raw-GUC permissive policy; hardened helper becomes sole canonical enforcement) **MUST gate on SI-024.1 ratification + integration. This gate is non-negotiable for canonical-floor status.** RATIONALE: SI-024 v1.0 is canonical TRANSITIONAL implementation guidance for Phases 1-3 only. Phase 4, INVARIANTS I-036, and any durable canonical-floor language all gate strictly on SI-024.1. Complete SI-024 v1.0 Phases 1-3 (foundation + RESTRICTIVE coexistence + telemetry) WHILE SI-024.1 is being authored + ratified; cutover (Phase 4) ONLY when SI-024.1 lands with cryptographic binding integrated into the helper. Without this discipline, the corpus would canonize an acknowledged-incomplete trust anchor (a regression on canonical-floor discipline that future SI authors might cite as precedent).
