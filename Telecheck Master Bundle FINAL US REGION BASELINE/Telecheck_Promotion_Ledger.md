...
5. **NEW STEP 0 — I-032 Tenant-GUC equality guard** added to BOTH procedures (cross-PR OQ3 Option A): per the canonical I-032 Mode 1 / Mode 2 split (R3+R5 closures 2026-05-19), STEP 0 first checks `NULLIF(current_setting('app.tenant_id', true), '') IS NULL` and RAISEs SQLSTATE P0001 (Mode 1 — environmental failure routed to error-stream + P0 alert); on non-NULL/non-empty GUC, the procedure checks `p_tenant_id IS DISTINCT FROM` that normalized value and returns the `tenant_guc_mismatch` structured rejection (Mode 2 — application call site emits canonical Cat B `security.security_definer_tenant_guc_mismatch` audit at SI-018 P2 + P0 alert). Both modes use the same `NULLIF()` normalization so RESET/blank-string GUC also routes to Mode 1.
...
- I-032 Tenant-GUC equality guard on SECURITY DEFINER procedures with actor-tenant parameters (STEP 0 with Mode 1 NULL/blank-GUC RAISE → error-stream + P0 alert; Mode 2 mismatched-non-NULL-GUC structured-rejection with `tenant_guc_mismatch` code; Mode 2 application call site emits Cat B `security.security_definer_tenant_guc_mismatch` audit event at SI-018 P2 keyed on GUC-side `app.tenant_id`; ELEVATED severity; P0 ops alert on both modes; per `NULLIF(current_setting('app.tenant_id', true), '')` normalization so RESET/blank-string GUC also routes to Mode 1): **APPROVED**
