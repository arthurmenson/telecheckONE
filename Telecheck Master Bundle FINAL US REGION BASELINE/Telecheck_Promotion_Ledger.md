...
**Status:** **CANONICAL 2026-05-18** (transitioned from RATIFIED-IN-INTENT 2026-05-17 → CANONICAL 2026-05-18 via SC6 PR-A2/A3 canonical-content-port lockstep commit; SECOND PR-A2/A3-class landing of the Q2 2026 ratifier ceremony per the smallest-first triage discipline; canonical content landed: Contracts Pack AUDIT_EVENTS v5.3 → v5.4 with 3 new Cat B identity binding-lifecycle action IDs + Identity_Authentication_Spec v1.0 → v1.1 with new §3.6 "Server-side actor context (per SI-010)" + Artifact Registry v2.12 → v2.13). Evans's verbatim ratifier instruction at the Sub-Ceremony 6 chat-message Decision Brief 2026-05-17: *"ratify"* (defaulted per the brief to "all 8 ratifier sub-decisions as recommended"). Standing autonomous-work authorization per CLAUDE.md "Autonomous-work authorization (Evans standing directive, 2026-05-16+)" authorized the PR-A2/A3 commit + push + Codex-per-PR adversarial review + merge cycle on 2026-05-18.
...
6. **AUDIT_EVENTS canonical content (LANDED as v5.4 in this PR-A2/A3 lockstep commit 2026-05-18) — 3 net-new Cat B action IDs (Sub-decision 6 APPROVED, refined across Codex PR #10 rounds 1–5 closures 2026-05-18):** `identity.actor_context_bound`, `identity.session_liveness_check_failed`, `identity.actor_context_unbound_rejected`. All three are governance auth-proof events (visible to compliance review + audit chain integrity, no escalation). Cat B because they are auth-proof events that compliance review needs to reconstruct authentication state at any prior point (mirrors SI-002's `identity.session.issued` / `identity.otp.{issued, consumed}` Cat B classification post-P-014). **Per-event mandatory-minimum field set (event-specific by design; SUPERSEDES the prior "All 3 carry session_id + account_id + tenant_id + nonce_hash + bound_at" framing which contradicted the per-event detail schemas — closed Codex PR #10 R1 round-1 HIGH + R4 round-4 HIGH findings 2026-05-18):** (a) `identity.actor_context_bound` carries the full bound identity (`actor_account_id`, `account_tenant_id`, `role`, `admin_home_tenant_id`, `session_id`, `nonce_hash` SHA-256, `bound_at`, `expires_at`, `pg_backend_pid`, `txid`); (b) `identity.session_liveness_check_failed` carries `session_id` + best-effort `account_id` from JWT (nullable when JWT verify itself failed first) + `tenant_id_claimed` + `failure_reason` enum (`revoked | missing | expired`) + `checked_at` + `pg_backend_pid` + `txid` (last two captured server-side at liveness-check time per Codex R2 round-2 HIGH closure); (c) `identity.actor_context_unbound_rejected` carries `procedure_name` + `rejection_code` (`actor_context_unbound | request_nonce_unbound_or_expired`) + `pg_backend_pid` + `txid` + nullable `nonce_hash` (actor/account/session intentionally absent because by definition no bound context resolved). **Forensic correlation invariant:** `pg_backend_pid` + `txid` are the cross-event correlation key on all three; the full bound identity (when known) lives on the `actor_context_bound` row; absence of that row in the same `(pg_backend_pid, txid)` tuple IS the forensic signal. **Envelope + hash-chain rules (added v5.4 patch per Codex R4 HIGH closure):** identity-lifecycle events are platform-scope governance, NOT patient-scope; `target_patient_id = NULL`; `resource_type ∈ {'actor_context', 'session'}`; `resource_id` = session_id when present else `pg_backend_pid::txid` composite; dedicated `identity_lifecycle` hash-chain partition keyed by `tenant_id_claimed` else `_PLATFORM_` sentinel. **Rollback-independent commit path (Identity Spec §3.6 step 4):** all three events use the rollback-durable audit-writer commit path (application-side `auditWriterPool` for plugin-layer emissions; `dblink_exec` loopback for procedure-side `actor_context_unbound_rejected`) + durable `audit_backstop.log` fsync-based disk fallback if the primary path fails (Codex R3 round-3 HIGH + R5 round-5 HIGH closures). **Fail-closed semantics on audit-writer failure (Codex R4 round-4 MEDIUM + R5 round-5 HIGH closures):** if BOTH primary AND backstop writes fail, the platform raises 503 + critical ops alert; the rejection itself is suppressed at the API surface until auditably recorded; I-003 audit completeness holds across all failure modes.
7. **Seven mandatory regression tests gate SI-010 IMPL-readiness (Sub-decision 7 APPROVED + extended across Codex PR #10 rounds 3+4+5 closures; merge-blocking for the SI-010 implementation PR, not for this PR-A2/A3 canonical-content-port commit):** (a) **GRANT enforcement test** — assert `telecheck_app_role` has zero INSERT/UPDATE/DELETE/SELECT on `_session_actor_context` + zero EXECUTE on `bind_actor_context()`; (b) **caller-spoof test (adversarial; R4 regression)** — with `telecheck_app_role`, attempt direct INSERT / EXECUTE / GUC-fabrication, all MUST fail `permission denied` OR a dependent SECURITY DEFINER procedure MUST fail `actor_context_unbound`; (c) **pooled-connection bleed test (per SI-009 R6)** — request B on the same backend connection as request A reads B's context (not A's), validates UPSERT semantics + composite `(pg_backend_pid, txid)` discriminator; (d) **expired-context test** — bind, sleep past expiry, invoke procedure → `request_nonce_unbound_or_expired` rejection (validates defense-in-depth `expires_at > NOW()` predicate); (e) **migration-deploy test** — asserts post-migration state: table exists as permanent + composite PK + `bind_actor_context_role` exists + `telecheck_app_role` has zero privileges + only `bind_actor_context_role` has EXECUTE on `bind_actor_context()`; (f) **rollback-durable audit (Codex R3 round-3 closure)** — open a request tx, bind + emit `actor_context_bound`, ROLLBACK; assert audit row still exists in `audit_events`. Repeat for procedure-side via `dblink_exec`; (g) **fail-closed on audit-writer failure with durable backstop (Codex R4 round-4 + R5 round-5 closures)** — simulate `auditWriterPool` failure AND `dblink_exec` failure across all 3 identity-lifecycle events in three sub-scenarios per event: (i) primary healthy; (ii) primary fails + backstop healthy (`audit_backstop.log` fsync persists row; idempotent ingest drains into `audit_events`); (iii) both primary and backstop fail (503 + critical ops alert; no PHI mutation; no orphan binding row; I-003 audit completeness holds across all sub-scenarios — no path exists where a rejection occurs without a durable audit trace).
...
> - SC6 (P-023 SI-010 Session Actor Context DB Binding) landed SECOND in the canonical-content-port wave 2026-05-18: Registry v2.12 → v2.13 + AUDIT_EVENTS v5.3 → v5.4 (3 new Cat B identity binding-lifecycle action IDs) + Identity_Authentication_Spec v1.0 → v1.1 (new §3.6 "Server-side actor context (per SI-010)" closing Phase 2 F-3). P-023 is now CANONICAL.
