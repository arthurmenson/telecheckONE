...
**Type:** Content-change promotion — **ratification-intent recorded in PR-A1⁗″ commit; physical content lands together across MULTIPLE future PR-A2/A3-class lockstep commits** per the lockstep invariant (one commit per sub-SI canonical-content-port + per-dependency-SI). Destination Registry version follows the ordering-dependent rule per top-of-Ledger interpretation rule (now updated to cover 7 sub-ceremonies / 10 entries; SC7 is unique in contributing Registry +1 minor with NO contract-file bumps).
...
**Status:** **RATIFIED IN INTENT 2026-05-18** (workstream lead chat-message sign-off; sub-ceremony 7 of the Q2 2026 ratifier ceremony per `arthurmenson/telecheck-app:docs/Ratifier-Ceremony-Agenda-Q2-2026.md`). Evans's verbatim ratifier instruction at the Sub-Ceremony 7 chat-message exchange: *"ratify"* given as forward authorization at the SC6 close-out exchange (defaulted per the brief to "all 9 ratifier sub-decisions as recommended"). **CANONICAL** after future PR-A2/A3-class lockstep commits land each sub-SI's canonical content (SI-011a + SI-011b can land independently; SI-011c requires SI-015 MarketingCopy ratification landing first; SI-011d requires SI-016 ai_workflow_handler_registry ratification landing first; the kill-switch implementation layers 1+2 already merged via PR #155 on the implementation track, with layers 3+4 to land separately as autonomous-track work).
...
> - **General rule:** each sub-ceremony's PR-A2/A3-class commit applies +1 minor Registry bump regardless of ordering; consolidation of N sub-ceremonies into a single commit applies +N minor Registry bumps. The specific version numbers cited in individual ratification-intent entries assume the default 1→2→3→4→5→6→7 sub-ceremony landing order; the actual destination version is determined by consulting the current Registry version at the time of each future PR-A2/A3-class landing.
> - **Sub-ceremonies 4 + 5 + 6 + 7 CDM-bump exemption:** unlike sub-ceremonies 1/2/3 (which all expand CDM), sub-ceremony 4 (SI-002 AUDIT_EVENTS placeholder ratification) AND sub-ceremony 5 (SI-003 DOMAIN_EVENTS amend-in-place + SI-004 Async-Consult AUDIT_EVENTS amendment) AND sub-ceremony 6 (SI-010 Session Actor Context DB Binding — Identity-slice procedure-only with 3 net-new Cat B AUDIT_EVENTS IDs and no entity additions) AND sub-ceremony 7 (SI-011 Forms-Intake publish-time governance gates UMBRELLA — scoping + design pattern + dependency-SI schedule only; per-sub-SI canonical content deferred to future SCs) are all CDM-exempt. Sub-ceremony 4 bumps Registry +1 minor + AUDIT_EVENTS +1 minor only. Sub-ceremony 5 bumps Registry +1 minor + AUDIT_EVENTS +1 minor (SI-004 contribution) + DOMAIN_EVENTS amend-in-place (no version bump per established pattern from P-011/P-013/P-018/P-019/P-021/P-015) — net: Registry + AUDIT_EVENTS bumps only. Sub-ceremony 6 bumps Registry +1 minor + AUDIT_EVENTS +1 minor (SI-010 binding-lifecycle Cat B contribution) + no DOMAIN_EVENTS contribution + no CDM contribution — net: Registry + AUDIT_EVENTS bumps only, identical scope shape to sub-ceremony 4. **Sub-ceremony 7 bumps Registry +1 minor ONLY** (no AUDIT_EVENTS, no DOMAIN_EVENTS, no CDM contribution — **FIRST SC across the Q2 2026 ratifier ceremony to be exempt from ALL THREE contract-file bumps**; SI-011 umbrella ratifies scoping + design pattern only, per-sub-SI canonical content lands at future successor SCs SI-011.1a/b/c/d). Total maximum CDM versions across all 7 sub-ceremonies: 3 bumps (sub-ceremony 1 + 2 + 3 each contribute one; sub-ceremonies 4+5+6+7 contribute none). Total maximum AUDIT_EVENTS bumps: 6 (sub-ceremonies 1-6 each contribute one; sub-ceremony 7 contributes NONE — umbrella ratification only). Total DOMAIN_EVENTS version bumps: 0 (all additive enum extensions across all 7 sub-ceremonies; SC6 + SC7 both contribute zero DOMAIN_EVENTS amendment).
