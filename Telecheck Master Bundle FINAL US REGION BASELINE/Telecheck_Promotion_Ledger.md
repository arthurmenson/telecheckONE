...
### Entry P-016 — 2026-05-17 — SI-004 ratification-intent: Async-Consult AUDIT_EVENTS canonical action IDs (sub-ceremony 5 of Q2 2026 ratifier ceremony; 10 net-new IDs across 11 PRD §13 events incl. State-Machines-required additions; ratify-all-11-up-front scope)
...
- AUDIT_EVENTS Contracts Pack **+1 minor bump** will apply in PR-A3⁗ (10 net-new action IDs from SI-004: 7 Cat C lifecycle events `consult.{initiated, intake_submitted, abandoned, expired, patient_notification_sent, follow_up_message, completed}` + 3 Cat B governance events `consult.{case_claimed_by_clinician, additional_data_requested, escalation_to_sync}`). Plus 2 reused Cat A IDs (`consult.clinician_decision_recorded` from P-021 SI-005 + `medication_request.approved.v1` from P-011 SI-001) — NO duplicate addition; consultation cross-references existing canonical IDs.
...
1. **AUDIT_EVENTS canonical content — 10 net-new action IDs for SI-004 Async-Consult scope:**
   - **Cat C lifecycle (7 events):** `consult.{initiated, intake_submitted, abandoned, expired, patient_notification_sent, follow_up_message, completed}`
...
3. **AUDIT_EVENTS — Categorization** (per Evans's ratified decision #7): 7 Cat C + 3 Cat B + 2 reused Cat A. Net-new from SI-004 = 10 IDs; the 2 Cat A events (`consult.clinician_decision_recorded` + `medication_request.approved.v1`) are REUSED from already-ratified P-021 SI-005 + P-011 SI-001 scopes — no duplicate definition.
...
>   - All-8-entry consolidation (P-012/P-013/P-018/P-019/P-021/P-014/P-015/P-016 in a single combined PR-A2/A3 covering all 5 sub-ceremonies): v2.11 → v2.16 directly (CDM v1.3 → v1.6 — only 3 CDM bumps since sub-ceremonies 4+5 are AUDIT_EVENTS-only; AUDIT_EVENTS v5.3 → v5.8 — 5 bumps since each sub-ceremony contributes one AUDIT_EVENTS amendment; DOMAIN_EVENTS unchanged at v5.2 throughout — all additive enum extensions per established pattern).
...
> - **Sub-ceremonies 4 + 5 CDM-bump exemption:** unlike sub-ceremonies 1/2/3 (which all expand CDM), sub-ceremony 4 (SI-002 AUDIT_EVENTS placeholder ratification) AND sub-ceremony 5 (SI-003 DOMAIN_EVENTS amend-in-place + SI-004 Async-Consult AUDIT_EVENTS amendment) are both CDM-exempt. Sub-ceremony 4 bumps Registry +1 minor + AUDIT_EVENTS +1 minor only. Sub-ceremony 5 bumps Registry +1 minor + AUDIT_EVENTS +1 minor (SI-004 contribution) + DOMAIN_EVENTS amend-in-place (no version bump per established pattern from P-011/P-013/P-018/P-019/P-021/P-015) — net: Registry + AUDIT_EVENTS bumps only. Total maximum CDM versions across all 5 sub-ceremonies: 3 bumps (sub-ceremony 1 + 2 + 3 each contribute one; sub-ceremonies 4+5 contribute none). Total maximum AUDIT_EVENTS bumps: 5 (each sub-ceremony contributes one). Total DOMAIN_EVENTS version bumps: 0 (all additive enum extensions across all sub-ceremonies).
