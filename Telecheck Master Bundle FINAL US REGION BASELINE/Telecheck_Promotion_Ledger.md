...
**Authority:** Evans (workstream lead + ratifier-quorum lead per CLAUDE.md). SI-023 v1.0 promotion via auto-proceed convergence (Codex R17 APPROVE / ship-it + Claude READY-TO-MERGE; 17 rounds, ~34 findings closed inline including 33 HIGH + 1 MED, 0 hard-floor item 6 escalations). The deep concurrency thread (R8 parent-template FOR UPDATE → R11 shared serialization → R13 state-continuity trigger → R14 advisory lock → R15 READ COMMITTED isolation → R16 trigger merge) converged at R17 with the unified lifecycle invariants trigger taking one per-review advisory lock + READ COMMITTED isolation precondition covering all 3 invariants (future-date bound + backdate reject + state-continuity) under a single lock-held read window.

**Type:** Content-change promotion + Registry v2.27 → v2.28 per operating rule 4.

**Cycle metrics:** 17 Codex adversarial-review rounds. Findings: ~34 closed inline (~33 HIGH + 1 MED). 0 hard-floor item 6 escalations. 0 ERR escalations. Concurrency-invariant tightening drove the long-tail (rounds R7-R17 progressively refined the one-active-review-per-template enforcement model from row locks → trigger checks → partial UNIQUE INDEX → trigger inheritance → advisory lock → READ COMMITTED isolation → merged unified trigger). Final design: 2-layer enforcement (LAYER 1 parent-template FOR UPDATE primary; LAYER 2 BEFORE INSERT trigger defense-in-depth via LATERAL-derived check) + unified lifecycle-invariants trigger combining append-only, monotonic-ordering, and state-continuity under single advisory lock + READ COMMITTED precondition. SECDEF wrapper authorization: 3-layer defense (LAYER A EXECUTE grant + LAYER B JWT-principal-to-role via tenant_account_membership schema-backed join per P-038 R6 dissolution pattern + LAYER C tenant scope verification).

**Files promoted:**

| File | Path | Version bump |
|---|---|---|
| Telecheck_SI_023_Admin_Backend_Basics_v1_0.md | bundle root | new SI spec at v1.0 RATIFIED (was v0.17 DRAFT POST-R16; promoted at R17 APPROVE) |
| Telecheck_Artifact_Registry_v2_10.md | bundle root | v2.27 → v2.28 (header + §8 changelog) |
| Telecheck_Active_Document_Index_v1_0.md | bundle root | §1 add SI-023 entry as canonical for admin-backend basics |
...
### Entry P-036a — 2026-05-22 — Post-P-036 cross-slice consistency fix (Mode 1 CDM follow-on amendment seed-scope correction): Evans's chat-message ratification "A" 2026-05-22 RATIFIED extending `jwt_migration_entity_status` seed scope from 5 → 6 entries to include `ai_mode1_conversation_state` view per the post-R7 plain-view design (the historical 5-entry seed scope was correct for the pre-R7 security_invoker view design; R7 changed the view to a plain view with explicit `current_tenant_id_strict()` predicate in its body, which made it a tenant-binding surface that should be tracked). **Trigger:** the 2026-05-22 post-P-042 cross-slice consistency audit Codex pass (Finding 2 HIGH) identified Mode 1 as the only pilot slice with a tenant-scoped view OMITTED from `jwt_migration_entity_status` seed scope (SI-020 P-038 = 9 entries / 2 views; SI-022 P-040 = 5 entries / 2 views; SI-023 P-042 = 7 entries / 3 views; Mode 1 P-036 had been 5 entries / 0 views). **Decision shape:** small ratifier mini-review via chat-message ("A" 2026-05-22) rather than full ERR escalation because the question was bounded (add the view to seed scope yes/no) and the cross-slice canonical pattern was the deciding factor. **Files changed:** `Telecheck_CDM_v1_7_to_v1_8_Amendment.md` §7 — seed-count "5" → "6" + INSERT statement extended with `ai_mode1_conversation_state` row + cdm_owner sequencing guidance updated explaining the pre-R7-vs-post-R7 rationale + stale-prose carve-out note (Finding 3 from same audit pass deferred to post-P-042 hygiene cycle). NO change to AUDIT_EVENTS / OpenAPI / State Machines / RBAC / Artifact Registry version (this is a within-P-036-scope clarification fix, not a separate cycle requiring its own version bump). **Cumulative cycle statistics through P-036a:** 17th successive Q2 2026 auto-proceed ratification (P-026 → … → P-042 → P-036a); 1 CORRECT hard-floor item 6 invocation (P-033 R1); 2 DISSOLVED (P-038 R6 + P-040 R15); 3 ratifier-decision-driven closures (P-042 R2 Option A + P-042 R20 Path-1 + **P-036a Option A — THIS ENTRY**). **Findings 1 + 3 from the same audit pass** (SI-019 SECDEF prose drift; Mode 1 stale security_invoker references) are deferred to the post-P-042 hygiene cycle along with the broader physical-merge work (P-009 v1.10.1-style consolidation of post-P-029 delta files into renamed bundle files).

---

### Entry P-042 — 2026-05-22 — CDM v1.10 → v1.11 + AUDIT_EVENTS v5.12 → v5.13 + OpenAPI v0.5 → v0.6 + State Machines v1.4 → v1.5 + RBAC v1.4 → v1.5 (SI-023 Admin Backend Basics follow-on amendment) RATIFIED via auto-proceed + 2 hard-floor-class ratifier decisions (R2 Option A 2026-05-22 + R20 Path-1 revert 2026-05-22, both via Evans's chat-message decisions). +4 new CDM entities (`admin_dashboard_query_execution` audit trail + `forms_template_admin_review` + `forms_template_admin_review_lifecycle_transition` append-only Option A per I-035 + `admin_template_decision_idempotency_key` canonical IDEMPOTENCY per P-027) + 4 OPTIONAL canonical views (`admin_crisis_operational_health_v` + `admin_consult_queue_health_v` + `admin_mode1_volume_health_v` per SI-023 Sub-decision 2 + `forms_template_admin_review_pending_v` reviewer-scoped pending-only surface per P-042 R7 HIGH-2 closure mechanically enforcing SI-023 §7's "pending reviews" prose qualifier) + 6 SECURITY DEFINER procedures (1 raw lifecycle writer + 3 dashboard read-wrappers + 2 template wrappers; 3-layer authorization defense LAYER A+B+C per P-038 R6 dissolution pattern; SECURITY INVOKER lifecycle-invariants trigger with unified advisory lock + READ COMMITTED isolation precondition covering future-date / backdate / state-continuity invariants under single lock-held window) + 6 new audit events (4 Cat A + 2 Cat C under `admin.*` namespace) + 5 OpenAPI endpoints under `/v1/admin/*` (3 dashboard GETs + submit/decision POSTs; submit endpoint NOT idempotent at API layer per R3 HIGH-2; lost-response recovery via `platform_operator_breakglass` runbook per R5 HIGH-1 closure reusing canonical P-040 break-glass framework) + 1 new state machine `forms_template_admin_review_lifecycle` (5 states / 5 transition triples; DERIVED from append-only per I-035) + 12 net-new RBAC roles (2 application + 3 dashboard-wrapper-owner + 2 template-wrapper-owner + 1 raw-writer-owner + 4 view-owner) + 7 jwt_migration_entity_status seed entries (4 tables + 3 dashboard views — pending view excluded; reviewer-scoped not multi-tenant-migrated) + class N two-DO-block + COMMIT + run_id scoping + Phase 10 autocommit contract (R21-R24 cascade) + class Q wrapper-owner DML/EXECUTE privilege preflight (R7 HIGH-1 + R8 HIGH-1 with UPDATE-for-FOR-UPDATE) + class H per-entity allowlist for admin review entities (R13 HIGH-1 reconciliation); Artifact Registry v2.28 → v2.29; **NINTH instance of established post-P-029 SI-spec-first promotion pattern** (P-029, P-032, P-034, P-036, P-038, P-040; P-035 was SI-only, P-037 paired with P-038); CDM follow-on for SI-023 Admin Backend Basics (P-041); **completes the 5/5 pilot-required Ghana revenue anchor slice CDM landings: Med-Interaction (P-033+P-034), Async-Consult (P-037+P-038), Mode 1 AI Service (P-035+P-036), Crisis Response (P-039+P-040), Admin Backend Basics (P-041+P-042)**. Post-P-042 the pilot scope is FULLY spec-ratified.

**Cycle metrics:** 25 Codex adversarial-review rounds; ~28 findings closed inline (~26 within-scope HIGH/MED + 2 ratifier-decision hard-floor-class closures: R2 Option A "We go with A" 2026-05-22 preserving SI-023 ratified wrapper-body ordering + R20 Path-1 "1" 2026-05-22 reverting v0.14-v0.20's operational-monitor wrong-turn to SI-023's literal blocking-preflight design for class N). Class N convergence trajectory was the cycle's biggest pivot: R14 noisy-comparison demotion → R15 GUC dependency → R16 silent observability regression → R17 inline-DO not deployable → R18 BYPASSRLS + scheduler EXECUTE → R19 scheduler-registration verification gap → R20 observability-proxy watches wrong signal → Path-1 revert eliminates entire scheduler dependency → R21 rollback-sensitive Cat C emissions → R22 stale sentinel hazard → R23 transactional-wrapping hazard → R24 wrong probe execution context → R25 APPROVE. Validates the dual-recommendation discipline: Codex flagged R20's residual gap; Claude surfaced three-path recommendation to ratifier; ratifier picked Path-1 which collapsed 7 rounds of patches into a single revert + made remaining R21-R24 closures mechanical refinements of the SI-023-faithful design.

**Evans's standing autonomous-work authorization (CLAUDE.md):** when Codex APPROVE + Claude READY agree, auto-proceed. P-042 follows the established post-P-029 mechanical-consolidation cadence; SI-023 v1.0 RATIFIED content (P-041) lands in named bundle file sections via this amendment cycle.

**Pilot-required slices status after P-042 — COMPLETE:**

| # | Slice | Status |
|---|---|---|
| 1 | Medication Interaction Service (SI-019) | **FULLY RATIFIED** (P-033 SI + P-034 CDM follow-on) |
| 2 | Async Consult Slice (SI-020) | **FULLY RATIFIED** (P-037 SI + P-038 CDM follow-on) |
| 3 | Mode 1 Conversational Floor (SI-021) | **FULLY RATIFIED** (P-035 SI + P-036 CDM follow-on) |
| 4 | Crisis Response Slice (SI-022) | **FULLY RATIFIED** (P-039 SI + P-040 CDM follow-on) |
| 5 | Admin Backend Basics (SI-023) | **FULLY RATIFIED** (P-041 SI + P-042 CDM follow-on — THIS ENTRY) |

**🎉 5 of 5 pilot-required Ghana revenue anchor slices NOW FULLY SPEC-RATIFIED (SI + CDM both landed).** Telecheck-app pilot implementation gate has been OPEN since P-041 (2026-05-22); P-042 completes the spec-corpus landing of the pilot scope. Remaining pilot work is implementation in the telecheck-app code repo, not specification authoring. Master Completion Plan v1.0 §A.5 pilot-viable scope item 5 of 5 spec-corpus deliverable RATIFIED.

---

### Entry P-040 — 2026-05-21 — CDM v1.9 → v1.10 + AUDIT_EVENTS v5.11 → v5.12 + OpenAPI v0.4 → v0.5 + State Machines v1.3 → v1.4 + RBAC v1.3 → v1.4 (SI-022 Crisis Response follow-on amendment) RATIFIED via auto-proceed + Evans's chat-message Option B ratification at R15 hard-floor item 6 escalation; +3 new CDM entities (`crisis_event` + `crisis_event_lifecycle_transition` + `crisis_sweep_execution`) + additive column extensions to 3 P-027 §4.66-4.68 entities (`notification_crisis_dispatch_ledger` + `notification_crisis_provider_attempt` + `notification_crisis_escalation_obligation`) + 2 OPTIONAL canonical views (`crisis_event_current_state_v` staff tenant-wide + `crisis_event_patient_summary_v` patient/delegate self-scoped — R1 HIGH-2 split per P-038 R5 data-minimization pattern) + 6 SECURITY DEFINER procedures (1 raw + 5 wrappers owned by 5 owner roles; anti-bypass EXECUTE-grant discipline) + 12 new audit events (7 Cat A + 0 Cat B + 5 Cat C; SI-022 v1.0 tally drift `8 Cat A + 4 Cat C` reconciled in §4 per-row labels) + 6 new OpenAPI endpoints under `/v1/crisis/*` (endpoint 6 unauthenticated-emergency posture deliberate I-019 carve-out) + 1 new state machine `crisis_event_lifecycle` (11 CHECK-enforced transition triples; DERIVED from append-only `crisis_event_lifecycle_transition` per I-035) + 15 new RBAC roles (7 application incl R1 HIGH-2 split crisis_event_staff_reader + crisis_event_patient_reader + 5 wrapper-owner + 1 raw-writer-owner + 2 view-owner) + 5 jwt_migration_entity_status seed entries; Artifact Registry v2.26 → v2.27; **NINTH instance of established post-P-029 SI-spec-first promotion pattern** (P-029, P-032, P-034, P-035, P-036, P-037, P-038, P-039, P-040); pilot-viable scope item 4 of 5 FULLY RATIFIED (SI + CDM both landed): Crisis Response now joins Med-Interaction (P-033+P-034), Mode 1 AI Service (P-035+P-036), Async-Consult (P-037+P-038). Remaining pilot-required slice: Admin Backend basics SI.

**Evans's standing autonomous-work authorization (CLAUDE.md "Autonomous-work authorization" + dual-recommendation + two-pass + auto-proceed):** when Codex APPROVE + Claude READY agree, auto-proceed. P-040 follows the established post-P-029 mechanical-consolidation cadence; SI-022 v1.0 RATIFIED content (P-039) lands in named bundle file sections via this amendment cycle.

**Authority:** Evans (workstream lead + ratifier-quorum lead per CLAUDE.md). P-040 promotion via auto-proceed convergence (Codex R19 APPROVE / ship-it + Claude READY-TO-MERGE; 19 rounds, ~25 findings closed inline including 1 hard-floor item 6 DISSOLUTION at R15 via Evans's chat-message Option B ratification 2026-05-21 — second instance of hard-floor item 6 dissolution in Q2 2026 after P-038 R6 + P-040 R15; precedent matches P-038 R6 dissolution pattern — Codex Option B recommendation adopted; closure path REMOVED the architectural-judgment trigger condition rather than escalating a net-new platform-floor primitive). Three-way ratifier consult on R15 hard-floor: Claude=B, Codex Pass-1=B-prime, Codex Pass-2=B-prime; Evans chose plain B (lighter governance; residual SECDEF wrapper indirection accepted as bounded at v1.10 because 6 §3 canonical wrappers all return VOID/uuid + EXECUTE-restricted).

**Type:** Content-change promotion + Registry v2.26 → v2.27 per operating rule 4.

**Cycle metrics:** 19 Codex adversarial-review rounds (long-tail driven by privilege-bypass closure thread — class A through M + G.2 + G.3 + H + I + J + K + L preflight assertions covering grant matrix + role-membership recursive closure + predefined-role bypass + SECDEF dependency + dynamic-SQL/dblink + derived-relation + view-owner + view-definition integrity + CTAS event trigger). Findings: ~25 closed inline (24 HIGH + 1 MED). 1 hard-floor item 6 DISSOLVED at R15 (NOT escalated to net-new invariant; closure path REMOVED architectural-judgment trigger via Evans's Option B ratification + two-pass Codex consult discipline). 0 ERR escalations beyond the R15 consult. The privilege-bypass closure thread was the deepest in any P-040+ amendment cycle to date; R1 HIGH-2 split crisis_event_reader into staff/patient pair (P-038 R5 pattern); R6/R7 added predefined-role + SECDEF-dependency + derived-relation + view-owner classes; R8-R12 progressively tightened CTAS event-trigger semantics until R14 categorical block triggered R15 escalation. Post-R15 dissolution: R16 capability-branch parity + R17 SECURITY DEFINER → SECURITY INVOKER + R18 prose-sync.

**Files promoted:**

| File | Path | Version bump |
|---|---|---|
| Telecheck_CDM_v1_9_to_v1_10_Amendment.md | bundle root | new amendment doc at v1.0 RATIFIED |
| (consolidation target) Telecheck_Canonical_Data_Model_v1_2.md | bundle root | engineering reference; v1.10 content lives in amendment doc per delta-artifact convention until next baseline bump |
| (consolidation target) Telecheck_Contracts_Pack_v5_00_AUDIT_EVENTS.md | bundle root | header bump to v5.12 with doc-control entry pointing to delta artifact |
| (consolidation target) Telecheck_OpenAPI_v0_2.md | bundle root | header bump to v0.5 with doc-control entry pointing to delta artifact |
| (consolidation target) Telecheck_State_Machines_v1_1.md | bundle root | header bump to v1.4 with doc-control entry pointing to delta artifact |
| (consolidation target) Telecheck_RBAC_Permissions_Matrix_v1_2.md | bundle root | header bump to v1.4 with doc-control entry pointing to delta artifact |
| Telecheck_Artifact_Registry_v2_10.md | bundle root | v2.26 → v2.27 (header + §8 changelog) |
| Telecheck_Promotion_Ledger.md (this entry) | bundle root | append-only |
...
**4 of 5 pilot-required Ghana revenue anchor slices NOW FULLY RATIFIED (SI + CDM both landed).** Remaining: Admin Backend basics SI (final pilot prerequisite). Once Admin Backend SI ratified, the telecheck-app pilot implementation gate opens fully.
...
**Authority:** Evans (workstream lead + ratifier-quorum lead per CLAUDE.md). Mode 1 Handler Spec v0.4 promotion via auto-proceed convergence (Codex R8 APPROVE / ship-it + Claude READY-TO-MERGE).

**Type:** Content-change promotion + Registry v2.21 → v2.22 per operating rule 4.

**Prerequisite:** P-031 (SI-024.1 v0.8 JWT-binding canonical pattern) + P-032 (CDM v1.6 session_jwt_admission + jwt_migration_entity_status) + P-027 (Contracts Pack v5.3 + I-035) + P-034 (cleanest worked example of SI-024.1 pattern across multiple trust-axes — applied to Mode 1 at R5-R7).

**Cycle convergence trajectory (8 rounds total; R1-R4 from Sprint 9 2026-05-19 + R5-R8 fresh-review post-P-034 2026-05-21):**

| Round | HIGH | MED | Class of defect closed |
|---|---|---|---|
| R1 | 3 | 3 | Detector-before-LLM runtime guard + Cat C drop classification + ACK durability semantics + no-Mode-2-side-effects predicate scope + history-window replay safety + missing test coverage |
| R2 | 2 | 2 | §12 cross-SI summary stale claim + multi-state lifecycle vs append-only contradiction + missing schema column + missing Mode 2 worker-queue denial test |
| R3 | 1 | 0 | last_turn_at mutable field violated INSERT-only model |
| R4 | 1 | 1 | I-027 append-only enforcement missing CREATE TRIGGER DDL + archival event tenant integrity FK gap |
| **R5** | **2** | **1** | **SI-017 raw-GUC tenant binding inside SECURITY DEFINER bodies SUPERSEDED by SI-024.1 JWT-binding (post-P-031); missing entity-name RLS policies + jwt_migration_entity_status seed scope; Contracts Pack v5.2/v5.1 → v5.3 + AUDIT_EVENTS v5.5 → v5.9 (fresh-review post-P-034 alignment)** |
| **R6** | **0** | **1** | **§10 OQ scope text stale (2 entities to CDM v1.2; CCR v5.2) → aligned to 5 entities to CDM v1.7→v1.8 + CCR v5.3** |
| **R7** | **1** | **0** | **Derived view ai_mode1_conversation_state used bare CREATE VIEW assumed to inherit RLS — replaced with security_invoker + dedicated non-BYPASSRLS owner-role + deployment-prerequisite** |
| **R8** | **0** | **0** | **ship-it APPROVE — "no material findings"** |

**Total:** 10 HIGH + 6 MED closed inline across 7 closure rounds. Zero hard-floor item 6 escalations.

**FIRST instance of 'fresh-review-post-major-cycles' pattern.** The spec sat at RATIFIER-READY status through 4 major canonical landings; fresh Codex review at promotion (R5) surfaced staleness against the post-P-034 baseline. R5-R7 closures applied the same SI-024.1 JWT-binding canonical pattern that was the cleanest worked example at P-034 to the Mode 1 spec. This validates a new operational pattern: specs that sit between authoring and promotion across major-canonical-landing cycles must undergo a fresh-review-post-major-cycles pass before ratification, NOT rely on the pre-major-landing closure status.

**Artifact landed:** `Telecheck Master Bundle FINAL US REGION BASELINE/Telecheck_AI_Service_Mode_1_Handler_Spec_v0_1.md` (path renamed from `Telecheck_v1_10_PRD_Update/AI-Service-Mode-1-Handler-Spec-v0-1-DRAFT.md` via `git mv` during promotion; ~691 lines after R5-R7 closures; merge commit `7b2e3ec`; spec branch `spec/ai-service-mode-1-handler-spec-ratification-2026-05-21` at convergence commit `f0b22b2`).

**Canonical landings:**

- **AI Service Mode 1 Handler Spec v0.4:** canonical HTTP handler contract for the conversational-assistant AI workload class per ADR-002 + ADR-029 WORKLOAD_TAXONOMY. Defines POST `/ai/mode-1/conversation` request/response shapes + error model + idempotency semantics + FLOOR-020 audit emission contract (Cat A turn lifecycle + Cat B governance + Cat C operational drop aggregates) + I-019 crisis-detection integration (always-on detector pre-LLM-call; multi-region ACK channel) + 3-layer Mode 1 / Mode 2 boundary enforcement (DB write enforcement + outbound HTTP allow-list + static analyzer + LLM tool-use limited to read-only) + 5-table append-only conversation-state durability + LLM provider abstraction (CCR-driven) + per-tenant rate limiting + latency budget (p99 ≤2.5s) + 22 merge-blocking integration tests + 3 static-analyzer rules (TLC-AI-001, TLC-AI-002, TLC-AI-003).
- **5 Mode 1 CDM entities (proposed for follow-on amendment cycle; queued as next deliverable):** `ai_mode1_conversation` (durable identity) + `ai_mode1_conversation_archival_event` (append-only archival event log) + `ai_mode1_conversation_turn_admission` + `ai_mode1_conversation_turn_detector_result` + `ai_mode1_conversation_turn_result`. All strict append-only per I-035 + I-027 + `enforce_append_only()` trigger.
- **1 derived view** `ai_mode1_conversation_state` with `security_invoker=true` + non-BYPASSRLS owner-role discipline.
- **5-entity `jwt_migration_entity_status` seed scope** required at follow-on amendment cycle (per the established post-P-034 R6 closure pattern).
- **Tenant binding:** SI-024.1 v0.8 JWT-verified-claims canonical pattern; raw GUC fallback ONLY under audited gates.

**Companion entries queued (this commit cluster):**

- Artifact Registry v2.21 → v2.22 bump (next commit)
- AI_Service_Rollout status-doc Addendum 63 + progress.json revision bump

**Cross-references:** P-031 (SI-024.1 v0.8 JWT-binding canonical trust anchor); P-032 (CDM v1.6 session_jwt_admission + jwt_migration_entity_status entities); P-033 (SI-019 Med-Interaction Option A append-only-only pattern); P-034 (cleanest worked example of SI-024.1 JWT-binding pattern across multiple trust-axes — same pattern applied to Mode 1 spec in R5-R7); P-027 (Contracts Pack v5.3 + I-035); ADR-002 (two-mode AI architecture); ADR-029 (AI workload taxonomy); I-019 (crisis-detection-always-on platform floor); Master Completion Plan v1.0 Track 2 (AI Service Mode 1) + pilot-viable scope item 3.

**Master Completion Plan progression:** AI Service Mode 1 Track 2 ratified at P-035. Implementation in `telecheck-app` code repo UNBLOCKED post-merge per "spec ratification leads implementation by ≥1 sprint" rule. **3 of 5 pilot-required Ghana revenue anchor slices now ratified** (Med-Interaction at P-033 + P-034; AI Service Mode 1 at P-035). Remaining: Async-Consult clinician-decision loop completion SI, Crisis Response slice SI, Admin Backend basics SI.

**Follow-on amendment cycle queued:** per the established post-P-029 SI-spec-first promotion pattern (precedent at P-029 → P-032 → P-034), the SI-019-style follow-on amendment cycle for Mode 1 will land the 5 new CDM entities + 1 derived view + entity-name RLS policies + jwt_migration_entity_status seed scope into canonical bundle files. Scope: CDM v1.7 → v1.8 + AUDIT_EVENTS v5.9 → v5.10 + DOMAIN_EVENTS additive + CCR_RUNTIME v5.3 → v5.4 (if `ai_provider` key not yet registered) + co-bumped Registry v2.22 → v2.23.
