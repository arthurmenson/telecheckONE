...
8. **AUDIT_EVENTS canonical content (destination version ordering-dependent per top-of-Ledger interpretation rule; default 4th-landing = v5.7) — 7 net-new Cat C action IDs:** `sync_session.{scheduled, started, completed, no_show, cancelled, escalation_target_swapped, escalation_target_swap_failed}`.
...
12. **AUDIT_EVENTS canonical content (destination version ordering-dependent per top-of-Ledger interpretation rule; default 4th-landing = v5.7) — 7 net-new Cat A action IDs:** `ai_workflow_execution.{started, completed, failed, cancelled, current_pointer_swapped, swap_rejected, race_lost}`.
