...
**Version:** 0.7 DRAFT (R5 closure applied 2026-05-21)
**Status:** **DRAFT / POST-R5.** Codex R5 returned 1 HIGH only (no MED — HIGH count converging) — within Option A scope, closed inline: HIGH-1 (activation could race against override evidence row INSERT because raw Sub-decision 8.5 writer treated any non-terminal latest state as continuation-valid; override wrapper inserts override row at Step 5 BEFORE appending the transition at Step 6, so a window exists where override evidence row is present but latest transition still shows `emitted` — concurrent activation wrapper checking only the transition log would see `emitted → active` is valid and advance the signal, leaving an overridden signal active in audit and contradicting the override rationale) → two-layer fix: (1) override wrapper acquires the `(tenant_id, signal_id)` advisory lock at new STEP 4.5 BEFORE the override row INSERT at Step 5 (serializes override evidence INSERT against all other lifecycle writers); Sub-decision 8.5 STEP 1 re-acquires the same lock no-op since re-entrant within same transaction; (2) Sub-decision 8.5 raw writer adds new STEP 3.5 that, when `transition_reason='activation'`, checks `EXISTS (SELECT 1 FROM interaction_signal_override WHERE signal_id = X)` under the lock and rejects with new code `activation_blocked_by_override_evidence` if any override row exists. Awaiting R6.
