...
7. `jwt_migration_entity_status` seed population for **6 entity names** (R6 HIGH-1 closure 2026-05-21): 4 new CDM v1.7 RLS-bearing entities (interaction_engine_evaluation, interaction_signal, interaction_signal_override, interaction_signal_lifecycle_transition) + 2 MV trust-anchor read-path surfaces (interaction_signal_current_state_mv + interaction_signal_current_state_v) per the R5 closure's `current_tenant_id_strict()` call sites. All seeded with `phase_4_cutover_eligible=FALSE` AND `raw_guc_fallback_audited=TRUE` defaults (Phase B fail-closed-with-audit posture).
...
CREATE TABLE interaction_signal_override (
    ...
    -- R4 HIGH-2 closure 2026-05-21: plaintext override_rationale column REMOVED — earlier draft
    -- had override_rationale TEXT NOT NULL alongside the KMS envelope below, which would have
    -- persisted clinician rationale in cleartext (PHI/clinical-rationale exposure in ordinary
    -- table reads, backups, replication, incident scope). Per canonical SI-005 KMS envelope
    -- pattern: plaintext lives only as procedure input + application-boundary buffer; persisted
    -- form is ONLY the 8-column envelope. Decrypt at read-time via the canonical decrypt helper
    -- + tenant-scoped KMS key + same caller-identity discipline.
...
-- R5 HIGH-1 closure 2026-05-21: use the canonical current_tenant_id_strict() trust anchor instead of
-- raw current_setting('app.tenant_id'). Per the SI-024.1 v0.8 ratified pattern (P-031),
-- current_tenant_id_strict() reads from the JWT admission record (session_jwt_admission, CDM v1.6
-- §4.NEW1) cryptographically bound to the current backend, OR falls back to GUC ONLY IF the entity's
-- phase_4_cutover_eligible=FALSE per jwt_migration_entity_status. Filtering the MV with raw GUC
-- alone would let any caller that can influence app.tenant_id query another tenant's signal state
-- (same defect class as R3 actor_role spoofing).
...
WHERE tenant_id = current_tenant_id_strict('interaction_signal_current_state_mv');
...
RETURNS TABLE(
    ...
    -- R5 HIGH-1 closure 2026-05-21: use the canonical current_tenant_id_strict() trust anchor
    -- (JWT-verified per SI-024.1 v0.8 admission-binding invariant; falls back to GUC ONLY when
    -- entity's phase_4_cutover_eligible=FALSE AND raw_guc_fallback_audited=TRUE, with audit
    -- emission). Raw current_setting('app.tenant_id') would let caller spoof tenant context.
    SELECT
        mv.signal_id,
    ...
    WHERE mv.tenant_id = public.current_tenant_id_strict('interaction_signal_current_state_mv')
...
--
-- Reference body pattern for STEP 4 in the override wrapper (R3-canonical + R4 HIGH-1 closure
-- 2026-05-21: fallback gate is fail-closed — REQUIRES BOTH `phase_4_cutover_eligible=FALSE` AND
-- `raw_guc_fallback_audited=TRUE`; audit emission happens BEFORE the role membership check; if
-- either condition fails, raise a structured rejection):
--
--   DECLARE
...
--           -- Phase B fallback path: GUC-supplied actor role, FAIL-CLOSED on either gate condition.
--           IF public.is_jwt_required_for_entity('interaction_signal_override') THEN
--               RAISE EXCEPTION 'jwt_required_no_admission_record'
--                   USING ERRCODE = 'insufficient_privilege';
--           END IF;
--           -- R4 HIGH-1 closure: audit gate is HARD, not optional.
--           IF NOT public.is_raw_guc_fallback_audited_for_entity('interaction_signal_override') THEN
--               RAISE EXCEPTION 'raw_guc_fallback_not_audited'
--                   USING ERRCODE = 'insufficient_privilege';
--           END IF;
...
--                   USING ERRCODE = 'insufficient_privilege';
--           END IF;
--           -- Emit fallback audit BEFORE role membership check so the audit evidence is
--           -- recorded regardless of whether the membership check subsequently rejects.
--           PERFORM public.emit_raw_guc_fallback_audit(
...
**New rejection codes:**
- **R2 HIGH-1:** `actor_role_missing` (Mode 1 — GUC absent or blank); `unauthorized_role` (Mode 2 — actor role lacks required membership).
- **R3 HIGH-1:** `jwt_required_no_admission_record` (no JWT-verified claims AND entity requires JWT per `is_jwt_required_for_entity()`).
- **R4 HIGH-1:** `raw_guc_fallback_not_audited` (Phase B fallback attempted on an entity where `is_raw_guc_fallback_audited_for_entity()` returned FALSE — fallback is fail-closed when audit is disabled).

All rejection codes use `insufficient_privilege` SQLSTATE for consistent caller handling.

**Updated 12-rejection-code total for override wrapper (R4 HIGH-1 closure):** `tenant_guc_missing`, `tenant_guc_mismatch`, `actor_role_missing`, `jwt_required_no_admission_record`, `raw_guc_fallback_not_audited`, `idempotency_replay_outcome_mismatch`, `signal_not_active`, `signal_state_terminal`, `medication_not_on_list`, `unauthorized_role`, `unique_violation`, `advisory_lock_unavailable`.
...
| SI-024.1 v0.8 RATIFIED `jwt_migration_entity_status` seed step (P-031 OQ8) | All 6 entity names seeded at amendment-apply time (4 RLS-bearing tables + 2 MV trust-anchor surfaces per R6 HIGH-1 closure) | All 4 new CDM v1.7 RLS-bearing entities + the 2 MV/view trust-anchor surfaces consumed by `current_tenant_id_strict()` calls in the R5 closure |
...
1. **OQ1 — `jwt_migration_entity_status` initial seed scope for v1.7 entities.** Recommendation: seed **6 entity names** at amendment-apply time with `phase_4_cutover_eligible=FALSE` AND `raw_guc_fallback_audited=TRUE` defaults (Phase B fail-closed-with-audit posture):
   - **4 RLS-bearing tables:** `interaction_engine_evaluation`, `interaction_signal`, `interaction_signal_override`, `interaction_signal_lifecycle_transition`
   - **2 MV trust-anchor read-path surfaces (R6 HIGH-1 closure 2026-05-21):** `interaction_signal_current_state_mv` (consumed by both the SECURITY BARRIER view and the SECURITY DEFINER access function as the entity-name argument to `current_tenant_id_strict()`); `interaction_signal_current_state_v` (in case implementers wire the view to a different entity-key for separate observability rollout).
   The R5 closure depends on these MV/view entity names being seeded — without them, `current_tenant_id_strict()` would either fail-closed (breaking clinician dashboard / pharmacy portal / patient hot-path reads) or fall back permissively (breaking the R5 tenant-isolation guarantee). Cdm_owner flips per-entity `phase_4_cutover_eligible=TRUE` as policies migrate to JWT-required posture (typically the 4 tables first, MV surfaces last so all writers are JWT-cutover before downstream readers).
...
- **R5 HIGH-1 closed:** MV access surfaces (SECURITY BARRIER view `interaction_signal_current_state_v` + SECURITY DEFINER access function `get_interaction_signal_current_state`) used raw `current_setting('app.tenant_id', false)::tenant_id_t` for tenant filtering. The MV itself is non-RLS-enforced (PostgreSQL materialized views don't natively enforce RLS per R2 HIGH-2 closure rationale), so the view + function were the trust boundary — but they trusted a mutable, caller-set GUC. Any caller that can influence `app.tenant_id` could query another tenant's signal lifecycle state from the MV. Same trust-class defect as R3 actor_role spoofing. Fix: replaced both surfaces with `current_tenant_id_strict('interaction_signal_current_state_mv')` per SI-024.1 v0.8 ratified canonical pattern — the helper reads from the JWT admission record (cryptographically bound to current backend via admission-binding invariant) or, when entity's `phase_4_cutover_eligible=FALSE` AND `raw_guc_fallback_audited=TRUE`, falls back to GUC with audit emission. No more raw-GUC trust paths on MV reads. Cross-reference: SI-024.1 v0.8 §Sub-decision 1 + CDM v1.6 §4.NEW5 jwt_migration_entity_status fallback gate.

**v0.5 DRAFT 2026-05-21 — R4 closures applied (2 HIGH):**
- **R4 HIGH-1 closed:** Phase B fallback gate fail-open. R3 closure pattern checked `NOT is_jwt_required_for_entity()` before accepting `app.actor_role`, but only conditionally emitted `emit_raw_guc_fallback_audit()` and continued whether or not audit was enabled. Meant the audit gate was optional rather than a hard requirement — re-opened R3 trust-boundary defect for any entity not yet cut over to JWT. Fix: gate is now fail-closed on BOTH conditions — `phase_4_cutover_eligible=FALSE` AND `raw_guc_fallback_audited=TRUE`. If either fails, raise structured rejection (`jwt_required_no_admission_record` or `raw_guc_fallback_not_audited`). Audit emission happens BEFORE the role membership check so audit evidence is recorded regardless of whether membership check subsequently rejects.
- **R4 HIGH-2 closed:** `interaction_signal_override.override_rationale TEXT NOT NULL` column would persist clinician rationale in cleartext alongside the KMS envelope — defeats KMS protection pattern; PHI/clinical-rationale exposure in ordinary table reads, backups, replication, and incident scope. Fix: REMOVED the plaintext column from the table DDL. Plaintext lives only as procedure input + application-boundary buffer per canonical SI-005 KMS envelope pattern. Persisted form is ONLY the 8-column KMS envelope; decrypt at read-time via canonical decrypt helper + tenant-scoped KMS key + same caller-identity discipline (R3 closure JWT-verified-claims pattern). Override wrapper total rejection codes now 12 (was 10).
