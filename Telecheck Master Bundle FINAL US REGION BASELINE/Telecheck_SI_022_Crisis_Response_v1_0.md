...
STEP 2 (synchronous; FLOOR-020 fail-closed; SINGLE transaction):
...
-- Sub-decision 6 sweep contract (R5 HIGH-1 closure):
...
--   WHERE now() > undeliverable_deadline AND escalation_key IS NULL
--   FOR UPDATE SKIP LOCKED;
-- Per row: (a) INSERT crisis_event_lifecycle_transition (escalated, reason='no_acknowledgement_timeout');
--          (b) UPDATE escalation_obligation SET escalation_key = gen_random_uuid() (one-way mutable
--              via BEFORE UPDATE trigger; idempotent re-sweep is a no-op once set);
--          (c) UPDATE escalation_obligation SET escalation_tier = next_tier(current_tier, severity)
--              one-way mutable; next_tier function returns 'clinical_on_call' from 'care_team',
--              'regulatory' from 'clinical_on_call' IFF severity = life_threatening, else terminal;
--          (d) call compute_crisis_recipient_mapping(crisis_event_id, severity) to derive new-tier
--              recipients + INSERT provider_attempt rows under the SMS/email/in_app_push
--              dispatch_ledger rows already established at STEP 4a;
--          (e) emit crisis.no_acknowledgement_escalation Cat A audit row.
...
Authored on `spec/SI-022-crisis-response-slice-2026-05-21` branch off main at `fab0615` (post-P-038 merge). v0.1 commit `e7a7ebb`. v0.2 commit `c2b9e15`. v0.3 commit `f4001d2`. v0.4 commit `6ec0cc9`. v0.5 commit `18ef338`. v0.6 commit pending push for R6 verification.
