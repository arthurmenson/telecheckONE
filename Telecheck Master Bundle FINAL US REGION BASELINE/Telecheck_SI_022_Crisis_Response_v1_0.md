...
|---|---|---|
| `record_crisis_acknowledgement_claim()` | UPDATE obligation SET undeliverable_deadline = now() + INTERVAL_for_severity_response_window(severity); escalation_tier left unchanged (continues to track current tier for next-tier advance IF response also times out); escalation_key reset to NULL | Acknowledgement is NOT safety-terminal — the patient is still in crisis until responded/resolved. A post-acknowledgement response-window timer enforces clinician response within bounded time. |
| `record_crisis_response()` | UPDATE obligation SET undeliverable_deadline = now() + INTERVAL_for_severity_resolution_window(severity); escalation_tier left unchanged; escalation_key reset to NULL | Response is NOT safety-terminal — clinician has taken action but resolution outcome is still pending. Post-response resolution-window timer enforces resolution within bounded time. |
...
3. **Recipient fan-out:** call `compute_crisis_recipient_mapping(crisis_event_id, severity, target_tier)` to derive recipients + INSERT provider_attempt rows under the dispatch_ledger rows from STEP 4a. **`target_tier` is explicitly defined to handle both pre-exhaustion and final-tier exhaustion/recheck paths (R16 HIGH-1 closure 2026-05-21):**

    ```sql
    v_target_tier := COALESCE(next_tier(escalation_tier, severity), escalation_tier);
    ...
    -- terminal-tier recipients (regulatory for life-threatening; clinical_on_call for
    -- non-imminent/imminent) on the recheck cadence until record_crisis_resolution() runs.
    ```
...
5. **Tier ADVANCE + RESET (release the per-tier-cycle lock + compute next deadline; R13 HIGH-1 closure 2026-05-21: do NOT terminalize on final-tier exhaustion — only `record_crisis_resolution()` may terminalize via escalation_tier=NULL):** UPDATE escalation_obligation SET:
    - `escalation_tier`: if next_tier(escalation_tier, severity) IS NOT NULL, advance to next tier; **else KEEP CURRENT TIER (final tier reached but unresolved; row remains sweep-visible)**. The R13 HIGH-1 closure splits "tier exhausted" from "resolved-terminal": tier exhaustion preserves escalation_tier at its terminal value (e.g., 'regulatory' for life-threatening, 'clinical_on_call' for imminent/non-imminent); resolution alone sets escalation_tier=NULL via the resolve wrapper.
    ...
    - `undeliverable_deadline = now() + INTERVAL_for_severity_and_tier(severity, next_tier(escalation_tier, severity)) WHEN next_tier IS NOT NULL ELSE now() + INTERVAL_final_tier_recheck_window(severity)` — final-tier rows get a separate recheck window so they continue to fire periodically until resolved (canonical: non-imminent 30min / imminent 10min / life-threatening 5min recheck cycle; matches resolution-window magnitudes from Sub-decision 5 but used for ongoing alert pressure, not initial resolution timeout)
    - `final_tier_exhausted_at`: NEW column added at this SI's CDM follow-on amendment; set to now() the first time escalation_tier reaches a tier where next_tier(escalation_tier, severity) IS NULL AND final_tier_exhausted_at IS NULL; left unchanged on subsequent re-sweeps. The first transition into final-tier-exhausted state emits an additional `crisis.final_tier_reached` Cat A audit (NEW; AUDIT_EVENTS amendment adds this to the 7 in §3, bringing total to 8) so operators are alerted on first exhaustion; subsequent re-sweeps emit the standard `crisis.no_acknowledgement_escalation` Cat A but with a payload field `is_final_tier_recheck=true`.
...
**Decision:** **17 merge-blocking integration tests** (R18 MED-1 closure 2026-05-21: "17" counts numbered tests 1-15 + #17 contention benchmark; test 16 is expanded into 8 sub-tests 16a-16h ALL of which are merge-blocking — sub-tests do NOT count toward the 17 total but ALL must pass for merge gate per CI gate language below; R10 added 16a-d; R11 added 16e-f; R13 added 16g; R16 added 16h) + 3 static-analyzer rules (mirrors P-035 Mode 1 + P-037 SI-020 pattern):

**CI gate language (R18 MED-1 closure):** the merge gate requires ALL of tests 1-15 + 16a + 16b + 16c + 16d + 16e + 16f + 16g + 16h + 17 to PASS. Total 24 individual assertions (15 base + 8 sub-tests + 1 benchmark). An implementation claiming compliance MUST execute and pass every individually-listed sub-test; CI script enumerates them all explicitly.

**Integration tests (17 base entries, with 16 expanded into 16a-16h sub-cluster covering R10+R11+R13+R16 closures):**
...
4. **OQ4 — Tenant configuration of fan-out channels.** Tenants configure WHICH channels exist (sms/email/in-app providers) but NOT whether fan-out happens. Ratifier confirms the canonical tenant_config keys: `crisis.fanout_channels[]` (allow-list per channel) + `crisis.clinical_on_call_channel` (which configured channel for clinical-on-call) + **(R18 HIGH-1 closure 2026-05-21)** `crisis.operator_escalation_channel` (channel-class for regulatory-tier operator on-call) + `crisis.operator_escalation_recipient` (recipient address for operator on-call).

**R19 HIGH-1 closure 2026-05-21 (enforcement contract):** I-019 platform-floor forbids ANY of the following keys from being empty/null on a tenant where `regulatory_reporting=true` (life-threatening crises can reach the regulatory tier on this tenant): `crisis.fanout_channels[]` (must be non-empty array), `crisis.clinical_on_call_channel` (must be non-null channel-class), `crisis.operator_escalation_channel` (must be non-null AND MUST be present in `crisis.fanout_channels[]` so the corresponding dispatch_ledger row is established at STEP 4a), `crisis.operator_escalation_recipient` (must be non-null and address-format-valid). For tenants with `regulatory_reporting=false`, the operator_escalation keys MAY be null (life-threatening crises will still escalate through care_team → clinical_on_call but will not reach the regulatory tier; the canonical next_tier() function returns NULL at clinical_on_call when severity='life_threatening' AND regulatory_reporting=false, treating clinical_on_call as the de-facto final tier).

**Enforcement points:**
1. **Deployment preflight DO block** (§10) — fail-closed assertion that REQUIRED keys above are non-null on tenants where regulatory_reporting=true (R19 HIGH-1 closure adds the 2 R18 keys + the cross-reference to fanout_channels[]).
2. **Static-analyzer rule TLC-CRISIS-003** (§7) — extended scope: now rejects tenant config rows where `regulatory_reporting=true` AND any of the 4 above keys is null/empty.
3. **Wrapper-procedure runtime validation** — `record_crisis_initiation()` (Phase A implementation) MUST assert these keys at session-bind time and fail-closed if missing, emitting `crisis.dispatch_attempt_failed` Cat C.
