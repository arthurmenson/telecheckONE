...
-- compute_crisis_recipient_mapping is a STABLE function that joins (R24 HIGH-1 closure 2026-05-21
-- adds operator_escalation_* source keys to make the regulatory target_tier branch normative + non-empty):
...
--                                       clinical_on_call_address) + emergency_contact tuple if consent grant
...
**Decision:** **17 merge-blocking integration tests** (R18+R21+R22+R25 MED closures 2026-05-21: "17" counts numbered tests 1-15 + #17 contention benchmark; test 16 expanded into 11 sub-tests 16a-16k ALL merge-blocking; R10 added 16a-d; R11 added 16e-f; R13 added 16g; R16 added 16h; R20 added 16i; R22 added 16j; R25 added 16k initial-fan-out-target_tier) + 3 static-analyzer rules:

**CI gate language (R18+R21+R22+R25 MED):** merge gate requires ALL of tests 1-15 + 16a + 16b + 16c + 16d + 16e + 16f + 16g + 16h + 16i + 16j + 16k + 17 to PASS. Total **27 individual assertions** (15 base + 11 sub-tests + 1 benchmark).

**Integration tests (17 base entries, 16 expanded into 16a-16k sub-cluster):**
...
1. **Deployment preflight DO block** (§10) — fail-closed assertion in TWO parts:
...
2. **Static-analyzer rule TLC-CRISIS-003** (§7) — extended to validate BOTH:
   - rejects ANY tenant config row where `fanout_channels[] IS NULL OR cardinality(fanout_channels[]) = 0 OR clinical_on_call_channel IS NULL OR NOT (clinical_on_call_channel = ANY(fanout_channels[]))` (every-tenant rule)
   - rejects tenant config rows where `regulatory_reporting=true AND (operator_escalation_channel IS NULL OR operator_escalation_recipient IS NULL OR NOT (operator_escalation_channel = ANY(fanout_channels[])))` (conditional rule)
