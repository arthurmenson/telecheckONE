...
Sweep work items are scheduled via a durable `crisis_sweep_execution` table (NEW at this SI's P-040 CDM follow-on): each scheduled sweep allocates a `sweep_execution_id UUID` and inserts a row with `(tenant_id, crisis_event_id, scheduled_at, claimed_by_worker_id NULL, completed_at NULL)`. The scheduler/worker MUST claim a row before executing (UPDATE SET claimed_by_worker_id = ... WHERE claimed_by_worker_id IS NULL — atomic claim). The atomic STEP 2-5 sweep transaction:
