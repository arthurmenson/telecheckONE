...
Additionally, STEP 2 of the sweep transaction includes an atomic obligation-eligibility revalidation BEFORE incrementing sweep_cycle_counter:
```sql
-- INSIDE STEP 2-5 atomic tx, BEFORE the sweep_cycle_counter increment:
SELECT 1 FROM notification_crisis_escalation_obligation obligation
JOIN LATERAL (
    SELECT to_state AS current_state
    FROM crisis_event_lifecycle_transition lt
    WHERE lt.tenant_id = obligation.tenant_id AND lt.crisis_event_id = obligation.crisis_event_id
    ORDER BY lt.transition_at DESC, lt.id DESC LIMIT 1
) AS lifecycle ON TRUE
WHERE obligation.tenant_id = v_tenant_id AND obligation.crisis_event_id = v_crisis_event_id
...
-- IF zero rows: sweep is stale (another execution already advanced obligation, or lifecycle changed,
-- or deadline was reset by acknowledge/respond/resolve). Raise sweep_stale_eligibility_lost; ROLLBACK
-- entire STEP 2-5 transaction; emit `crisis.sweep_stale_eligibility_dropped` Cat C audit + mark the
-- execution row's completed_at=now() with sweep_cycle_id_committed=NULL (terminal mark indicating
-- "completed-as-stale-no-op"); no fan-out, no audit emit, no tier-advance.
```
This makes "the obligation must still be in the same generation we scheduled for" a database invariant. If acknowledge/respond/resolve runs between scheduling and execution (resets deadline/escalation_key OR changes lifecycle OR advances sweep_cycle_counter), the sweep execution is correctly marked stale + dropped.
