...
- 12 new AUDIT_EVENTS action IDs (8 Cat A + 0 Cat B + 4 Cat C; R13 added crisis.final_tier_reached Cat A; R1 moved no_acknowledgement_escalation Cat B → Cat A; R43 + R45 MED-1 added crisis.sweep_replay_after_commit_ack_loss Cat C + crisis.sweep_claim_recovery_with_committed_cycle Cat C) under `crisis.*` namespace EXTENDING the P-025 `crisis.escalation_destination_resolved` ratified Cat B scope
...
-- (ii) atomic 6-step (STEP A→F) per-row closure with STEP B four-way reason-mapping CASE (R12 closure;
--   R62 HIGH-1 renamed from STEP 1), STEP C explicit target_tier = COALESCE(next_tier(escalation_tier, severity), escalation_tier)
--   (R16 closure; R62 HIGH-1 renamed from STEP 3) so final-tier rechecks produce non-empty recipient fan-out, and STEP E final-tier (R62 HIGH-1 renamed from STEP 5)
...
-- sweep_cycle_counter UPDATE-RETURNING in Sub-decision 6 STEP A (R62 HIGH-1 renamed from STEP 2; R39/R40/R41 closure unified
...
--
-- R25 HIGH-1 + R26 HIGH-1 + R64 HIGH-1 closure 2026-05-21: STEP 4b uses a DISTINCT initial-dispatch
-- function compute_crisis_initial_recipient_mapping(crisis_event_id, severity, regulatory_reporting)
-- which returns ALL Sub-decision 3 logical recipients eligible AT DETECTION TIME per severity/consent/
...
--
-- **R64 HIGH-1 closure 2026-05-21:** compute_crisis_initial_recipient_mapping reads the SAME tenant
-- config source list as compute_crisis_recipient_mapping (see below) — including the new
-- crisis.clinical_on_call_recipient + crisis.clinical_on_call_principal_id keys for the
-- clinical_on_call branch + crisis.operator_escalation_principal_id for the regulatory branch.
-- The clinical_on_call tuple at initial dispatch is (clinical_on_call_channel, 'clinical_on_call',
-- crisis.clinical_on_call_recipient, crisis.clinical_on_call_principal_id) — symmetric with the
-- sweep tuple. Implementer MUST NOT derive clinical_on_call_address from any other source (no
-- tenant_account_membership lookup for clinical_on_call; the principal_id is the canonical binding).
--
-- The OTHER function compute_crisis_recipient_mapping(crisis_event_id, severity, target_tier) is
-- ONLY called by Sub-decision 6 sweep STEP C (R62 HIGH-1 renamed from STEP 3) to derive recipients for a NEW target_tier when the
...
--   compute_crisis_recipient_mapping(..., target_tier) → Sub-decision 6 STEP C sweep (R62 HIGH-1 renamed from STEP 3; tier-advance
--     additions; called per sweep cycle)
-- compute_crisis_recipient_mapping is a STABLE function that joins (R24 HIGH-1 + R33 HIGH-1 + R64 HIGH-1
-- closure 2026-05-21 — added emergency_contact_channel + clinical_on_call_recipient + clinical_on_call_principal_id
-- to the source list for tenant-configurable emergency-contact fan-out + clinical_on_call recipient/principal
-- binding at sweep time):
...
--   tenant.crisis.clinical_on_call_recipient (R64 HIGH-1 closure 2026-05-21: address for clinical_on_call dispatch;
--     R63 HIGH-2: required non-null on EVERY tenant; preflight Part A + TLC-CRISIS-003 every-tenant rule + runtime
--     enforce non-null + address-format-valid for the configured clinical_on_call_channel)
--   tenant.crisis.clinical_on_call_principal_id (R64 HIGH-1 closure 2026-05-21: principal_id binding for the
--     clinical_on_call recipient; R63 HIGH-2: required non-null + principal_exists on EVERY tenant; populated into
--     provider_attempt.recipient_principal_id at fan-out for the R36/R37 wrapper-authorization contract)
...
--   tenant.crisis.operator_escalation_principal_id (principal_id binding for the operator_escalation
--     recipient — R37 HIGH-1 source; required non-null on regulatory_reporting=true tenants)
...
-- to insert as provider_attempt rows for THIS crisis_event (R37 HIGH-1 + R64 HIGH-1 closure 2026-05-21:
-- function output extended with recipient_principal_id derived from tenant_account_membership.principal_id /
-- consent_grant.delegate_principal_id / tenant.crisis.operator_escalation_principal_id / NEW
-- tenant.crisis.clinical_on_call_principal_id; required non-null for care_team, clinical_on_call,
-- operator_escalation; nullable for unauthenticated emergency_contact where principal identity is not knowable).
-- Branches on target_tier:
...
--                                       crisis.clinical_on_call_recipient, crisis.clinical_on_call_principal_id)
--                                       (R64 HIGH-1 closure 2026-05-21: recipient_address sourced from
--                                       crisis.clinical_on_call_recipient — NOT a derived clinical_on_call_address
--                                       value; recipient_principal_id sourced from crisis.clinical_on_call_principal_id
--                                       — NOT tenant_account_membership lookup; both keys are R63 HIGH-2 canonical
--                                       OQ4 additions; preflight Part A + TLC-CRISIS-003 + runtime guarantee
--                                       non-null on EVERY tenant)
--                                       + emergency_contact tuple (R66 HIGH-1 closure 2026-05-21: emergency_contact
--                                       projection is the FULL 4-tuple symmetric with all other recipient_roles in
--                                       the mapping; the 4th column recipient_principal_id is sourced from
--                                       consent_grant.delegate_principal_id when the consent grant authenticates
--                                       a specific delegate principal, OR explicitly NULL::uuid for an unauthenticated
--                                       emergency-contact address (the only recipient_role allowed to have NULL
--                                       recipient_principal_id per the provider_attempt CHECK constraint):
--                                       (emergency_contact_channel, 'emergency_contact', emergency_contact_address,
--                                        COALESCE(consent_grant.delegate_principal_id, NULL::uuid))
...
--                                       (R66 HIGH-1 closure 2026-05-21: implementer MUST emit the 4-column projection
--                                       even when delegate_principal_id is NULL — a UNION of branches across recipient_roles
--                                       requires uniform arity; tests assert emergency_contact rows from both mapping
--                                       functions expose recipient_principal_id as a nullable column)
--   target_tier = 'regulatory' → returns exactly 1 tuple (R65 HIGH-1 closure 2026-05-21: full 4-tuple
--                                       including recipient_principal_id; symmetric with the
--                                       clinical_on_call branch closure at R64 HIGH-1):
--                                       (operator_escalation_channel, 'operator_escalation',
--                                        crisis.operator_escalation_recipient,
--                                        crisis.operator_escalation_principal_id)
--                                       — recipient_address sourced from crisis.operator_escalation_recipient;
--                                       recipient_principal_id sourced from crisis.operator_escalation_principal_id
--                                       (NOT a tenant_account_membership lookup; NOT derived from any other source).
--                                       Both required non-null on regulatory_reporting=true tenants per
--                                       Part B preflight + TLC-CRISIS-003 + runtime validation (R65 HIGH-1
--                                       extended Part B to add operator_escalation_principal_id non-null +
--                                       principal_exists() rule symmetric to R63 HIGH-2's every-tenant
--                                       clinical_on_call_principal_id rule).
...
WHERE now() > obligation.undeliverable_deadline
  AND obligation.escalation_key IS NULL       -- legacy P-027 §4.68 column; ALWAYS NULL in canonical SI-022 (R55-R57 closure: NOT a claim lock; sweep concurrency is via R53 row lock + crisis_sweep_execution generation/fencing + sweep_cycle_counter)
...
The sweep LATERAL JOIN predicate now reads `current_state IN ('detected', 'escalated', 'acknowledged', 'responded')` — i.e., everything except `resolved`. The STEP B (R62 HIGH-1 renamed from STEP 1) transition reason switches by current_state:
...
For each eligible row, the job runs the following atomic per-row transaction. **Per R60 HIGH-2 closure 2026-05-21, ownership/eligibility MUST be proven before any other mutation: the R53 guarded UPDATE (formerly STEP 2 in earlier drafts) is now the FIRST mutating statement after the fencing-token guard. Lifecycle transition INSERT is renumbered to run AFTER the R53 guarded UPDATE has returned ROW_COUNT=1 (proving the sweep is the authoritative winner for this generation):**

**STEP A (formerly STEP 2 / R53 guarded UPDATE; now FIRST mutating statement):** R53 atomic eligibility-revalidation + sweep_cycle_counter increment UPDATE (described in the SQL block above). If ROW_COUNT=0: raise sweep_stale_eligibility_lost + ROLLBACK + emit crisis.sweep_stale_eligibility_dropped Cat C + mark execution completed-as-stale-no-op + EXIT (no lifecycle transition, no audit, no fan-out). If ROW_COUNT=1: v_sweep_cycle_id captured; current_state of obligation now derived from latest crisis_event_lifecycle_transition row (looked up inside the locked tx so it cannot change underneath); proceed.

**STEP B (formerly STEP 1; now renumbered to run AFTER R53 success):** Lifecycle transition INSERT — crisis_event_lifecycle_transition. The from_state is current_state read AFTER STEP A's lock acquisition. The to_state is `escalated`. **The transition reason is a four-way mapping by from_state (R8 HIGH-1 + R11 HIGH-1 + R12 HIGH-2 closure 2026-05-21; matches the 4 valid escalated-to triples in the 11-triple §6 enumeration):**
    ...
    ```
**STEP A (RESTATED — folded from prior numbered "2. Tier CLAIM + counter increment"; R53 HIGH-1 + R54 HIGH-1 + R55 HIGH-1 + R62 HIGH-1 closure 2026-05-21: the R53 single guarded UPDATE described above is the SOLE eligibility-revalidation-and-increment primitive in STEP A; the historical separate "Tier CLAIM" UPDATE setting escalation_key=gen_random_uuid() is REMOVED).** The R53 single guarded UPDATE (already shown as the SQL block at the start of STEP A) atomically (a) revalidates ALL eligibility predicates (deadline + escalation_key IS NULL + escalation_tier IS NOT NULL + lifecycle current_state + sweep_cycle_counter+1 = scheduled_for_obligation_generation) AND (b) increments sweep_cycle_counter via RETURNING. In-flight per-tier exclusion is enforced by the SINGLE row-level write lock that PostgreSQL takes on the obligation row for the duration of that UPDATE statement; concurrent sweep workers either win the lock + see ROW_COUNT=1 OR lose the lock + see ROW_COUNT=0 (dropped as stale).

**Normative escalation_key invariant (R55 HIGH-1 + R56 HIGH-1 + R57 HIGH-1 + R62 HIGH-1 closure 2026-05-21 — applies across all six canonical steps STEP A through STEP F):** `escalation_key` is **always NULL** between sweep transactions (i.e., post-commit of every STEP A→F transaction). STEP E's tier-advance UPDATE EXPLICITLY does NOT set escalation_key to non-NULL — it remains NULL so the next scheduled deadline expiry passes the R53 eligibility predicate `obligation.escalation_key IS NULL` (evaluated inside STEP A). The pre-R52 "per-tier-cycle marker" semantic is now provided by **sweep_cycle_counter** itself (durable monotonic value) + **provider_attempt rows with the matching sweep_cycle_id** (durable evidence of recipient fan-out per tier). escalation_key is preserved in the schema for backward compatibility with P-027 §4.68 but is now permanently NULL in the canonical SI-022 model; it's not used as a marker. acknowledge/respond/resolve wrappers don't touch escalation_key. **Implementer: do NOT set escalation_key to non-NULL anywhere in the canonical sweep flow.** R53 eligibility predicate (STEP A), STEP E UPDATE, idempotency contract, and tests #9/#10 are all consistent with this single-state invariant: escalation_key IS NULL ⟺ sweep eligible (combined with other predicates).

**STEP C (RESTATED — folded from prior numbered "3. Recipient fan-out"; R27 HIGH-1 + R29 HIGH-2 + R38 HIGH-1 + R39 HIGH-1 + R62 HIGH-1 closure 2026-05-21 deterministic-sweep-cycle-id idempotency + principal-id contract):** call `compute_crisis_recipient_mapping(crisis_event_id, severity, target_tier)`; the normative sweep INSERT uses a DETERMINISTIC per-sweep attempt_sequence captured BEFORE recipient fan-out (R39 HIGH-1: replaces non-deterministic MAX+1 recomputation). The fan-out INSERT runs AFTER STEP B (lifecycle transition) has succeeded and BEFORE STEP D (Cat A audit):

```sql
-- R39 HIGH-1 + R40 HIGH-1 + R40 HIGH-2 + R63 HIGH-1 closure 2026-05-21 (unified attempt_sequence
-- contract; STEP C MUST NOT re-increment sweep_cycle_counter):
...
--
-- **R63 HIGH-1 closure 2026-05-21 (single counter mutation site):** sweep_cycle_counter is
-- incremented EXACTLY ONCE per sweep transaction — at STEP A's R53 guarded UPDATE-RETURNING
-- (shown elsewhere in this §6). The previously-shown standalone UPDATE block here at STEP C
-- is REMOVED to prevent a duplicate increment: an implementer following both the R53 guarded
-- UPDATE at STEP A AND a second UPDATE here at STEP C would advance sweep_cycle_counter twice
-- per sweep, committing an execution under a generation different from the scheduled
-- scheduled_for_obligation_generation. That breaks the deterministic per-sweep idempotency
-- proof + replay safety. STEP C consumes v_sweep_cycle_id captured by STEP A's RETURNING clause
-- — it does NOT re-derive or re-mutate the counter.
--
...
-- STEP C fan-out INSERT (R62 HIGH-1 renamed from STEP 3; R63 HIGH-1: STEP C starts here at the
-- provider_attempt INSERT — no sweep_cycle_counter UPDATE in STEP C): uses v_sweep_cycle_id
-- captured from STEP A's RETURNING clause directly as attempt_sequence (NOT MAX+1):
INSERT INTO notification_crisis_provider_attempt
...
--
-- STEP C EXISTENCE invariant verification (R62 HIGH-1 renamed from "STEP 5 final verification"; R39 HIGH-1): before committing the tier-advance UPDATE at STEP E,
...
-- (all recipient_role variants). **R59 HIGH-1 + R60 HIGH-1 closure 2026-05-21: validation is
-- an EXISTENCE invariant (NOT raw INSERT ROW_COUNT), counting both newly-inserted and pre-existing
-- idempotent rows.** After the STEP C INSERT...ON CONFLICT DO NOTHING (R62 HIGH-1 renamed from STEP 3) completes, run:
--   SELECT COUNT(*) AS observed
--   FROM notification_crisis_provider_attempt
--   WHERE tenant_id=v_tenant_id
--     AND crisis_event_id=v_crisis_event_id
--     AND sweep_cycle_id=v_sweep_cycle_id;
-- **R63 HIGH-2 + R64 HIGH-2 closure 2026-05-21 (zero-recipients fail-closed for required target tiers):**
--   **R64 HIGH-2 correction:** target_tier is one of {'care_team', 'clinical_on_call', 'regulatory'}
--   per next_tier() canonical return — NOT a recipient_role. The prior R63 wording incorrectly named
--   'operator_escalation' (which is a recipient_role returned by the regulatory branch, not a target_tier
--   value itself); a regulatory sweep with zero mapping rows could miss the fail-closed clause and
--   still pass the existence check vacuously with observed=expected=0, allowing STEP E to advance or
--   recheck regulatory tier with no provider_attempt pressure.
--
--   For target_tier ∈ {'care_team', 'clinical_on_call', 'regulatory'} (the safety-critical non-
--   emergency-contact tiers), a mapping cardinality of ZERO is a config-floor violation: it means
--   tenant config passed preflight but a required recipient source resolved to nothing at sweep time
--   (e.g., race between preflight and tenant config change, principal_id no longer resolves, address
--   validation drift, or — for regulatory target_tier — operator_escalation_recipient/_principal_id
--   resolution disappeared on a regulatory_reporting=true tenant). ROLLBACK STEP A→F + emit
--   crisis.dispatch_attempt_failed Cat C with payload zero_recipients_for_required_tier=true (include
--   target_tier value + recipient_role missing list) + abort sweep. STEP E (tier ADVANCE) MUST NOT
--   run against an empty mapping for a required target tier. Tier='care_team' branch also includes
--   the same zero-recipients fail-closed for the initial-dispatch path at STEP 4b (the
--   compute_crisis_initial_recipient_mapping output for care_team MUST be ≥ 1 — checked at STEP 4b
--   pre-INSERT; otherwise crisis.dispatch_attempt_failed Cat C + rollback). Emergency_contact
--   recipient_role within target_tier='clinical_on_call' MAY have zero tuples (no active
--   emergency_contact_share consent grant); that is in-design, not a config-floor violation —
--   evaluated by recipient_role within the mapping, not by target_tier.
-- If observed != cardinality of compute_crisis_recipient_mapping output: ROLLBACK + emit
-- crisis.dispatch_attempt_failed Cat C + retry under exponential backoff. If observed = expected
-- (whether via fresh inserts OR pre-existing idempotent rows from same-transaction worker retry):
-- proceed to STEP D (Cat A audit emit). STEP E (Tier ADVANCE) MUST run to completion once the existence invariant passes (R62 HIGH-1 renamed from "STEP 4 ... STEP 5").
-- Retry/no-op detection at the worker ENTRY POINT remains bound to crisis_sweep_execution.completed_at
-- + sweep_cycle_id_committed (committed-prior-attempt skip).
...
**R41 MED-1 + R42 HIGH-2 + R43 HIGH-1 transaction-model clarification (SOLE normative recovery contract; durable sweep-execution-id idempotency for committed-replay):**

Sweep work items are scheduled via a durable `crisis_sweep_execution` table (NEW at this SI's P-040 CDM follow-on): each scheduled sweep allocates a `sweep_execution_id UUID` and inserts a row with `(tenant_id, crisis_event_id, scheduled_at, scheduled_for_obligation_generation INT NOT NULL, claimed_by_worker_id NULL, claim_expires_at NULL, completed_at NULL, sweep_cycle_id_committed NULL, fencing_token=1)`.

**R52 HIGH-1 closure 2026-05-21: per-obligation-generation uniqueness invariant.** Each escalation_obligation row's sweep_cycle_counter is the canonical "current generation" — the scheduler MUST schedule at most one execution per (tenant_id, crisis_event_id, generation), enforced by canonical UNIQUE constraint `crisis_sweep_execution_open_uk` ON `(tenant_id, crisis_event_id, scheduled_for_obligation_generation) WHERE completed_at IS NULL` (partial unique index covering only un-completed rows). If two scheduler instances attempt to insert an execution for the same overdue deadline, only one INSERT succeeds; the other gets a constraint violation + retries by reading the now-committed row. **One open execution per logical sweep generation; no duplicate sweeps possible at the row-level invariant.**

Additionally, STEP A of the sweep transaction (R62 HIGH-1 renamed from STEP 2) performs eligibility revalidation AND counter increment in a **single guarded UPDATE statement** (R52 HIGH-1 + R53 HIGH-1 closure 2026-05-21: SELECT-then-UPDATE is non-atomic; collapsed into one atomic UPDATE with RETURNING):
```sql
-- INSIDE STEP A→F atomic tx, ONE guarded UPDATE atomically revalidates eligibility AND
-- increments sweep_cycle_counter. No separate SELECT-then-UPDATE — race window eliminated.
UPDATE notification_crisis_escalation_obligation obligation
SET sweep_cycle_counter = sweep_cycle_counter + 1
FROM (
    SELECT to_state AS current_state
    FROM crisis_event_lifecycle_transition lt
    WHERE lt.tenant_id = v_tenant_id AND lt.crisis_event_id = v_crisis_event_id
    ORDER BY lt.transition_at DESC, lt.id DESC LIMIT 1
) AS lifecycle
WHERE obligation.tenant_id = v_tenant_id
  AND obligation.crisis_event_id = v_crisis_event_id
  AND now() > obligation.undeliverable_deadline
  AND obligation.escalation_key IS NULL
  AND obligation.escalation_tier IS NOT NULL
  AND lifecycle.current_state IN ('detected', 'escalated', 'acknowledged', 'responded')
  AND obligation.sweep_cycle_counter + 1 = $execution.scheduled_for_obligation_generation
RETURNING obligation.sweep_cycle_counter INTO v_sweep_cycle_id;
-- IF zero rows updated (ROW_COUNT = 0): sweep is stale (another execution already advanced obligation,
-- OR lifecycle changed, OR deadline was reset by acknowledge/respond/resolve, OR generation mismatch).
-- Raise sweep_stale_eligibility_lost; ROLLBACK entire STEP A→F transaction (R62 HIGH-1 renamed from STEP A→F); emit
-- crisis.sweep_stale_eligibility_dropped Cat C audit (in a separate post-rollback autocommit tx);
-- mark execution row's completed_at=now() with sweep_cycle_id_committed=NULL (terminal mark indicating
-- "completed-as-stale-no-op"); no fan-out, no Cat A audit, no tier-advance.
```
The single guarded UPDATE acquires PostgreSQL's row-level write lock on the obligation row atomically with the eligibility revalidation — no race window between "validated" and "incremented". If acknowledge/respond/resolve runs between scheduling and sweep execution (resets deadline/escalation_key OR changes lifecycle), the predicates fail; ROW_COUNT=0; sweep correctly dropped as stale.

**R48 HIGH-1 closure 2026-05-21 (sweep STEP C is transactionally ENQUEUE-ONLY; provider delivery is fenced) [R62 HIGH-1 renamed from STEP 3]:** STEP C fan-out INSERTs provider_attempt rows + (per the existing P-027 outbox model) enqueues dispatch work into a downstream notification-delivery outbox. **STEP C itself MUST NOT make external provider calls (SMS send, email send, in-app push) during the sweep transaction** — external sends are downstream of COMMIT, executed by a separate notification-delivery worker reading from the outbox. The delivery worker reads outbox rows that include `sweep_execution_id` + `sweep_cycle_id` + `expected_fencing_token` (R49 HIGH-1 closure 2026-05-21: expected_fencing_token is now structurally enforced in the pre-dispatch recheck); before dispatch the delivery worker rechecks:
```sql
SELECT 1 FROM crisis_sweep_execution
WHERE sweep_execution_id = $...
  AND completed_at IS NOT NULL
  AND sweep_cycle_id_committed = $outbox.sweep_cycle_id
  AND fencing_token = $outbox.expected_fencing_token;  -- R49 HIGH-1: structurally bind dispatch to winning lease owner
```
If zero rows (sweep was rolled back, taken over, or otherwise non-authoritative), the outbox row is dropped + delivery suppressed + a `crisis.delivery_fence_mismatch_dropped` Cat C audit emitted for forensic correlation. This separates irreversible external sends from the atomic STEP A→F transaction (R62 HIGH-1 renamed from STEP A→F): a worker that loses its lease mid-fan-out has its STEP F closing guard reject COMMIT → ROLLBACK discards both the provider_attempt INSERTs AND the outbox enqueues; the delivery worker never sees the stale work. Even if the delivery worker has a copy of an outbox row from a transient cache/visibility delay, the recheck against `completed_at IS NOT NULL AND sweep_cycle_id_committed = $sweep_cycle_id` catches the rollback. **No duplicate external sends, no orphaned deliveries, no double notifications across takeovers.**

**R44 HIGH-1 + R45 HIGH-1 claim-lease + fencing-token contract (closure 2026-05-21; non-locking claim outside the atomic STEP A→F transaction):** the scheduler/worker claim happens in a SEPARATE autocommit transaction BEFORE the atomic STEP A→F sweep transaction begins — never under SELECT FOR UPDATE on the same row that STEP A→F locks. Claim acquisition:
```sql
-- AUTOCOMMIT (separate from STEP A→F atomic transaction):
-- Atomic claim with lease + monotonic fencing_token; takeover allowed for expired/unclaimed
UPDATE crisis_sweep_execution
SET claimed_by_worker_id = $worker_id,
    claim_expires_at = now() + INTERVAL '60 seconds',
    fencing_token = fencing_token + 1   -- monotonic per-takeover token
WHERE sweep_execution_id = $next_due_id
  AND completed_at IS NULL
  AND (claimed_by_worker_id IS NULL OR claim_expires_at < now())
RETURNING sweep_execution_id, fencing_token;  -- caller MUST capture v_fencing_token
```
The 60-second lease bound ensures a crashed worker's claim becomes reclaimable. **Heartbeat renewal is also done in SHORT AUTOCOMMIT updates** (NOT inside the STEP A→F transaction) every 20s during long-running fan-out:
```sql
-- Heartbeat (autocommit; outside STEP A→F):
UPDATE crisis_sweep_execution
SET claim_expires_at = now() + INTERVAL '60 seconds'
WHERE sweep_execution_id = $execution_id
  AND claimed_by_worker_id = $worker_id
  AND fencing_token = $captured_fencing_token  -- protect against takeover
  AND completed_at IS NULL;
-- If no rows updated → the worker has been taken over (different fencing_token) or completed; ABORT in-flight work.
```
**Fencing-token protection:** STEP A→F atomic transaction includes a fencing-token guard as its first statement:
```sql
-- INSIDE the STEP A→F atomic transaction (before any other writes):
SELECT 1 FROM crisis_sweep_execution
WHERE sweep_execution_id = $execution_id
  AND claimed_by_worker_id = $worker_id
  AND fencing_token = $captured_fencing_token  -- must match claim-time token
  AND completed_at IS NULL;
-- IF zero rows: a takeover occurred OR worker's claim was lost; raise sweep_fencing_token_mismatch
-- and rollback before any side effects. This bounds STEP A→F to the worker whose fencing_token won.
```
**Lease takeover safety:**
- Crashed before COMMIT: second worker takes over (fencing_token incremented); first worker's heartbeat fails (fencing_token mismatch); if first worker somehow tries to commit STEP A→F, the fencing-token guard at tx start raises sweep_fencing_token_mismatch and rolls back. Second worker re-executes STEP A→F cleanly with fresh sweep_cycle_id.
- COMMITTED but completed_at not set: **architecturally impossible post-R47** because sweep_cycle_id_committed is now written by the SAME final guarded UPDATE that sets completed_at — they are committed atomically. The pre-R47 "impossible-but-bounded" defense-in-depth case (completed_at IS NULL + sweep_cycle_id_committed IS NOT NULL) cannot occur under the R47 model. The `crisis.sweep_claim_recovery_with_committed_cycle` Cat C audit + recovery path are retained ONLY as a tripwire: if ever observed, it indicates a database integrity defect (e.g., manual UPDATE of sweep_cycle_id_committed outside the canonical wrapper) and the recovery path emits the audit + heals completed_at = now() defensively. Under the canonical R47 contract this code path should never execute in production.
- ACK loss after COMMIT: completed_at IS NOT NULL detected on next-due check → emit `crisis.sweep_replay_after_commit_ack_loss` Cat C audit + skip.

This non-locking claim + autocommit heartbeat + fencing-token-guarded STEP A→F works correctly under PostgreSQL row-lock semantics: the atomic STEP A→F transaction takes an UPDATE lock on the obligation row + provider_attempt inserts but does NOT hold a row lock on crisis_sweep_execution for the full duration; heartbeat updates run in separate short autocommit transactions and don't block on the in-flight sweep tx.

**Atomic sweep transaction — single canonical ordering (R61 HIGH-1 closure 2026-05-21: collapsed the legacy STEP A→F numbered ordering into the single canonical STEP A→F sequence used by Sub-decision 6's per-row algorithm; sweep_cycle_counter is incremented EXACTLY ONCE at STEP A; there is no duplicate increment elsewhere):**

The transaction begins with a fencing-token guard SELECT (R46 HIGH-2 + R47 HIGH-1 closure 2026-05-21: NO SELECT FOR UPDATE on crisis_sweep_execution; sweep_cycle_id_committed is NOT written here — it is written ONLY by the closing guarded UPDATE so the durable replay marker is unambiguous post-commit). The non-locking fencing-token guard SELECT validates the caller still holds the claim (claimed_by_worker_id = $worker_id AND fencing_token = $captured_fencing_token AND completed_at IS NULL); raises sweep_fencing_token_mismatch + rolls back if zero rows. Then the canonical STEP A→F ordering (per Sub-decision 6's per-row algorithm) runs:

- **STEP A** = R53 atomic eligibility-revalidation + sweep_cycle_counter UPDATE-RETURNING (the SOLE place sweep_cycle_counter increments; ROW_COUNT=0 → ROLLBACK + stale audit + EXIT)
- **STEP B** = Lifecycle transition INSERT (after STEP A ROW_COUNT=1)
- **STEP C** = Recipient fan-out via INSERT...SELECT...FROM compute_crisis_recipient_mapping(...) ON CONFLICT DO NOTHING + EXISTENCE invariant verification
- **STEP D** = Cat A audit emit (`crisis.no_acknowledgement_escalation`)
- **STEP E** = Tier ADVANCE + deadline reschedule UPDATE on escalation_obligation
- **STEP F** = Final guarded UPDATE on crisis_sweep_execution (sets completed_at + sweep_cycle_id_committed atomically with fencing_token + claim_expires_at predicates per R46/R47/R51)

The legacy "STEP A→F" numbered transaction model is SUPERSEDED by the STEP A→F sequence. The closing guarded UPDATE (STEP F) is detailed below:
```sql
UPDATE crisis_sweep_execution
SET completed_at = now(),
    sweep_cycle_id_committed = $v_sweep_cycle_id   -- written ONLY here, atomically with completed_at
WHERE sweep_execution_id = $execution_id
  AND claimed_by_worker_id = $worker_id
  AND fencing_token = $captured_fencing_token   -- must STILL match captured value at claim time
  AND completed_at IS NULL
  AND claim_expires_at > now();                  -- R51 HIGH-1: lease MUST still be un-expired; a
                                                  -- crashed/paused worker whose lease has expired
                                                  -- cannot commit even under fencing_token-coincidence
                                                  -- races. Combined with the autocommit takeover
                                                  -- claim (which advances fencing_token AND extends
                                                  -- claim_expires_at), this triple-guard
                                                  -- (claimed_by_worker_id + fencing_token +
                                                  -- claim_expires_at) makes takeover and
                                                  -- old-token finalization mutually exclusive.
-- Implementer MUST verify exactly 1 row was updated. If 0 rows: a takeover occurred mid-transaction
-- (different fencing_token now) — raise sweep_fencing_token_mismatch_at_commit + ROLLBACK; do NOT
-- COMMIT. The taking-over worker will re-execute STEP A→F cleanly with its new fencing_token.
```
This final guarded compare-and-set is the database invariant that bounds STEP A→F commit to the worker whose fencing_token is still authoritative. A worker that passed the opening guard but lost its lease during fan-out CANNOT commit; the rollback discards all side effects atomically (per the R42 atomic-tx model). The opening fencing-token guard (STEP 2) + this closing guarded UPDATE (STEP 5) together form a "trapped invariant" — both must succeed under the same fencing_token, with COMMIT only happening after the second guard returns 1 row. Heartbeat updates that lose to a takeover are detected by the next renewal cycle (where 0 rows updated → worker aborts).

**Pre-commit failure:** rollback discards everything including the completed_at mark; a retry re-claims and re-allocates a new sweep_cycle_id (no overlap with rolled-back attempt because no rows are visible).

**Post-commit ambiguity** (worker COMMITs but ACK/connection lost before scheduler removes the work item): the scheduler attempts to re-execute the SAME crisis_sweep_execution row — but the row now has `completed_at IS NOT NULL` AND `sweep_cycle_id_committed IS NOT NULL`. The replay logic checks this state FIRST: if completed_at is set, the sweep is already done — emit `crisis.sweep_replay_after_commit_ack_loss` Cat C audit (informational) + skip re-execution + return success to the scheduler. **No duplicate fan-out, no duplicate Cat A audit, no double tier-advance.**

**R43 HIGH-1 idempotency proof:** crisis_sweep_execution.completed_at + crisis_sweep_execution.sweep_cycle_id_committed are the durable per-sweep replay-detection record. A committed sweep is permanently marked terminal; replays after COMMIT detect this state and no-op. A scheduler cannot re-enter a committed sweep row to execute it twice. The next sweep deadline (per Sub-decision 6 escalation_obligation.undeliverable_deadline) is fired by a NEW crisis_sweep_execution row insertion, allocating a NEW sweep_execution_id + NEW sweep_cycle_id — orthogonal to any committed predecessor.
...
5. **Tier ADVANCE + deadline reschedule (R13 HIGH-1 + R20 HIGH-1 + R21 HIGH-1 + R58 HIGH-1 closure 2026-05-21: R58 removed "release the per-tier-cycle lock" terminology from heading — there is no per-tier-cycle lock in the canonical SI-022 model; sweep concurrency is enforced by R53 row lock + crisis_sweep_execution generation/fencing + sweep_cycle_counter, NOT by escalation_key claim/release. do NOT terminalize on final-tier exhaustion — only `record_crisis_resolution()` may terminalize via escalation_tier=NULL; ALL next_tier() invocations in STEP 5 use the tenant-aware 3-arg signature `next_tier(current_tier, severity, regulatory_reporting)` per R20/R21 closures; v_regulatory_reporting is read ONCE from tenant_config at sweep transaction start and reused for STEP 3 + STEP 5):** UPDATE escalation_obligation SET:
    ...
    - `escalation_key = NULL` — preserved (NO-OP in canonical SI-022 since R55-R57: escalation_key is ALWAYS NULL between sweep transactions and is NOT a per-tier-cycle lock; this UPDATE field is preserved for explicit clarity that the legacy P-027 §4.68 column is being kept at its NULL invariant value; sweep concurrency is via R53 row lock + crisis_sweep_execution generation/fencing + sweep_cycle_counter)
...
All call sites in Sub-decision 6 sweep contract (STEP C target_tier, STEP E escalation_tier advance; R62 HIGH-1 renamed from STEP 3 / STEP 5) MUST pass the tenant's `regulatory_reporting` flag as the third argument; per-row sweep transaction reads `regulatory_reporting` once from tenant_config alongside severity and uses it for the per-row tier-advance computation.
...
**Idempotency within a tier cycle (R55 HIGH-1 + R56 HIGH-1 closure 2026-05-21: escalation_key plays NO ROLE in sweep concurrency control; it is permanently NULL in the canonical sweep flow):** in-flight exclusion is enforced solely by (a) the PostgreSQL row-level write lock acquired by the R53 guarded UPDATE on the obligation row for the duration of the STEP A→F atomic transaction; (b) the crisis_sweep_execution generation/fencing invariant (one open execution per (tenant_id, crisis_event_id, scheduled_for_obligation_generation) WHERE completed_at IS NULL); (c) sweep_cycle_counter/provider_attempt.sweep_cycle_id durable evidence. The atomic per-row transaction commits all 5 steps together OR rolls all back; partial-state mid-cycle is impossible. Multiple concurrent sweep workers either win the obligation row lock + see ROW_COUNT=1 on the R53 UPDATE, OR lose the lock + see ROW_COUNT=0 (dropped as stale-no-op).

**Difference from one-shot model (R7 HIGH-1 historical context; R55-R57 architectural refinement: claim-lock semantics fully removed):** the pre-R55 design used escalation_key as a per-tier-cycle CLAIM lock (set during sweep, NULL'd at the end of each tier). The R55+ design REPLACES that with PostgreSQL row-level write lock on the obligation row + crisis_sweep_execution generation/fencing as the in-flight exclusion mechanism. escalation_key is now permanently NULL in canonical SI-022 (NOT a lock or marker). **Terminal exclusion is NOT determined by tier-exhaustion** (final-tier rows stay sweep-visible on a recheck cadence per R13 closure) — it is determined ONLY by record_crisis_resolution() setting escalation_tier=NULL.
...
9. No-acknowledgement timer — tier-cycle progression (R7 HIGH-1 + R13 HIGH-1 + R15 HIGH-1 + R56 HIGH-2 closure 2026-05-21: escalation_key=uuid claim language removed; sweep concurrency control is now via R53 guarded UPDATE row lock + sweep_cycle_counter + crisis_sweep_execution generation): from initial `escalation_tier='care_team'` row, after first undeliverable_deadline expiry, sweep performs atomic 4-step closure (R53 guarded UPDATE atomically revalidates eligibility + increments sweep_cycle_counter + lifecycle transition + recipient fan-out at sweep_cycle_id + Cat A audit emit + STEP 5 UPDATE: escalation_tier='clinical_on_call', new deadline; **escalation_key remains NULL throughout**). After second deadline expiry, second sweep: (a) IFF severity='life_threatening', advances escalation_tier to 'regulatory'; (b) IFF severity ∈ {'non_imminent', 'imminent'}, reaches final-tier-exhausted state — escalation_tier PRESERVED at 'clinical_on_call' (NOT NULL), final_tier_exhausted_at SET, one-time `crisis.final_tier_reached` Cat A emitted, undeliverable_deadline rescheduled. Test asserts EACH tier transition fires in sequence; provider_attempt rows have sweep_cycle_id matching the obligation.sweep_cycle_counter for that sweep; escalation_key remains NULL on the obligation row before, during, and after every sweep transaction (asserted via SELECT immediately after each sweep COMMIT).
10. Tier-cycle idempotency + final-tier non-terminalization (R7 HIGH-1 + R13 HIGH-1 + R15 HIGH-1 + R56 HIGH-2 closure 2026-05-21: in-flight exclusion via R53 guarded UPDATE row lock + sweep_cycle_counter generation, NOT escalation_key claim-lock): within a single sweep transaction, concurrent workers contend for the obligation row lock; the winner sees ROW_COUNT=1 + commits, losers see ROW_COUNT=0 + roll back as stale-no-op. After commit + tier advance OR final-tier exhaustion, the row becomes eligible for the NEXT deadline expiry. Test asserts: (a) concurrent sweep workers do NOT double-escalate (one wins via row lock; others ROW_COUNT=0 stale-drop); (b) re-running sweep after commit + before next deadline does NOT escalate (predicates fail); (c) re-running sweep after next deadline DOES advance to next tier OR (if final-tier) fires a recheck-cadence sweep with `is_final_tier_recheck=true`; (d) **ONLY** rows where `escalation_tier IS NULL` (set exclusively by `record_crisis_resolution()`) are permanently excluded; final-tier-exhausted rows continue to fire on recheck cadence; (e)/(f)/(g) per-severity assertions unchanged. **Additionally (R56 HIGH-2): escalation_key MUST be NULL on the obligation row at all points before, during, and after every sweep transaction; tests verify this with explicit SELECT assertions.**
...
**12 new action IDs** under `crisis.*` namespace EXTENDING P-025 (which contributed `crisis.escalation_destination_resolved` Cat B): **8 Cat A + 0 Cat B + 4 Cat C** (R13 + R1 + R43 + R45 + R49 + R52 additions tracked in closure log):
...
| 9 | `crisis.sweep_replay_after_commit_ack_loss` | Cat C | NOT sampled (governance recovery; R43 HIGH-1 + R45 MED-1 closure 2026-05-21: emitted when scheduler attempts to re-execute a crisis_sweep_execution row whose completed_at IS NOT NULL — committed sweep was replayed due to worker ACK loss. Skip re-execution + return success. Payload: sweep_execution_id, sweep_cycle_id_committed, original_completed_at, replay_at, scheduler_id) | P2 governance-partition |
| 10 | `crisis.sweep_claim_recovery_with_committed_cycle` | Cat C | NOT sampled (governance recovery; R44 HIGH-1 + R45 MED-1 closure 2026-05-21: emitted when a second worker takes over a stranded claim and observes the impossible-but-bounded state completed_at IS NULL + sweep_cycle_id_committed IS NOT NULL — heal terminal mark to now() without re-executing STEP A→F. Payload: sweep_execution_id, sweep_cycle_id_committed, previous_worker_id, recovery_worker_id, fencing_token_before, fencing_token_after, recovery_at) | P2 governance-partition |
| 11 | `crisis.delivery_fence_mismatch_dropped` | Cat C | NOT sampled (governance recovery; R49 HIGH-1 closure 2026-05-21: emitted when a downstream notification-delivery worker rechecks crisis_sweep_execution before dispatching an outbox row and detects fencing_token mismatch — outbox row dropped + delivery suppressed to prevent stale/non-authoritative external sends. Payload: outbox_row_id, sweep_execution_id, sweep_cycle_id, expected_fencing_token, observed_fencing_token, observed_completed_at, delivery_worker_id, dropped_at) | P2 governance-partition |
| 12 | `crisis.sweep_stale_eligibility_dropped` | Cat C | NOT sampled (governance recovery; R52 HIGH-1 closure 2026-05-21: emitted when a sweep transaction STEP 2 obligation-eligibility revalidation finds the sweep is stale (another generation already advanced obligation, OR lifecycle changed, OR deadline reset by acknowledge/respond/resolve, OR sweep_cycle_counter mismatched scheduled_for_obligation_generation) — sweep is correctly marked completed-as-stale-no-op without fan-out/audit/tier-advance. Payload: sweep_execution_id, scheduled_for_obligation_generation, current_obligation_sweep_cycle_counter, current_undeliverable_deadline, current_escalation_key_status, current_escalation_tier, current_lifecycle_state, dropped_at) | P2 governance-partition |

**Total: 12 new action IDs (8 Cat A + 0 Cat B + 4 Cat C; was 11 = 8A+3C pre-R52 HIGH-1; was 10 = 8A+2C pre-R49; was 9 = 8A+1C pre-R45).** §1 + §3 header + all per-class counters updated.
...
4. **OQ4 — Tenant configuration of fan-out channels.** Tenants configure WHICH channels exist (sms/email/in-app providers) but NOT whether fan-out happens. Ratifier confirms the canonical tenant_config keys: `crisis.fanout_channels[]` (allow-list per channel) + `crisis.clinical_on_call_channel` (which configured channel for clinical-on-call) + **(R63 HIGH-2 closure 2026-05-21)** `crisis.clinical_on_call_recipient` (recipient address for clinical-on-call; symmetric with `crisis.operator_escalation_recipient`; required non-null/address-valid on EVERY tenant per the every-tenant clinical_on_call advancement rule) + `crisis.clinical_on_call_principal_id` (principal_id binding for the clinical_on_call recipient; required non-null on EVERY tenant; sourced via the canonical principal-resolution surface — tenant_account_membership.principal_id with the clinical_on_call_role, or a dedicated tenant config column; populated into provider_attempt.recipient_principal_id at fan-out per the R36/R37 principal-id contract) + **(R18 HIGH-1 closure 2026-05-21)** `crisis.operator_escalation_channel` (channel-class for regulatory-tier operator on-call) + `crisis.operator_escalation_recipient` (recipient address for operator on-call) + **(R31 HIGH-1 closure 2026-05-21)** `crisis.emergency_contact_channel` (channel-class for emergency-contact dispatch when patient has active emergency_contact_share consent grant).
...
- **(R63 HIGH-2 closure 2026-05-21)** `crisis.clinical_on_call_recipient` — MUST be non-null AND address-format-valid for the configured channel-class **on ALL tenants** (symmetric with the operator_escalation_recipient rule for regulatory_reporting=true tenants below; tightened because final-tier clinical_on_call rechecks are safety-critical AND clinical_on_call is the de-facto final tier for non-life-threatening severities AND life-threatening on regulatory_reporting=false; a tenant configuring `clinical_on_call_channel` but leaving recipient null would allow `compute_crisis_recipient_mapping(..., target_tier='clinical_on_call')` to return zero rows, leaving STEP C's existence-invariant check vacuously satisfied AND silently suppressing the clinical_on_call escalation pressure at the exact unresolved terminal-risk state)
- **(R63 HIGH-2 closure 2026-05-21)** `crisis.clinical_on_call_principal_id` — MUST be non-null AND resolve to an existing principal (tenant_account_membership.principal_id OR tenant config column) **on ALL tenants** (required so `compute_crisis_recipient_mapping` returns a non-null `recipient_principal_id` on the clinical_on_call tuple; required for R36/R37 wrapper authorization to match the JWT-verified principal during clinical_on_call acknowledgement/respond; missing principal_id binding would either fail authorization for legitimate clinical_on_call responders OR silently fall back to address-equality matching which was REMOVED at R36 HIGH-2)
...
- **(R65 HIGH-1 closure 2026-05-21)** `crisis.operator_escalation_principal_id` — MUST be non-null AND resolve to an existing principal (`principal_exists(...)`); symmetric with R63 HIGH-2's every-tenant `clinical_on_call_principal_id` rule. Required so `compute_crisis_recipient_mapping(..., target_tier='regulatory')` returns a non-null `recipient_principal_id` on the operator_escalation tuple; required for R36/R37 wrapper authorization to match the JWT-verified principal during regulatory-tier acknowledgement/respond; missing principal_id binding would either violate the provider_attempt CHECK constraint (recipient_principal_id NOT NULL for principal-addressable recipient_roles) and roll back regulatory fan-out, OR leave legitimate operator responders unable to pass tier-ownership authorization.
...
1. **Deployment preflight DO block** (§10) — fail-closed assertion in THREE parts:
   - **Part A (every tenant; R63 HIGH-2 extended 2026-05-21):** assert `cardinality(crisis.fanout_channels[]) > 0` AND `crisis.clinical_on_call_channel IS NOT NULL` AND `crisis.clinical_on_call_channel = ANY(crisis.fanout_channels[])` AND `crisis.clinical_on_call_recipient IS NOT NULL` (address-format-valid for the configured `clinical_on_call_channel`) AND `crisis.clinical_on_call_principal_id IS NOT NULL` (resolves to existing principal) — applies regardless of regulatory_reporting value.
   - **Part B (regulatory_reporting=true tenants only; R65 HIGH-1 extended 2026-05-21):** assert `crisis.operator_escalation_channel IS NOT NULL` AND `crisis.operator_escalation_recipient IS NOT NULL` AND `crisis.operator_escalation_channel = ANY(crisis.fanout_channels[])` AND `crisis.operator_escalation_principal_id IS NOT NULL` (resolves to existing principal via `principal_exists(...)`).
...
2. **Static-analyzer rule TLC-CRISIS-003** (§7) — extended to validate THREE rule sets:
   - **(every tenant; R63 HIGH-2 extended 2026-05-21)** rejects ANY tenant config row where `fanout_channels[] IS NULL OR cardinality(fanout_channels[]) = 0 OR clinical_on_call_channel IS NULL OR NOT (clinical_on_call_channel = ANY(fanout_channels[])) OR clinical_on_call_recipient IS NULL OR clinical_on_call_principal_id IS NULL OR NOT principal_exists(clinical_on_call_principal_id)`
   - **(regulatory_reporting=true conditional; R65 HIGH-1 extended 2026-05-21)** rejects tenant config rows where `regulatory_reporting=true AND (operator_escalation_channel IS NULL OR operator_escalation_recipient IS NULL OR operator_escalation_principal_id IS NULL OR NOT (operator_escalation_channel = ANY(fanout_channels[])) OR NOT principal_exists(operator_escalation_principal_id))`
...
3. **Wrapper-procedure runtime validation** — `record_crisis_initiation()` (Phase A implementation) at session-bind time:
   - ALWAYS asserts fanout_channels[] non-empty + clinical_on_call_channel non-null + clinical_on_call_channel ∈ fanout_channels[] + **(R63 HIGH-2 closure 2026-05-21)** clinical_on_call_recipient non-null + clinical_on_call_principal_id non-null + principal_exists(clinical_on_call_principal_id); fail-closed emitting `crisis.dispatch_attempt_failed` Cat C if any missing
   - ASSERTS operator_escalation keys IFF regulatory_reporting=true — **(R65 HIGH-1 extended 2026-05-21)** including operator_escalation_channel + operator_escalation_recipient + operator_escalation_principal_id non-null + principal_exists(operator_escalation_principal_id); same fail-closed behavior
...
Authored on `spec/SI-022-crisis-response-slice-2026-05-21` branch off main at `fab0615` (post-P-038 merge). v0.1 commit `e7a7ebb`. v0.2 commit `c2b9e15`. v0.3 commit `f4001d2`. v0.4 commit `6ec0cc9`. v0.5 commit `18ef338`. v0.6 commit `595db3c`. v0.7 commit `0e287b5`. v0.8 commit `e34bf69`. v0.9 commit `adafe4b`. v0.10 commit `88af316`. v0.11 commit `91ebc7c`. v0.12 commit `bef4682`. v0.13 commit `0db543b`. v0.14 commit `ff19084`. v0.15 commit `e20b00a`. v0.16 commit `44b283b`. v0.17 commit `fcded14`. v0.18 commit `37ae5ea`. v0.19 commit `34ea3af`. v0.20 commit `b210fa0`. v0.21 commit `72a49c8`. v0.22 commit `f542665`. v0.23 commit `23cad27`. v0.24 commit `1c3bf88`. v0.25 commit `a76ff62`. v0.26 commit `e03ef23`. v0.27 commit `20ce894`. v0.28 commit `c3ff7a7`. v0.29 commit `12bcb0f`. v0.30 commit `2a57218`. v0.31 commit `a1e1060`. v0.32 commit `d6ad512`. v0.33 commit `38f8698`. v0.34 commit `987e2ee`. v0.35 commit `7c22693`. v0.36 commit `43a844a`. v0.37 commit `4a61b24`. v0.38 commit `e2212f9`. v0.39 commit `f2d9b5d`. v0.40 commit `a677e41`. v0.41 commit `9b38c6b`. v0.42 commit `3f94899`. v0.43 commit `5ba6da8`. v0.44 commit `2e19fd9`. v0.45 commit `71b07e1`. v0.46 commit `b4a5518`. v0.47 commit `820b757`. v0.48 commit `ddd090c`. v0.49 commit `e447d3f`. v0.50 commit `773cd08`. v0.51 commit `377628d`. v0.52 commit `102e8dd`. v0.53 commit `8c5917d`. v0.54 commit `3ae684f`. v0.55 commit `781274b`. v0.56 commit `e5e59d9`. v0.57 commit `40cc1e2`. v0.58 commit `204a0f9`. v0.59 commit `54d39ac`. v0.60 commit `cc4c76b`. v0.61 commit `9f18a22`. v0.62 commit `a25fad4`. v0.63 commit `d706877`. v0.64 commit `c237f97`. v0.65 commit `f9c9eda`. v0.66 commit `347f0f6`. v0.67 commit pending push for R67 verification.

**Non-normative historical-context disclaimer for §12 closure-log entries (R62 HIGH-1 codification 2026-05-21):** All STEP 1 / STEP 2 / STEP 3 / STEP 4 / STEP 5 references in the per-round closure-log entries below are HISTORICAL context describing the pre-R60/R61/R62 numbering and are NON-NORMATIVE. The canonical Sub-decision 6 sweep transaction step labels are STEP A → STEP F (per the §6 Sub-decision 6 normative body; R60 + R61 + R62 closures); engineering implementations MUST follow STEP A-F only. Sub-decision 2's STEP 1-5 ordering is a separate normative algorithm (synchronous emission flow) and is unrelated to Sub-decision 6's STEP A-F sweep ordering.

**v0.67 DRAFT 2026-05-21 — R66 closure applied (1 HIGH):**
- **R66 HIGH-1 closed:** emergency_contact projection inside clinical_on_call branch of compute_crisis_recipient_mapping was still 3-tuple `(emergency_contact_channel, 'emergency_contact', emergency_contact_address)` — arity-hole defect: cannot UNION across other 4-tuple branches; downstream STEP 4b/STEP C INSERTs reference mapping.recipient_principal_id which doesn't exist on the emergency_contact projection. Tenant with active emergency_contact_share consent could make clinical_on_call fan-out fail or diverge from existence/principal checks. Fix: emergency_contact projection now 4-tuple `(emergency_contact_channel, 'emergency_contact', emergency_contact_address, COALESCE(consent_grant.delegate_principal_id, NULL::uuid))` — explicit NULL::uuid 4th column when no delegate principal exists; emergency_contact is the ONLY recipient_role allowed to have NULL recipient_principal_id per provider_attempt CHECK (care_team / clinical_on_call / operator_escalation MUST be non-null per R37/R63/R65); explicit uniform-arity note for UNION discipline; tests assert emergency_contact rows expose nullable recipient_principal_id.

**v0.66 DRAFT 2026-05-21 — R65 closure applied (1 HIGH):**
- **R65 HIGH-1 closed:** regulatory branch of compute_crisis_recipient_mapping still returned 3-tuple `(operator_escalation_channel, 'operator_escalation', operator_escalation_recipient)` — omitted recipient_principal_id; symmetric gap to R64 HIGH-1 (clinical_on_call branch missing keys). Implementer would insert NULL recipient_principal_id for principal-addressable operator_escalation row → either violate provider_attempt CHECK (recipient_principal_id NOT NULL for care_team/clinical_on_call/operator_escalation) and ROLLBACK regulatory fan-out, OR leave legitimate operator responders unable to pass R36/R37 wrapper authorization. Part B enforcement also lacked operator_escalation_principal_id requirement. Fix: (a) regulatory branch returns 4-tuple `(operator_escalation_channel, 'operator_escalation', crisis.operator_escalation_recipient, crisis.operator_escalation_principal_id)` symmetric with clinical_on_call branch (R64 HIGH-1); (b) explicit prose 'recipient_address from operator_escalation_recipient; recipient_principal_id from operator_escalation_principal_id — NOT a tenant_account_membership lookup'; (c) Part B preflight + TLC-CRISIS-003 regulatory_reporting=true rule + record_crisis_initiation() runtime validation ALL extended to require operator_escalation_principal_id non-null + principal_exists() — symmetric with R63 HIGH-2's every-tenant clinical_on_call_principal_id rule.

**v0.65 DRAFT 2026-05-21 — R64 closures applied (2 HIGH):**
- **R64 HIGH-1 closed:** compute_crisis_recipient_mapping + compute_crisis_initial_recipient_mapping contracts still listed only `clinical_on_call_channel` in their source list; clinical_on_call branch returned `clinical_on_call_address` without binding to the R63 HIGH-2 NEW keys (`clinical_on_call_recipient` + `clinical_on_call_principal_id`); principal-derivation text listed tenant_account_membership/consent_grant/operator_escalation_principal_id sources but NOT the new clinical key. Implementer following the mapping contract could pass preflight while failing to populate clinical_on_call recipient address OR recipient_principal_id, suppressing fan-out OR breaking R36/R37 wrapper authorization for legitimate clinical_on_call responders. Fix: extended both mapping function source lists with crisis.clinical_on_call_recipient + crisis.clinical_on_call_principal_id + crisis.operator_escalation_principal_id; clinical_on_call branch now returns (clinical_on_call_channel, 'clinical_on_call', crisis.clinical_on_call_recipient, crisis.clinical_on_call_principal_id) — explicit assertion that recipient_address is NOT derived from any other source and recipient_principal_id is NOT a tenant_account_membership lookup; symmetric for initial-dispatch + sweep paths.
- **R64 HIGH-2 closed:** STEP C zero-recipient fail-closed clause named target_tier ∈ {'care_team', 'clinical_on_call', 'operator_escalation'} but operator_escalation is a recipient_role returned by the regulatory branch — NOT a target_tier value; the canonical target_tier set per next_tier() is {'care_team', 'clinical_on_call', 'regulatory'}. A regulatory sweep with zero mapping rows could miss the fail-closed clause + pass the existence check vacuously (observed=expected=0) + allow STEP E to advance/recheck regulatory tier with no provider_attempt pressure. Fix: changed target_tier set to {'care_team', 'clinical_on_call', 'regulatory'}; explicit prose correction noting prior 'operator_escalation' was a recipient_role mis-classification; payload extended to include target_tier value + recipient_role missing list; emergency_contact MAY-have-zero clause re-scoped to "recipient_role within target_tier='clinical_on_call'" (evaluated by recipient_role, not target_tier).

**v0.64 DRAFT 2026-05-21 — R63 closures applied (2 HIGH):**
- **R63 HIGH-1 closed:** STEP C RESTATED SQL block still included an executable UPDATE that incremented sweep_cycle_counter and reassigned v_sweep_cycle_id — duplicate of STEP A's R53 guarded UPDATE; implementer following both would advance sweep_cycle_counter twice per sweep, committing under a generation different from `scheduled_for_obligation_generation` + breaking deterministic per-sweep idempotency proof + replay safety. Fix: removed the standalone UPDATE block from STEP C SQL; replaced with explicit "STEP C consumes v_sweep_cycle_id from STEP A's RETURNING clause — does NOT re-derive or re-mutate the counter" prose; STEP C SQL now starts at the provider_attempt INSERT. sweep_cycle_counter mutation occurs at EXACTLY ONE site: STEP A's R53 guarded UPDATE-RETURNING.
- **R63 HIGH-2 closed:** OQ4 enforcement contract required `crisis.clinical_on_call_channel` non-null on every tenant but did NOT require the actual clinical_on_call recipient address or principal_id binding (symmetric gap to operator_escalation_recipient). A tenant configuring `clinical_on_call_channel` with null recipient would pass preflight + TLC-CRISIS-003 + runtime validation; `compute_crisis_recipient_mapping(..., target_tier='clinical_on_call')` would return zero rows; STEP C's existence-invariant check would vacuously pass against the empty mapping; clinical_on_call escalation pressure would silently be suppressed at the exact unresolved terminal-risk state R16/R20/R21 were meant to protect. Fix: (a) added 2 canonical OQ4 keys — `crisis.clinical_on_call_recipient` (address-format-valid) + `crisis.clinical_on_call_principal_id` (resolves to existing principal); both required non-null on EVERY tenant; (b) extended preflight Part A + TLC-CRISIS-003 every-tenant rule + record_crisis_initiation() runtime validation to ASSERT both new keys + principal_exists() resolution; (c) added STEP C zero-recipients fail-closed clause: target_tier ∈ {'care_team', 'clinical_on_call', 'operator_escalation'} (safety-critical tiers) with zero-cardinality mapping → ROLLBACK STEP A→F + crisis.dispatch_attempt_failed Cat C with payload zero_recipients_for_required_tier=true + sweep aborted; STEP E (tier ADVANCE) MUST NOT run against empty mapping for required target tier; emergency_contact tier MAY have zero recipients (no active consent grant) — in-design, not a config-floor violation.

**v0.63 DRAFT 2026-05-21 — R62 closure applied (1 HIGH):**
- **R62 HIGH-1 closed:** R61 closure was incomplete — Sub-decision 6 still contained legacy numbered "2. Tier CLAIM + counter increment" + "3. Recipient fan-out" paragraphs immediately following the new STEP A/B intro, plus residual SQL-comment STEP 2/3/5 references and lease/heartbeat/fencing-section STEP 2-5 references; same-class duplicate-ordering ambiguity that R61 was supposed to close. Fix: (a) folded legacy "2." block into a STEP A RESTATED note + reframed R55 escalation_key paragraph as a normative invariant "applies across STEP A through STEP F"; (b) folded legacy "3." block into a STEP C RESTATED note explicitly ordering "AFTER STEP B and BEFORE STEP D"; (c) renamed all in-SQL-comment STEP 2 → STEP A, STEP 3 → STEP C, STEP 5 → STEP E in Sub-decision 6 sweep block; (d) renamed all "STEP 2-5" → "STEP A→F" in lease/heartbeat/fencing section; (e) renamed all Sub-decision 5 + Sub-decision 6 + §7 Open Questions normative cross-references to Sub-decision 6 step labels (STEP 1 → STEP B, STEP 3 → STEP C, STEP 5 → STEP E); (f) §12 closure-log historical STEP-N references explicitly marked non-normative via codification note above. Sub-decision 2's STEP 1-5 ordering is the separate normative algorithm for the synchronous emission flow and is preserved (unrelated to Sub-decision 6's STEP A-F sweep ordering).

**v0.62 DRAFT 2026-05-21 — R61 closure applied (1 HIGH):**
- **R61 HIGH-1 closed:** R60 closure did not actually unify the sweep ordering — the spec still contained an older numbered STEP A→F path that incremented sweep_cycle_counter before the newly introduced STEP A/B sequence, leaving implementers with two contradictory execution orders for the same safety-critical sweep transaction. Fix: collapsed the legacy "The atomic STEP A→F sweep transaction" numbered 1./2./3. block into a single canonical "Atomic sweep transaction — single canonical ordering" section listing STEP A→F explicitly: STEP A = R53 atomic eligibility-revalidation + sweep_cycle_counter UPDATE-RETURNING (first mutating statement; sole sweep_cycle_counter mutation site), STEP B = Lifecycle transition INSERT (runs after STEP A ROW_COUNT=1 proves authoritative-winner), STEP C = recipient fan-out + EXISTENCE invariant, STEP D = Cat A audit emit, STEP E = Tier ADVANCE + deadline reschedule UPDATE on escalation_obligation, STEP F = Final guarded UPDATE on crisis_sweep_execution (completed_at + sweep_cycle_id_committed atomic with fencing_token + claim_expires_at predicates). sweep_cycle_counter is incremented EXACTLY ONCE at STEP A; no duplicate increment elsewhere.

**v0.61 DRAFT 2026-05-21 — R60 closures applied (2 HIGH):**
- **R60 HIGH-1 closed:** replaced raw ROW_COUNT validation with EXISTENCE invariant (SELECT COUNT(*) matching mapping cardinality, counting newly-inserted + pre-existing idempotent rows); ROLLBACK only when existence-count is short.
- **R60 HIGH-2 closed:** renumbered per-row steps — R53 guarded UPDATE is now STEP A (first mutating statement); Lifecycle transition INSERT is STEP B (runs after STEP A ROW_COUNT=1).

**v0.60 DRAFT 2026-05-21 — R59 closure applied (1 HIGH):**
- **R59 HIGH-1 closed:** removed "ROW_COUNT=0 → safe to no-op tier UPDATE" loophole; retry detection now bound to pre-STEP-2 worker entry guard against crisis_sweep_execution.completed_at + sweep_cycle_id_committed; inside STEP A→F, ROW_COUNT < expected ALWAYS triggers ROLLBACK + Cat C audit + retry.

**v0.59 DRAFT 2026-05-21 — R58 closure applied (1 HIGH):**
- **R58 HIGH-1 closed:** STEP 5 heading "release the per-tier-cycle lock" terminology removed; renamed to "Tier ADVANCE + deadline reschedule"; explicit no-per-tier-cycle-lock note added.

**v0.58 DRAFT 2026-05-21 — R57 closure applied (1 HIGH):**
- **R57 HIGH-1 closed:** removed 3 residual claim-lock references — (1) R53 SQL comment; (2) STEP 5 escalation_key=NULL prose; (3) "Difference from one-shot model" paragraph. All now consistent with R55-R56 always-NULL invariant; escalation_key is legacy P-027 column preserved at NULL.

**v0.57 DRAFT 2026-05-21 — R56 closures applied (2 HIGH):**
- **R56 HIGH-1 closed:** rewrote "Idempotency within a tier cycle" paragraph — in-flight exclusion via R53 row lock + crisis_sweep_execution generation + sweep_cycle_counter; escalation_key plays NO role.
- **R56 HIGH-2 closed:** rewrote tests #9 + #10 to assert sweep_cycle_counter increment + sweep_cycle_id matching + explicit escalation_key NULL assertions throughout sweep lifecycle.

**v0.56 DRAFT 2026-05-21 — R55 closure applied (1 HIGH):**
- **R55 HIGH-1 closed:** escalation_key semantic unified — ALWAYS NULL between sweep txs; STEP 5 does NOT set it non-NULL; per-tier-cycle marker provided by sweep_cycle_counter + provider_attempt.sweep_cycle_id instead.

**v0.55 DRAFT 2026-05-21 — R54 closure applied (1 HIGH):**
- **R54 HIGH-1 closed:** removed legacy STEP 2 separate "Tier CLAIM" UPDATE that set escalation_key before the R53 guarded UPDATE; the R53 atomic UPDATE is now the SOLE eligibility-revalidation primitive. PostgreSQL row-level write lock provides in-flight exclusion. escalation_key repurposed as STEP 5 tier-cycle marker.

**v0.54 DRAFT 2026-05-21 — R53 closure applied (1 HIGH):**
- **R53 HIGH-1 closed:** collapsed SELECT-then-UPDATE eligibility revalidation into single guarded UPDATE with RETURNING for sweep_cycle_counter increment. Atomic; eliminates race window.

**v0.53 DRAFT 2026-05-21 — R52 closure applied (1 HIGH):**
- **R52 HIGH-1 closed:** added scheduled_for_obligation_generation column on crisis_sweep_execution + partial UNIQUE constraint `WHERE completed_at IS NULL`; STEP 2 obligation-eligibility revalidation pre-sweep; stale executions marked completed-as-stale-no-op; new crisis.sweep_stale_eligibility_dropped Cat C audit (action 12; total 12 = 8A + 4C).

**v0.52 DRAFT 2026-05-21 — R51 closure applied (1 HIGH):**
- **R51 HIGH-1 closed:** added `AND claim_expires_at > now()` to STEP 5 final guarded UPDATE; triple-guard (claimed_by_worker_id + fencing_token + claim_expires_at) makes takeover and old-token finalization mutually exclusive.

**v0.51 DRAFT 2026-05-21 — R50 closure applied (1 MED):**
- **R50 MED-1 closed:** §3 "Total: 10 new action IDs" line was stale; updated to "Total: 11 new action IDs (8 Cat A + 0 Cat B + 3 Cat C)" consistent with §1 + §3 header + R49 closure log.

**v0.50 DRAFT 2026-05-21 — R49 closure applied (1 HIGH):**
- **R49 HIGH-1 closed:** delivery-worker pre-dispatch recheck now structurally includes `AND fencing_token = $outbox.expected_fencing_token`; mismatch drops outbox row + emits new crisis.delivery_fence_mismatch_dropped Cat C audit (action 11; total 11 new = 8A + 3C).

**v0.49 DRAFT 2026-05-21 — R48 closure applied (1 HIGH):**
- **R48 HIGH-1 closed:** STEP 3 is transactionally enqueue-only. Provider notifications happen in downstream delivery worker reading outbox AFTER completed_at + sweep_cycle_id_committed are durable. Delivery worker rechecks (sweep_execution_id, sweep_cycle_id_committed) before dispatch; rolled-back/taken-over state drops outbox row + suppresses delivery. No duplicate external sends across takeover.

**v0.48 DRAFT 2026-05-21 — R47 closure applied (1 HIGH):**
- **R47 HIGH-1 closed:** sweep_cycle_id_committed write moved from STEP 2 to STEP 5 final guarded UPDATE (committed atomically with completed_at + fencing_token predicate). The "impossible-but-bounded" pre-commit-marker case is now architecturally impossible; tripwire audit retained for database integrity defects only.

**v0.47 DRAFT 2026-05-21 — R46 closures applied (2 HIGH):**
- **R46 HIGH-1 closed:** added final guarded compare-and-set UPDATE at STEP 5 with fencing_token predicate; trapped-invariant model (opening + closing guards).
- **R46 HIGH-2 closed:** removed lingering SELECT FOR UPDATE wording; STEP 2 uses non-locking guard SELECT.

**v0.46 DRAFT 2026-05-21 — R45 closures applied (1 HIGH + 1 MED):**
- **R45 HIGH-1 closed:** claim + heartbeat moved to separate autocommit txs; added fencing_token column; STEP A→F begins with fencing-token guard for takeover safety; no row-lock contention.
- **R45 MED-1 closed:** registered crisis.sweep_replay_after_commit_ack_loss + crisis.sweep_claim_recovery_with_committed_cycle as Cat C entries 9 + 10; updated tally 8 → 10 new actions (8A + 2C).

**v0.45 DRAFT 2026-05-21 — R44 closure applied (1 HIGH):**
- **R44 HIGH-1 closed:** added claim_expires_at lease column to crisis_sweep_execution (60s default); claim SQL allows takeover of expired claims; worker heartbeat renewal at 20s intervals during fan-out; defense-in-depth takeover semantics with crisis.sweep_claim_recovery_with_committed_cycle Cat C audit for impossible-but-bounded edge case.

**v0.44 DRAFT 2026-05-21 — R43 closure applied (1 HIGH):**
- **R43 HIGH-1 closed:** introduced crisis_sweep_execution durable per-sweep table (sweep_execution_id + claimed_by_worker_id + completed_at + sweep_cycle_id_committed). Sweep STEP 2 SELECT FOR UPDATE binds tx to specific execution_id; STEP 5 UPDATEs completed_at = now(). Replay logic: if completed_at IS NOT NULL on a re-execution attempt, emit crisis.sweep_replay_after_commit_ack_loss Cat C audit + skip re-execution + return success to scheduler. Committed sweeps are permanently terminal; replays cannot duplicate fan-out/audit/tier-advance.
...
- **R42 HIGH-2 closed:** removed half-committed sweep retry paragraph; clarified SOLE normative recovery contract (STEP A→F commit-or-rollback together; post-commit handled by outbox-worker dead-letter).
...
- **R41 MED-1 closed:** transaction model clarified — STEP A→F single atomic transaction; partial failure rolls back increment too; no "half-committed" sweep state in normative model; post-commit failures handled by outbox-worker dead-letter.
