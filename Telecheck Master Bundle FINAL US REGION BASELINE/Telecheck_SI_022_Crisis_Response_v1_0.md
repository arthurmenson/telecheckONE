...
1. **Lifecycle transition INSERT:** crisis_event_lifecycle_transition. The from_state is the current to_state derived from the latest crisis_event_lifecycle_transition row. The to_state is `escalated`. **The transition reason is a four-way mapping by from_state (R8 HIGH-1 + R11 HIGH-1 + R12 HIGH-2 closure 2026-05-21; matches the 4 valid escalated-to triples in the 11-triple §6 enumeration):**
    ...
    ```
2. **Tier CLAIM + counter increment (R53 HIGH-1 + R54 HIGH-1 closure 2026-05-21: SUPERSEDED — escalation_key is no longer set by a separate UPDATE before the R53 guarded UPDATE; the R53 single guarded UPDATE described below is the SOLE eligibility-revalidation-and-increment primitive in STEP 2. The historical "Tier CLAIM" UPDATE setting escalation_key = gen_random_uuid() before the increment is REMOVED — it directly conflicted with the R53 UPDATE's `obligation.escalation_key IS NULL` predicate and would have caused every legitimate sweep to be dropped as stale-no-op). The R53 single guarded UPDATE atomically (a) revalidates ALL eligibility predicates (deadline + escalation_key IS NULL + escalation_tier IS NOT NULL + lifecycle current_state + sweep_cycle_counter+1 = scheduled_for_obligation_generation) AND (b) increments sweep_cycle_counter via RETURNING. In-flight per-tier exclusion is enforced by the SINGLE row-level write lock that PostgreSQL takes on the obligation row for the duration of that UPDATE statement; concurrent sweep workers either win the lock + see ROW_COUNT=1 OR lose the lock + see ROW_COUNT=0 (dropped as stale). The escalation_key field is REPURPOSED at the END of STEP 5 as a per-tier-cycle marker (UUID set during STEP 5's tier-advance UPDATE alongside the deadline-reset/tier-advance writes; reset to NULL by acknowledge/respond/resolve wrappers — preserving the pre-R52 semantic that escalation_key signals "this tier's escalation has been emitted" for downstream readers). R53 atomic UPDATE handles claim invariance + counter increment; STEP 5 handles tier-marker bookkeeping.
