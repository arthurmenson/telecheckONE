...
16g. **Final-tier exhaustion stays sweep-visible until resolved (R13 HIGH-1 closure 2026-05-21):** simulate a life-threatening crisis through all 3 tiers (care_team → clinical_on_call → regulatory) without acknowledgement/response/resolution. After the third sweep, escalation_tier MUST remain at 'regulatory' (NOT NULL); final_tier_exhausted_at MUST be set to the timestamp of that third sweep; one-time `crisis.final_tier_reached` Cat A audit MUST be emitted at the third sweep. After INTERVAL_final_tier_recheck_window elapses, sweep MUST fire AGAIN on the same row, emitting `crisis.no_acknowledgement_escalation` Cat A with payload `is_final_tier_recheck=true`. Only `record_crisis_resolution()` setting escalation_tier=NULL terminalizes the row.
