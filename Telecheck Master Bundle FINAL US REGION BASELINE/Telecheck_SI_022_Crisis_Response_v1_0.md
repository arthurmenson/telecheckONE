...
- **§4.67 `notification_crisis_provider_attempt`** — per-recipient ATTEMPT (one row per (recipient_role, recipient_address, channel) tuple). The child table is where multi-recipient fan-out is represented; multiple recipients on the same channel (e.g., care-team SMS + clinical-on-call SMS + emergency-contact SMS) are multiple provider_attempt rows under the same dispatch_ledger SMS row.
  ...
  **R27 HIGH-1 + R28 HIGH-1 closure 2026-05-21 (this SI extends P-027 §4.67 row shape):** the provider_attempt entity is amended at this SI's CDM follow-on amendment (P-040 target) to add:
   ...
   - canonical UNIQUE constraint `notification_crisis_provider_attempt_idempotency_uk` ON `(tenant_id, crisis_event_id, channel, recipient_role, recipient_address, attempt_sequence)` — exact ON CONFLICT target name used by both STEP 4b initial fan-out + Sub-decision 6 STEP 3 sweep fan-out
   - **Every provider_attempt INSERT in this SI's normative SQL MUST populate crisis_event_id non-null AND use `ON CONFLICT ON CONSTRAINT notification_crisis_provider_attempt_idempotency_uk DO NOTHING`.** R29 HIGH-2 + R30 HIGH-2 closure 2026-05-21: ON CONFLICT is for **worker-retry idempotency only** (e.g., outbox worker re-running the same logical transaction emits the same `(crisis_event_id, channel, recipient_role, recipient_address, attempt_sequence)` tuple twice and the second insert is correctly no-op'd). **Sweep tier-advance + final-tier recheck inserts MUST compute per-recipient `attempt_sequence = MAX(prior attempt_sequence for this recipient on this crisis_event) + 1`** so an already-dispatched recipient receives a NEW row at every tier transition / recheck (intentional re-notification). The ON CONFLICT DO NOTHING NEVER suppresses an escalation-time re-notification; it only suppresses true retry duplicates.
...
INSERT INTO notification_crisis_provider_attempt
    (tenant_id, patient_id, server_signal_id, crisis_event_id, channel, recipient_role, recipient_address, attempt_sequence)
...
-- Initial dispatch attempt_sequence=1. The ON CONFLICT clause only fires on worker-retry
-- idempotency for the SAME initial-dispatch transaction; sweep tier-advance uses INCREMENTED
-- attempt_sequence via MAX+1 per-recipient lookup (R29 HIGH-2 + R30 HIGH-2 closure) so
-- escalation-time recipients are ALWAYS re-notified.
--
...
-- provider_attempt rows for THIS crisis_event. Branches on target_tier:
--   target_tier = 'care_team' → returns care-team-member tuples on configured channels
...
|---|---|---|
| `record_crisis_acknowledgement_claim()` (no caller-supplied tier; tier derived internally — R35 HIGH-1 closure 2026-05-21) | UPDATE obligation SET undeliverable_deadline = now() + INTERVAL_for_severity_response_window(severity); escalation_tier = GREATEST(escalation_tier, v_derived_tier) where v_derived_tier is **derived inside the SECURITY DEFINER wrapper from the JWT-bound principal**: lookup the principal's recipient_role from the most recent successful `notification_crisis_provider_attempt` row matching (tenant_id, crisis_event_id, recipient_address ∈ {principal.address, JWT-verified email/phone of principal}) — recipient_role drives the tier (care_team → 'care_team'; clinical_on_call → 'clinical_on_call'; operator_escalation → 'regulatory'). If no eligible provider_attempt row exists for this principal, the wrapper RAISES `tier_ownership_unauthorized` (the principal was never dispatched a notification for this crisis_event, so they cannot acknowledge from any tier). escalation_key reset to NULL | Acknowledgement is NOT safety-terminal — the patient is still in crisis until responded/resolved. **R35 HIGH-1 closure: the acknowledging_tier is NEVER caller-supplied — making it a wrapper input would let a lower-tier responder self-assert 'regulatory' and skip clinical_on_call escalation pressure. Tier is derived authoritatively from the JWT-verified principal's actual recipient_role in the canonical notification_crisis_provider_attempt row.** Wrapper is a SECURITY DEFINER procedure; the JWT-bound principal is the authoritative tier source. |
| `record_crisis_response()` (no caller-supplied tier; tier derived internally — R35 HIGH-1 closure) | Same JWT-derived-tier discipline as `record_crisis_acknowledgement_claim()`: v_derived_tier from principal's recipient_role in canonical provider_attempt row; RAISE tier_ownership_unauthorized if no eligible row; UPDATE obligation SET undeliverable_deadline = now() + INTERVAL_for_severity_resolution_window(severity), escalation_tier = GREATEST(escalation_tier, v_derived_tier), escalation_key = NULL | Response is NOT safety-terminal — clinician has taken action but resolution outcome is still pending. Same JWT-derived-tier authorization as acknowledgement wrapper. |
...
3. **Recipient fan-out (with R27 HIGH-1 + R29 HIGH-2 closure 2026-05-21 idempotency contract):** call `compute_crisis_recipient_mapping(crisis_event_id, severity, target_tier)` to derive recipients; INSERT provider_attempt rows USING `ON CONFLICT ON CONSTRAINT notification_crisis_provider_attempt_idempotency_uk DO NOTHING` against the canonical idempotency key UNIQUE(tenant_id, crisis_event_id, channel, recipient_role, recipient_address, attempt_sequence).

**R29 HIGH-2 closure: attempt_sequence INCREMENTS on EVERY tier transition + every final-tier recheck (not just final-tier rechecks).** This guarantees escalation-time re-notification of recipients already dispatched at initial detection — a recipient who got attempt_sequence=1 at STEP 4b initial dispatch gets attempt_sequence=2 at first sweep tier-advance, attempt_sequence=3 at second sweep tier-advance, attempt_sequence=N at every subsequent recheck sweep. New escalation-time fan-out is NEVER silently no-op'd by the idempotency contract; the ON CONFLICT clause only fires on worker-retry idempotency for the SAME (recipient, attempt_sequence) tuple within a single sweep transaction.

Implementation: `v_attempt_sequence := COALESCE((SELECT MAX(attempt_sequence) FROM notification_crisis_provider_attempt WHERE tenant_id=v_tenant_id AND crisis_event_id=v_crisis_event_id AND channel=mapping.channel AND recipient_role=mapping.recipient_role AND recipient_address=mapping.recipient_address), 0) + 1`. This MAX+1 lookup runs per-recipient at every sweep tier-advance AND final-tier recheck (uniform contract; no special-case branch). Net-new recipients introduced at this tier transition (e.g., a regulatory operator_escalation recipient appearing for the first time at sweep #3) get attempt_sequence=1 via COALESCE(NULL, 0)+1 = 1. **`target_tier` is explicitly defined to handle both pre-exhaustion and final-tier exhaustion/recheck paths (R16 HIGH-1 closure 2026-05-21):**
...
16l. **Tier-ownership authorization on acknowledge/respond wrappers (R35 HIGH-1 closure 2026-05-21):** simulate a care-team-tier responder attempting to call `record_crisis_acknowledgement_claim()` for a crisis_event where the responder is in the care_team but NOT in clinical_on_call or operator_escalation provider_attempt rows. The wrapper MUST derive v_derived_tier='care_team' from the responder's JWT-bound principal looking up notification_crisis_provider_attempt; escalation_tier MUST be set to GREATEST(current, 'care_team') — NOT advanced to 'clinical_on_call' or 'regulatory'. Negative test variant: simulate a responder NOT in any provider_attempt row for this crisis_event calling acknowledge; wrapper MUST RAISE `tier_ownership_unauthorized`. Positive test variant: simulate a clinical_on_call-tier responder (in clinical_on_call provider_attempt row) calling acknowledge; wrapper MUST derive v_derived_tier='clinical_on_call' and advance escalation_tier accordingly. caller-supplied tier parameter is REJECTED (function signature has no tier parameter).
...
escalation_tier remains 'care_team' (initial value) — initial multi-tier dispatch does NOT skip care_team_no_acknowledgement_timeout deadline. First sweep at care_team deadline expiry uses the SEPARATE `compute_crisis_recipient_mapping(..., target_tier='clinical_on_call')` to compute the clinical_on_call recipient set. **Per R29 HIGH-2 + R30 HIGH-2 + R34 HIGH-1 closure: the first sweep MUST INSERT new provider_attempt rows with `attempt_sequence = MAX(prior) + 1` for EVERY clinical_on_call recipient (including recipients already dispatched at initial detection time at attempt_sequence=1) — escalation-time re-notification is GUARANTEED. ON CONFLICT DO NOTHING is for worker-retry idempotency on the SAME (recipient, attempt_sequence) tuple only; it does NOT suppress escalation-time re-notification of already-dispatched recipients.**
