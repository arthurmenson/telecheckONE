...
|---|---|---|
| `record_crisis_acknowledgement_claim(acknowledging_tier)` | UPDATE obligation SET undeliverable_deadline = now() + INTERVAL_for_severity_response_window(severity); **(R34 HIGH-2 closure 2026-05-21)** escalation_tier = GREATEST(escalation_tier, acknowledging_tier) — when a higher-tier recipient (clinical_on_call or operator_escalation) initially dispatched at R26 multi-tier detection-time acknowledges from their tier directly, escalation_tier advances to that tier so subsequent no-response/no-resolution timeouts advance from the actual responsible tier (NOT stale care_team); escalation_key reset to NULL | Acknowledgement is NOT safety-terminal — the patient is still in crisis until responded/resolved. A post-acknowledgement response-window timer enforces clinician response within bounded time. Wrapper REQUIRED to accept `acknowledging_tier` ∈ {'care_team', 'clinical_on_call', 'regulatory'} per the recipient_role of the acknowledging principal; tier-ownership rule prevents the post-ack/post-respond timer from advancing from a stale tier. |
| `record_crisis_response(responding_tier)` | UPDATE obligation SET undeliverable_deadline = now() + INTERVAL_for_severity_resolution_window(severity); **(R34 HIGH-2 closure)** escalation_tier = GREATEST(escalation_tier, responding_tier); escalation_key reset to NULL | Response is NOT safety-terminal — clinician has taken action but resolution outcome is still pending. Post-response resolution-window timer enforces resolution within bounded time. `responding_tier` required for same tier-ownership rule as acknowledgement wrapper. |
