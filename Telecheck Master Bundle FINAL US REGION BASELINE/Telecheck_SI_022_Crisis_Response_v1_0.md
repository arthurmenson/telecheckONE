...
### Sub-decision 6 — No-acknowledgement escalation timer (R7 HIGH-1 closure 2026-05-21: tier-cycle resettable model is now the authoritative normative timer contract)
...
FROM notification_crisis_escalation_obligation
WHERE now() > undeliverable_deadline
  AND escalation_key IS NULL       -- per-tier-cycle CLAIM lock (NULL = currently pending)
  AND escalation_tier IS NOT NULL  -- terminal-tier exclusion (NULL = no further escalation)
FOR UPDATE SKIP LOCKED;
```

For each eligible row, the job runs the following atomic per-row transaction:

1. **Lifecycle transition INSERT:** crisis_event_lifecycle_transition (from current to_state derived from crisis_event_lifecycle_transition latest row → `escalated`, reason='no_acknowledgement_timeout').
...
**Decision:** 16 merge-blocking integration tests + 3 static-analyzer rules (mirrors P-035 Mode 1 + P-037 SI-020 pattern; R6 MED-1 closure 2026-05-21 added test #16 contention benchmark to the normative list):
...
Authored on `spec/SI-022-crisis-response-slice-2026-05-21` branch off main at `fab0615` (post-P-038 merge). v0.1 commit `e7a7ebb`. v0.2 commit `c2b9e15`. v0.3 commit `f4001d2`. v0.4 commit `6ec0cc9`. v0.5 commit `18ef338`. v0.6 commit `595db3c`. v0.7 commit `0e287b5`. v0.8 commit pending push for R8 verification.
