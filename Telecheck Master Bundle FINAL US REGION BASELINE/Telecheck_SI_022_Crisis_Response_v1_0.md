...
- COMMITTED but completed_at not set (impossible per atomic-tx model; defense-in-depth): second worker observes completed_at IS NULL + sweep_cycle_id_committed IS NOT NULL → emits `crisis.sweep_claim_recovery_with_committed_cycle` Cat C audit + marks completed_at = now() (heal terminal mark).
...
1. STEP 2 (R46 HIGH-2 closure 2026-05-21: NO SELECT FOR UPDATE on crisis_sweep_execution — that would re-introduce the R45 HIGH-1 lock-contention with autocommit heartbeat updates. Instead): a NON-LOCKING fencing-token guard SELECT against crisis_sweep_execution validates the caller still holds the claim (claimed_by_worker_id = $worker_id AND fencing_token = $captured_fencing_token AND completed_at IS NULL); raises sweep_fencing_token_mismatch + rolls back if zero rows; **the row is NOT locked for the duration of STEP 2-5**. Then: increment escalation_obligation.sweep_cycle_counter via UPDATE-RETURNING (this is the only row this tx locks for the full duration); update the crisis_sweep_execution row's sweep_cycle_id_committed column to v_sweep_cycle_id (autocommit-friendly; one UPDATE, no held lock past commit).
2. STEP 3-5: INSERT provider_attempt rows (with sweep_cycle_id=v_sweep_cycle_id); Cat A audit; tier-advance UPDATE.
3. STEP 5 (final; R46 HIGH-1 closure 2026-05-21: **final guarded compare-and-set** before COMMIT to prevent a paused/lagging worker from committing after another worker has taken over):
```sql
UPDATE crisis_sweep_execution
