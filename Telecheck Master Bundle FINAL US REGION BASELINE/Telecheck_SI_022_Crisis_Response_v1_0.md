...
**10 new action IDs** under `crisis.*` namespace EXTENDING P-025 (which contributed `crisis.escalation_destination_resolved` Cat B): **8 Cat A + 0 Cat B + 2 Cat C** (R13 added crisis.final_tier_reached Cat A; R1 moved no_acknowledgement_escalation Cat B → Cat A; R43+R45 added the 2 sweep recovery Cat C events):
