...
The atomic STEP 2-5 sweep transaction (now executed after non-locking claim + with fencing-token guard at tx start):

1. STEP 2 (R46 HIGH-2 + R47 HIGH-1 closure 2026-05-21: NO SELECT FOR UPDATE on crisis_sweep_execution; sweep_cycle_id_committed is NOT written here — it is written ONLY by the final guarded STEP 5 compare-and-set so the durable replay marker is unambiguous post-commit): a NON-LOCKING fencing-token guard SELECT against crisis_sweep_execution validates the caller still holds the claim (claimed_by_worker_id = $worker_id AND fencing_token = $captured_fencing_token AND completed_at IS NULL); raises sweep_fencing_token_mismatch + rolls back if zero rows; **the row is NOT locked for the duration of STEP 2-5**. Then: increment escalation_obligation.sweep_cycle_counter via UPDATE-RETURNING into v_sweep_cycle_id (this is the only row this tx locks for the full duration). **sweep_cycle_id_committed on crisis_sweep_execution is NOT written here** — it is set atomically by the closing guarded UPDATE in STEP 5 along with completed_at.
2. STEP 3-5: INSERT provider_attempt rows (with sweep_cycle_id=v_sweep_cycle_id); Cat A audit; tier-advance UPDATE.
3. STEP 5 (final; R46 HIGH-1 + R47 HIGH-1 + R51 HIGH-1 closure 2026-05-21: **final guarded compare-and-set sets BOTH completed_at AND sweep_cycle_id_committed atomically AND requires the lease to be still un-expired** so an expired-lease worker cannot commit even if its fencing_token captured value coincidentally still equals the current row value via some race):
```sql
UPDATE crisis_sweep_execution
