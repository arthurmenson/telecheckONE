...
-- (all recipient_role variants). **R59 HIGH-1 closure 2026-05-21: ROW_COUNT=0 is NOT a
-- blanket signal to skip STEP 5; retry/no-op detection is bound to crisis_sweep_execution.completed_at
-- + sweep_cycle_id_committed BEFORE entering STEP 2-5 (the pre-tx retry-detection guard at the
-- worker entry point). Inside the atomic STEP 2-5 tx, ROW_COUNT < expected on STEP 3 always
-- triggers ROLLBACK + emit crisis.dispatch_attempt_failed Cat C + retry — never a tier-update
-- skip. If the worker entered STEP 2-5 at all, the R53 guarded UPDATE already verified the
-- sweep is the authoritative winner for this generation, so STEP 5 MUST run to completion
-- (deadline reschedule, tier advance, sweep_cycle_id_committed, completed_at).** If fewer
-- rows than expected INSERTed, ROLLBACK + emit crisis.dispatch_attempt_failed Cat C + retry
