...
1. STEP 2: SELECT FOR UPDATE on the crisis_sweep_execution row (binds the transaction to a specific sweep_execution_id); increment escalation_obligation.sweep_cycle_counter RETURNING v_sweep_cycle_id; UPDATE the crisis_sweep_execution row's sweep_cycle_id_committed column to v_sweep_cycle_id.
2. STEP 3-5: INSERT provider_attempt rows (with sweep_cycle_id=v_sweep_cycle_id); Cat A audit; tier-advance UPDATE.
3. STEP 5 (final): UPDATE crisis_sweep_execution SET completed_at = now() (terminal mark; the row becomes idempotent).
