...
-- R28 HIGH-1 closure 2026-05-21: crisis_event_id is non-null on every provider_attempt INSERT;
-- ON CONFLICT target is the canonical constraint name; initial dispatch at STEP 4b + sweep
-- tier-advance at Sub-decision 6 STEP 3 use the SAME constraint target so overlapping recipient
-- inserts (same attempt_sequence=1) are silently no-op'd. Final-tier recheck cycles compute
-- v_attempt_sequence = MAX+1 to enable intentional re-notification on terminal-risk state.
...
3. **Recipient fan-out (with R27 HIGH-1 closure 2026-05-21 idempotency contract):** call `compute_crisis_recipient_mapping(crisis_event_id, severity, target_tier)` to derive recipients; **INSERT provider_attempt rows USING ON CONFLICT DO NOTHING against the canonical idempotency key UNIQUE(tenant_id, crisis_event_id, channel, recipient_role, recipient_address, attempt_sequence)**. The recipient set for a given target_tier may overlap with recipients already dispatched at STEP 4b initial fan-out (R26 immediate high-urgency dispatch). The uniqueness key ensures: (a) recipients already attempted with attempt_sequence=1 at initial dispatch are NOT re-inserted by the first sweep (silent no-op via ON CONFLICT) — no duplicate notifications, no transaction rollback; (b) intentional final-tier recheck cycles (R13 closure) increment attempt_sequence so the recipient IS re-notified — implemented by computing `v_attempt_sequence := COALESCE((SELECT MAX(attempt_sequence) FROM notification_crisis_provider_attempt WHERE tenant_id=v_tenant_id AND crisis_event_id=v_crisis_event_id AND channel=mapping.channel AND recipient_role=mapping.recipient_role AND recipient_address=mapping.recipient_address), 0) + 1` IFF this sweep is a final-tier recheck (final_tier_exhausted_at IS NOT NULL); else attempt_sequence=1 for first-time recipient additions on tier advance. **`target_tier` is explicitly defined to handle both pre-exhaustion and final-tier exhaustion/recheck paths (R16 HIGH-1 closure 2026-05-21):**
