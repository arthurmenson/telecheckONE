...
```sql
-- R39 HIGH-1 closure 2026-05-21: extended notification_crisis_escalation_obligation with
-- `sweep_cycle_counter INT NOT NULL DEFAULT 0` column (P-040 CDM follow-on). On each sweep
-- transaction, STEP 2 (BEFORE recipient fan-out) atomically increments this counter:
...
-- v_sweep_cycle_id is a deterministic per-sweep-transaction integer. Retries within the same
-- logical sweep transaction observe the same value (because the increment is in the same
-- transaction). New sweeps get the next value.
...
-- @@ROW_COUNT or equivalent matches the count of mapping rows with non-null
-- recipient_principal_id). If zero rows INSERTed because all already existed at the
...
This INSERT is the SOLE normative sweep fan-out spec. The deterministic v_sweep_cycle_id replaces non-deterministic MAX+1 recomputation: same-logical-sweep retries correctly hit ON CONFLICT (no duplicate notifications); new sweeps always succeed (escalation-time re-notification guaranteed); a half-committed sweep that left provider_attempt rows but failed before STEP 5 produces a retry that sees the same sweep_cycle_id, no-ops the inserts, and re-executes STEP 5 (idempotent tier-advance). The idempotency key UNIQUE on (tenant_id, crisis_event_id, channel, recipient_role, recipient_address, attempt_sequence) is preserved; attempt_sequence is now equal to sweep_cycle_id for sweep-driven INSERTs (initial STEP 4b INSERTs use attempt_sequence=1 + sweep_cycle_id=1 from the initial increment in STEP 2 of the detection transaction).
