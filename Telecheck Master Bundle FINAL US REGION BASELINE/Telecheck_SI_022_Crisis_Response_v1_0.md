...
**R41 MED-1 transaction-model clarification:** the sweep STEP 2 sweep_cycle_counter increment + STEP 3 INSERT + STEP 4 Cat A audit + STEP 5 tier-advance UPDATE are ALL in a single atomic transaction. On partial failure before commit, the entire transaction rolls back (including the increment) — no provider_attempt rows are visible to a retry. A retry then re-increments and creates a NEW v_sweep_cycle_id (no overlap with the rolled-back attempt). Post-commit failures of the outbox worker (e.g., transient connection loss after COMMIT) are handled by the canonical outbox-worker dead-letter SLO — the worker's at-least-once delivery semantics + outbox-row idempotency ensure no escalation-time row is lost. **There is no "half-committed" sweep state in the normative model** — either all of STEP 2-5 commit together or all roll back together.
