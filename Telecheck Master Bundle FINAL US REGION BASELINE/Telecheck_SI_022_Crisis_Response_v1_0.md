...
- **§4.67 `notification_crisis_provider_attempt`** — per-recipient ATTEMPT (one row per (recipient_role, recipient_address, channel) tuple). The child table is where multi-recipient fan-out is represented; multiple recipients on the same channel (e.g., care-team SMS + clinical-on-call SMS + emergency-contact SMS) are multiple provider_attempt rows under the same dispatch_ledger SMS row.
  ...
  **R27 HIGH-1 + R28 HIGH-1 closure 2026-05-21 (this SI extends P-027 §4.67 row shape):** the provider_attempt entity is amended at this SI's CDM follow-on amendment (P-040 target) to add:
   ...
   - **Every provider_attempt INSERT in this SI's normative SQL MUST populate crisis_event_id non-null AND use `ON CONFLICT ON CONSTRAINT notification_crisis_provider_attempt_idempotency_uk DO NOTHING`** — both initial dispatch at STEP 4b AND sweep tier-advance at Sub-decision 6 STEP 3 use the same constraint target so initial-dispatch rows (attempt_sequence=1) collide with first-sweep tier-advance attempts on the same recipient and are silently no-op'd. Final-tier rechecks compute attempt_sequence=MAX+1 so the same recipient gets a NEW row (intentional re-notification).
...
-- R28 HIGH-1 + R29 HIGH-1 closure 2026-05-21: crisis_event_id non-null + correct SQL ordering
-- (FROM ... before ON CONFLICT; PostgreSQL INSERT ... SELECT ... FROM ... ON CONFLICT ... DO ...
-- syntax). Initial dispatch attempt_sequence=1. The ON CONFLICT clause only fires on worker-retry
-- idempotency for the SAME initial-dispatch transaction; sweep tier-advance uses INCREMENTED
-- attempt_sequence (R29 HIGH-2 closure) so escalation-time recipients are ALWAYS re-notified.
FROM compute_crisis_initial_recipient_mapping(v_crisis_event_id, v_severity, v_regulatory_reporting) AS mapping;
