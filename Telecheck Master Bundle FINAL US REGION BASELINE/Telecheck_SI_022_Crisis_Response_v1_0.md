...
**STEP B (formerly STEP 1; now renumbered to run AFTER R53 success):** Lifecycle transition INSERT — crisis_event_lifecycle_transition. The from_state is current_state read AFTER STEP A's lock acquisition. The to_state is `escalated`. **The transition reason is a four-way mapping by from_state (R8 HIGH-1 + R11 HIGH-1 + R12 HIGH-2 closure 2026-05-21; matches the 4 valid escalated-to triples in the 11-triple §6 enumeration):**
    ...
    ```
2. **Tier CLAIM + counter increment (R53 HIGH-1 + R54 HIGH-1 + R55 HIGH-1 closure 2026-05-21: the R53 single guarded UPDATE described below is the SOLE eligibility-revalidation-and-increment primitive in STEP 2; the historical separate "Tier CLAIM" UPDATE setting escalation_key=gen_random_uuid() is REMOVED).** The R53 single guarded UPDATE atomically (a) revalidates ALL eligibility predicates (deadline + escalation_key IS NULL + escalation_tier IS NOT NULL + lifecycle current_state + sweep_cycle_counter+1 = scheduled_for_obligation_generation) AND (b) increments sweep_cycle_counter via RETURNING. In-flight per-tier exclusion is enforced by the SINGLE row-level write lock that PostgreSQL takes on the obligation row for the duration of that UPDATE statement; concurrent sweep workers either win the lock + see ROW_COUNT=1 OR lose the lock + see ROW_COUNT=0 (dropped as stale).

**R55 HIGH-1 closure 2026-05-21 — escalation_key semantic unified:** `escalation_key` is **always NULL** between sweep transactions (i.e., post-commit of every STEP 2-5 transaction). STEP 5's tier-advance UPDATE EXPLICITLY does NOT set escalation_key to non-NULL — it remains NULL so the next scheduled deadline expiry passes the R53 eligibility predicate `obligation.escalation_key IS NULL`. The pre-R52 "per-tier-cycle marker" semantic is now provided by **sweep_cycle_counter** itself (durable monotonic value) + **provider_attempt rows with the matching sweep_cycle_id** (durable evidence of recipient fan-out per tier). escalation_key is preserved in the schema for backward compatibility with P-027 §4.68 but is now permanently NULL in the canonical SI-022 model; it's not used as a marker. acknowledge/respond/resolve wrappers don't touch escalation_key. **Implementer: do NOT set escalation_key to non-NULL anywhere in the canonical sweep flow.** R53 eligibility predicate, STEP 5 UPDATE, idempotency contract, and tests #9/#10 are all consistent with this single-state invariant: escalation_key IS NULL ⟺ sweep eligible (combined with other predicates).
...
-- idempotent rows.** After the STEP 3 INSERT...ON CONFLICT DO NOTHING completes, run:
...
-- proceed to STEP 4. STEP 5 MUST run to completion once the existence invariant passes.
...
Additionally, STEP 2 of the sweep transaction performs eligibility revalidation AND counter increment in a **single guarded UPDATE statement** (R52 HIGH-1 + R53 HIGH-1 closure 2026-05-21: SELECT-then-UPDATE is non-atomic; collapsed into one atomic UPDATE with RETURNING):
```sql
-- INSIDE STEP 2-5 atomic tx, ONE guarded UPDATE atomically revalidates eligibility AND
...
-- Raise sweep_stale_eligibility_lost; ROLLBACK entire STEP 2-5 transaction; emit
...
```
The 60-second lease bound ensures a crashed worker's claim becomes reclaimable. **Heartbeat renewal is also done in SHORT AUTOCOMMIT updates** (NOT inside the STEP 2-5 transaction) every 20s during long-running fan-out:
```sql
-- Heartbeat (autocommit; outside STEP 2-5):
UPDATE crisis_sweep_execution
...
```
**Fencing-token protection:** STEP 2-5 atomic transaction includes a fencing-token guard as its first statement:
```sql
-- INSIDE the STEP 2-5 atomic transaction (before any other writes):
SELECT 1 FROM crisis_sweep_execution
WHERE sweep_execution_id = $execution_id
  AND claimed_by_worker_id = $worker_id
...
-- and rollback before any side effects. This bounds STEP 2-5 to the worker whose fencing_token won.
```
**Lease takeover safety:**
- Crashed before COMMIT: second worker takes over (fencing_token incremented); first worker's heartbeat fails (fencing_token mismatch); if first worker somehow tries to commit STEP 2-5, the fencing-token guard at tx start raises sweep_fencing_token_mismatch and rolls back. Second worker re-executes STEP 2-5 cleanly with fresh sweep_cycle_id.
...
This non-locking claim + autocommit heartbeat + fencing-token-guarded STEP 2-5 works correctly under PostgreSQL row-lock semantics: the atomic STEP 2-5 transaction takes an UPDATE lock on the obligation row + provider_attempt inserts but does NOT hold a row lock on crisis_sweep_execution for the full duration; heartbeat updates run in separate short autocommit transactions and don't block on the in-flight sweep tx.

**Atomic sweep transaction — single canonical ordering (R61 HIGH-1 closure 2026-05-21: collapsed the legacy STEP 2-5 numbered ordering into the single canonical STEP A→F sequence used by Sub-decision 6's per-row algorithm; sweep_cycle_counter is incremented EXACTLY ONCE at STEP A; there is no duplicate increment elsewhere):**
...
The legacy "STEP 2-5" numbered transaction model is SUPERSEDED by the STEP A→F sequence. The closing guarded UPDATE (STEP F) is detailed below:
```sql
UPDATE crisis_sweep_execution
...
-- COMMIT. The taking-over worker will re-execute STEP 2-5 cleanly with its new fencing_token.
```
This final guarded compare-and-set is the database invariant that bounds STEP 2-5 commit to the worker whose fencing_token is still authoritative. A worker that passed the opening guard but lost its lease during fan-out CANNOT commit; the rollback discards all side effects atomically (per the R42 atomic-tx model). The opening fencing-token guard (STEP 2) + this closing guarded UPDATE (STEP 5) together form a "trapped invariant" — both must succeed under the same fencing_token, with COMMIT only happening after the second guard returns 1 row. Heartbeat updates that lose to a takeover are detected by the next renewal cycle (where 0 rows updated → worker aborts).
...
| 10 | `crisis.sweep_claim_recovery_with_committed_cycle` | Cat C | NOT sampled (governance recovery; R44 HIGH-1 + R45 MED-1 closure 2026-05-21: emitted when a second worker takes over a stranded claim and observes the impossible-but-bounded state completed_at IS NULL + sweep_cycle_id_committed IS NOT NULL — heal terminal mark to now() without re-executing STEP 2-5. Payload: sweep_execution_id, sweep_cycle_id_committed, previous_worker_id, recovery_worker_id, fencing_token_before, fencing_token_after, recovery_at) | P2 governance-partition |
...
- **R59 HIGH-1 closed:** removed "ROW_COUNT=0 → safe to no-op tier UPDATE" loophole; retry detection now bound to pre-STEP-2 worker entry guard against crisis_sweep_execution.completed_at + sweep_cycle_id_committed; inside STEP 2-5, ROW_COUNT < expected ALWAYS triggers ROLLBACK + Cat C audit + retry.
...
- **R45 HIGH-1 closed:** claim + heartbeat moved to separate autocommit txs; added fencing_token column; STEP 2-5 begins with fencing-token guard for takeover safety; no row-lock contention.
