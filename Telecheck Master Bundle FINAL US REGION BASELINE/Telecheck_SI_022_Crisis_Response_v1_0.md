...
**R44 HIGH-1 claim-lease + takeover contract (closure 2026-05-21):** the scheduler/worker MUST claim a row before executing under a TIME-BOUNDED lease:
```sql
-- Atomic claim with lease: claim a row that is either un-claimed OR whose prior claim has expired
UPDATE crisis_sweep_execution
SET claimed_by_worker_id = $worker_id, claim_expires_at = now() + INTERVAL '60 seconds'
WHERE sweep_execution_id = $next_due_id
  AND completed_at IS NULL
  AND (claimed_by_worker_id IS NULL OR claim_expires_at < now())
RETURNING sweep_execution_id;
```
The 60-second lease bound ensures a crashed worker's claim becomes reclaimable. The worker MUST renew the lease (UPDATE claim_expires_at = now() + INTERVAL '60s') periodically during long-running fan-out (e.g., every 20 seconds) until COMMIT. Lease takeover by a second worker is safe because:
- If the first worker rolled back STEP 2-5 (no COMMIT), no provider_attempt / Cat A audit / tier-advance rows are visible; the second worker re-executes cleanly.
- If the first worker COMMITTED but lease expired before completed_at was set (impossible per atomic-tx model, but defense-in-depth), the second worker would observe completed_at IS NULL but sweep_cycle_id_committed IS NOT NULL → emits `crisis.sweep_claim_recovery_with_committed_cycle` Cat C audit + marks completed_at = now() (recovering the row) WITHOUT re-executing STEP 2-5 (the committed work is preserved; only the terminal mark is healed).
- If the first worker crashed before COMMIT (the typical case), the second worker re-executes STEP 2-5 atomically — fresh sweep_cycle_id from the next increment.

**Worker heartbeat requirement:** workers MUST renew the lease at least every (lease_duration / 3) = 20 seconds while STEP 2-5 is in flight. Crashed workers without heartbeat are reclaimed after 60s.

The atomic STEP 2-5 sweep transaction:
