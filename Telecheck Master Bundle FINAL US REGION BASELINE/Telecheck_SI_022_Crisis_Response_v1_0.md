...
FROM compute_crisis_recipient_mapping(v_crisis_event_id, v_severity, 'care_team') AS mapping;
-- R25 HIGH-1 closure 2026-05-21: explicit target_tier='care_team' for initial STEP 4b fan-out
-- (matches the 3-arg form used by all sweep call sites in Sub-decision 6). Initial dispatch
-- targets ONLY care_team recipients (the initial escalation_tier); sweep-driven advances target
-- clinical_on_call / regulatory via Sub-decision 6 STEP 3 target_tier per R20/R21 closures.
-- Every normative call site MUST pass a non-null target_tier ∈ {'care_team', 'clinical_on_call',
-- 'regulatory'}; the function signature is canonically 3-arg.
...
16k. **Initial fan-out targets only care_team (R25 HIGH-1 closure 2026-05-21):** simulate crisis_detection_trigger emission on any severity tenant; verify STEP 4b INSERT statements produce provider_attempt rows for target_tier='care_team' ONLY (NOT clinical_on_call or regulatory). compute_crisis_recipient_mapping(v_crisis_event_id, v_severity, 'care_team') returns ONLY care-team-member tuples; first sweep at care_team deadline expiry is what advances target_tier to clinical_on_call via Sub-decision 6 STEP 3 + STEP 5.
