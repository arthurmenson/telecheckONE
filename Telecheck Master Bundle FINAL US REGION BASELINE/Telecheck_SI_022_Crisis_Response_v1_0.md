...
3. **Recipient fan-out (with R27 HIGH-1 + R29 HIGH-2 + R38 HIGH-1 closure 2026-05-21 idempotency + principal-id contract):** call `compute_crisis_recipient_mapping(crisis_event_id, severity, target_tier)` to derive recipients; the normative sweep INSERT statement (R38 HIGH-1: mirrors STEP 4b principal_id end-to-end):

```sql
INSERT INTO notification_crisis_provider_attempt
    (tenant_id, patient_id, server_signal_id, crisis_event_id, channel, recipient_role,
     recipient_address, recipient_principal_id, attempt_sequence)
SELECT v_tenant_id, v_patient_id, v_server_signal_id, v_crisis_event_id, mapping.channel,
       ...
       COALESCE((SELECT MAX(attempt_sequence)
                 FROM notification_crisis_provider_attempt
                 WHERE tenant_id = v_tenant_id
                   AND crisis_event_id = v_crisis_event_id
                   AND channel = mapping.channel
                   AND recipient_role = mapping.recipient_role
                   AND recipient_address = mapping.recipient_address), 0) + 1
FROM compute_crisis_recipient_mapping(v_crisis_event_id, v_severity, v_target_tier) AS mapping
ON CONFLICT ON CONSTRAINT notification_crisis_provider_attempt_idempotency_uk DO NOTHING;
-- R38 HIGH-1 closure 2026-05-21: sweep INSERT now normative + executable + populates recipient_principal_id
-- end-to-end. Per-recipient attempt_sequence MAX+1 lookup runs in the SELECT projection (R29/R30 closure).
-- CHECK constraint on notification_crisis_provider_attempt asserts recipient_principal_id IS NOT NULL
-- when recipient_role ∈ {'care_team', 'clinical_on_call', 'operator_escalation'} (principal-addressable
-- roles); NULL only permitted when recipient_role='emergency_contact' (unauthenticated path).
```

The above INSERT statement is the SOLE normative sweep tier-advance fan-out specification per R38 HIGH-1. Idempotency key UNIQUE(tenant_id, crisis_event_id, channel, recipient_role, recipient_address, attempt_sequence) ON CONFLICT DO NOTHING for worker-retry only.
