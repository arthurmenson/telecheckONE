...
-- **R63 HIGH-2 closure 2026-05-21 (zero-recipients fail-closed for required target tiers):**
--   For target_tier ∈ {'care_team', 'clinical_on_call', 'operator_escalation'} (the safety-critical
--   non-emergency-contact tiers), a mapping cardinality of ZERO is a config-floor violation: it means
--   tenant config passed preflight but a required recipient source resolved to nothing at sweep time
--   (e.g., race between preflight and tenant config change, or principal_id no longer resolves, or
--   address validation drift). ROLLBACK STEP A→F + emit crisis.dispatch_attempt_failed Cat C with
--   payload zero_recipients_for_required_tier=true + abort sweep. STEP E (tier ADVANCE) MUST NOT run
--   against an empty mapping for a required target tier. Tier='care_team' branch also includes the
--   same zero-recipients fail-closed for the initial-dispatch path at STEP 4b (the
--   compute_crisis_initial_recipient_mapping output for care_team MUST be ≥ 1 — checked at STEP 4b
--   pre-INSERT; otherwise crisis.dispatch_attempt_failed Cat C + rollback). Emergency_contact tier
--   MAY have zero recipients (no active emergency_contact_share consent grant); that is in-design,
--   not a config-floor violation.
