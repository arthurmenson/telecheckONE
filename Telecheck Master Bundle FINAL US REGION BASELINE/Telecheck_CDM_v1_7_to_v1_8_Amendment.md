...
-- R5 MED-1 closure 2026-05-21: SECURITY INVOKER view checks base-table privileges as the
-- invoking role (not the view owner). Reader role MUST also have SELECT on the 3 base tables
-- the view reads from. Without these grants, runtime reads via the view fail with permission
-- errors even though the view itself + view RLS allow them. RLS policies on base tables remain
-- enforced via current_tenant_id_strict() per §2.NEW1-NEW5.
GRANT SELECT ON ai_mode1_conversation TO ai_mode1_reader;
GRANT SELECT ON ai_mode1_conversation_turn_result TO ai_mode1_reader;
GRANT SELECT ON ai_mode1_conversation_archival_event TO ai_mode1_reader;
```
...
Authored on `spec/cdm-v1-8-audit-v5-10-ccr-v5-4-mode-1-followon-2026-05-21` branch off main at `9a1fcd2` (post-P-035 + Addendum 63). v0.2 commit `cba5266`. v0.3 commit `318c37a`. v0.4 commit `c473030`. v0.5 commit `fcd239b`. v0.6 commit pending push for R6 ship-it verification.
