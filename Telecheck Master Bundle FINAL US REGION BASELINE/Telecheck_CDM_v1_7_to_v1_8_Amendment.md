...
1. **OQ1 — `tenant.ai_provider` update audit event.** §5 references an `update_audit_action` field but the Mode 1 spec didn't enumerate a specific Cat B audit event for `tenant.ai_provider` updates. Recommendation: register a new `ccr.ai_provider_updated` Cat B audit event in a separate amendment cycle (not in scope here); for now, the existing platform CCR-update audit handles the emission.
