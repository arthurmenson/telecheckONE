...
```sql
-- R7 HIGH-1 closure 2026-05-21 (from Mode 1 spec): explicit security_invoker semantics +
-- non-BYPASSRLS owner-role discipline.
...
-- R5 MED-1 closure 2026-05-21 + R6 HIGH-1 closure 2026-05-21: SECURITY INVOKER view checks
-- base-table privileges as the invoking role (not the view owner). Reader role MUST have
-- SELECT on the base-table COLUMNS the view actually reads — but NOT on message-bearing
-- columns (user_message, assistant_message) which contain PHI/raw AI conversation content
-- that ai_mode1_reader (granted to clinician dashboard, pharmacy portal, admin roles) is
-- only supposed to access via the narrow state view. R6 closure replaces broad table-level
-- SELECT with column-level least-privilege grants matching exactly the view's SELECT list.
-- RLS policies on base tables remain enforced via current_tenant_id_strict() per §2.NEW1-NEW5.
GRANT SELECT (id, tenant_id, patient_id, created_at) ON ai_mode1_conversation TO ai_mode1_reader;
GRANT SELECT (tenant_id, conversation_id, completed_at) ON ai_mode1_conversation_turn_result TO ai_mode1_reader;
GRANT SELECT (tenant_id, conversation_id, archived_at) ON ai_mode1_conversation_archival_event TO ai_mode1_reader;
-- Explicit non-grant on the message-bearing columns: ai_mode1_reader CANNOT read raw
-- user_message or assistant_message via direct base-table access. To read message content,
-- a different role (e.g., Mode 1 service account, audit-tooling role) with explicit grants
-- on those columns is required; that grant is out of scope for this amendment.
```
...
Authored on `spec/cdm-v1-8-audit-v5-10-ccr-v5-4-mode-1-followon-2026-05-21` branch off main at `9a1fcd2` (post-P-035 + Addendum 63). v0.2 commit `cba5266`. v0.3 commit `318c37a`. v0.4 commit `c473030`. v0.5 commit `fcd239b`. v0.6 commit `c06bb32`. v0.7 commit pending push for R7 ship-it verification.
