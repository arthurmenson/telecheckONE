...
- I-029 — research data export 6-condition reject-unless gate (canonical full statement at §14 / INVARIANTS v5.2 I-029 — *summary expanded 2026-05-02 per Codex Round-12 Scope 3 HIGH-1 finding from prior 5-condition gate to canonical 6-condition gate adding per-export grant artifact re-validation*): (1) `dsa_status_at_export = active`; (2) `k_threshold_actual >= k_min_required`; (3) `permitted_data_domains_at_export` matches snapshot (no CCR drift); (4) `consent_cohort_snapshot_hash_completed = consent_cohort_snapshot_hash_initiated`; (5) every contributing patient has active `ResearchConsent` at completion-time; (6) per-export grant artifact (PolicyAuthorization or named-equivalent) exists, unexpired, matches export, attests signer chain. Failed delivery MUST emit `research.export_completed(status=invalidated)` paired with `signal_enforcement_trigger` per I-003.
...
- **I-029 (NEW for v1.10)** — research data export 6-condition reject-unless gate (per ADR-028; canonical 6-condition gate per INVARIANTS v5.2 / TYPES v5.2 / AUDIT_EVENTS v5.2 §5 / OpenAPI v0.2 / STATE_MACHINES v1.1 / GOVERNANCE_CONTROLS v5.2 / CCR_RUNTIME v5.2; *expanded 2026-05-02 per Codex Round-12 Scope 3 HIGH-1 finding from prior 5-condition to canonical 6-condition gate adding per-export grant artifact re-validation*): (1) `dsa_status_at_export = active`; (2) `k_threshold_actual >= k_min_required` (default k_min=11); (3) `permitted_data_domains_at_export` matches the `research.export_initiated` snapshot (no CCR drift mid-export); (4) `consent_cohort_snapshot_hash_completed = consent_cohort_snapshot_hash_initiated` (cohort unchanged mid-export); (5) every contributing patient has active `ResearchConsent` at completion-time evaluation; (6) per-export grant artifact exists, unexpired, matches export by ID/hash binding, and still attests the multi-party signer chain (ADR-028 v0.4 quad + REC concurrence). Failed delivery MUST emit `research.export_completed(status=invalidated)` with canonical `invalidation_reason` enum value (one of `dsa_inactive | k_anonymity_violation | permitted_domain_drift | consent_cohort_change | consent_revocation_mid_export | grant_artifact_invalidated`) paired with `signal_enforcement_trigger` Category B audit per I-003 (bare suppression forbidden).
