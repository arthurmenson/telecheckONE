...
**Version:** 1.1
**Status:** ✅ Canonical for development (Phase 6 promoted 2026-05-01 per Evans's "authorized" instruction; promoted from v1.0 PROVISIONAL per Evans's Option B 2026-04-28 directive folded into v1.10 cycle as Phase 5.6 / F49 alongside C3 brand-structure cascade)
**Owner:** Design Lead + Engineering Lead (joint)
**Parent documents:** Master Platform PRD v1.10 §11 + §24 (design contract decision; DIC fold-in DECIDED 2026-04-28), Design System v1.1, Patient App IA v1.0, Clinician Portal IA v1.0, Admin Operator IA v1.1, Forms/Intake Engine Slice PRD v2.1, Pharmacy + Refill Slice PRD v2.1, Admin Backend Slice PRD v1.1
**Companion documents:** Engineering Handoff & Build Guide v1.3, RBAC Permissions Matrix v1.1, **`telecheck-design-system/project/Patient interactive mock v7.html` (binding visual reference per v1.1 Canonical for development status)**
**Format:** Markdown

---

## ✅ Canonical for development — v1.1 status notice (2026-05-01)

**This document is CANONICAL FOR DEVELOPMENT** as of 2026-05-01 Phase 6 promotion. Per Evans's Option B decision 2026-04-28, the DIC v1.0 PROVISIONAL → v1.1 Canonical promotion was folded into the v1.10 PRD update cycle as Phase 5.6 / F49 alongside the C3 brand-structure cascade.

**Binding visual reference:** `telecheck-design-system/project/Patient interactive mock v7.html` (and its sibling `v7 - *.jsx` companions and `v7.css` files) — Patient mock v7 authoritative per CLAUDE.md design handoff section. §4.1 / §4.2 pixel-exact-match clauses are now ACTIVATED (no longer marked PROVISIONAL).

**Substitution flags carry forward:** Manrope (font), Lucide (icons), wordmark in `assets/logo/`, photography placeholders — all flagged for replacement before customer ship. The substitution-flag carry-forward does NOT relax the pixel-exact-match requirement against the v7 mock; placeholders are visually exact, and the substitution lifecycle replaces them with production assets.
...
DIC v1.0 PROVISIONAL is superseded by this v1.1; v1.0 is preserved at `Telecheck_Design_Implementation_Contract_v1_0.md` for traceability per the bundle's copy + supersede convention. Active Document Index §4 records the supersession; Promotion Ledger entry P-008 records the v1.10 cycle promotion ceremony.

---

## Historical PROVISIONAL status notice (preserved for traceability — superseded 2026-05-01)

**(The following text was the v1.0 PROVISIONAL status notice; it is preserved here unchanged for historical reference. The v1.1 status notice above supersedes it.)**
...
- Design files must include the multi-tenant brand variations: at minimum, neutral default + Heros Health (Telecheck-US consumer DBA) brand + Heros Health Ghana (Telecheck-Ghana consumer DBA) brand applied to the same screen, demonstrating how brand tokens substitute.
...
- Tenant brand variant demonstration (at minimum: Heros Health Ghana applied [Telecheck-Ghana consumer DBA] + Heros Health applied [Telecheck-US consumer DBA])
...
Visual regression runs on the platform-default tenant configuration AND on at least the two day-1 operating tenants (Telecheck-US trading as Heros Health DBA, and Telecheck-Ghana trading as Heros Health Ghana DBA). This catches tenant-specific bugs (e.g., a brand color override producing illegible contrast, a custom logo breaking a layout).
...
- **v1.1 — 2026-05-01** (per v1.10 Phase 6 promotion ceremony per Evans's "authorized" instruction; per Evans's Option B 2026-04-28 directive folded into v1.10 cycle as Phase 5.6 / F49 alongside C3 brand-structure cascade). **Status flip: PROVISIONAL → Canonical for development.** Patient mock v7 (`telecheck-design-system/project/Patient interactive mock v7.html`) declared the binding visual reference per CLAUDE.md design handoff section. §4.1 / §4.2 pixel-exact-match clauses ACTIVATED (no longer marked PROVISIONAL). Substitution flags carry forward: Manrope (font), Lucide (icons), wordmark in `assets/logo/`, photography placeholders — all flagged for replacement before customer ship. Pharmacy portal kit gap noted: not in v1 design system; gap to be filled when pharmacy slice work begins. C3 brand-structure cascade applied: §103 (and the multi-tenant brand variations clause at the top of this document) updated per Phase5 delta Row 44 — "the neutral platform default + the Heros Health consumer DBA (country-instanced; primary surface is `heroshealth.com` for Telecheck-US and `ghana.heroshealth.com` for Telecheck-Ghana)". Cross-reference to `telecheck-design-system/` design handoff bundle. v1.0 PROVISIONAL framing deprecated and superseded; v1.0 preserved at `Telecheck_Design_Implementation_Contract_v1_0.md` for traceability. **v1.10.1 hygiene-cycle verification (2026-05-02):** confirmed Status = "Canonical for development" (line 4); confirmed §4.1 / §4.2 pixel-exact-match clauses are activated (per the v1.1 Status notice at lines 12–24 superseding the historical PROVISIONAL notice at lines 28–53); confirmed Patient mock v7 referenced as authoritative (lines 7, 16); confirmed substitution flags carry forward (line 18); confirmed §103 / Multi-tenant brand variations clause updated per Row 44 (lines 22, 121); no body edits required at this verification pass.
