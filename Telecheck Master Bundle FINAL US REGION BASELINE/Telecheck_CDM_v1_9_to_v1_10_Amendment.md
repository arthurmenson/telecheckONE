...
BEGIN
    ...
    BEGIN
        ...
        IF v_offending_grant IS NOT NULL THEN
            RAISE EXCEPTION 'crisis-view-grant-allowlist-violation (staff view): %; only crisis_event_staff_reader may hold SELECT on crisis_event_current_state_v', v_offending_grant;
        END IF;
    ...
    BEGIN
        ...
        BEGIN
            -- Verify the synthetic test role exists (created by Phase 7.1.a; NOT a
            -- member of any canonical reader/admin role; NO LOGIN; used only by
            -- preflight via SET ROLE):
            ...
            -- Switch role; attempt CTAS; expect the event trigger to raise. SAVEPOINT
            -- ensures any partial state from the failed CTAS is rolled back cleanly.
            EXECUTE 'SET LOCAL ROLE ' || quote_ident(v_test_role);
            BEGIN
                EXECUTE 'CREATE MATERIALIZED VIEW pg_temp.__crisis_ctas_preflight_probe AS SELECT * FROM public.crisis_event_current_state_v WHERE FALSE';
                v_blocked := FALSE;   -- if we reach here, trigger did NOT block — defect
            EXCEPTION
                WHEN insufficient_privilege THEN
                    v_blocked := TRUE;   -- expected outcome
                WHEN OTHERS THEN
                    -- Any other exception type means the trigger fired but with the
                    -- wrong errcode, or some other unexpected error; reject so the
                    -- behavioral guarantee is preserved.
                    RAISE EXCEPTION 'crisis-view-ctas-preflight-unexpected-error: behavioral CTAS probe raised unexpected exception (SQLSTATE %, message %); expected insufficient_privilege from crisis_view_ctas_provenance_block', SQLSTATE, SQLERRM;
            END;
            -- Restore role context:
            RESET ROLE;

            IF NOT v_blocked THEN
                RAISE EXCEPTION 'crisis-view-ctas-behavioral-violation: synthetic non-breakglass role % was able to CREATE MATERIALIZED VIEW from crisis_event_current_state_v WITHOUT triggering crisis_view_ctas_provenance_block; the trigger is mis-configured (e.g., command-tag filter wrong, function body stub, or trigger disabled mid-deploy); recreate per Phase 7.1', v_test_role;
            END IF;
        END;
    END;
...
BEGIN
    ...
    -- Crisis-domain object reference detection (case-insensitive, word-boundary):
    IF v_query ~* '\m(crisis_event_current_state_v|crisis_event_patient_summary_v|crisis_event|crisis_event_lifecycle_transition|crisis_sweep_execution|notification_crisis_dispatch_ledger|notification_crisis_provider_attempt|notification_crisis_escalation_obligation)\M' THEN
        ...
        IF NOT v_breakglass_allowed THEN
            RAISE EXCEPTION
                'crisis-view-ctas-provenance-block: role % is not in the canonical break-glass-admin allowlist and attempted % referencing a crisis-domain object; CTAS / SELECT INTO / CREATE MATERIALIZED VIEW on crisis-domain objects is REJECTED to prevent persistent rows outside the security boundary',
                v_role, TG_TAG
...
The function runs SECURITY DEFINER + locked search_path; `current_query()` returns the original statement text including the CTAS body so a regex on crisis-domain object names is the safe inspection vector at `ddl_command_start` (pg_event_trigger_ddl_commands() is unavailable at start phase per PostgreSQL docs). Trigger is enabled before Phase 7 view creation runs. **§8.1 class K secondary assertion expanded (R9 HIGH-1 closure 2026-05-21) to BEHAVIORALLY verify the trigger blocks CTAS** — not just existence/enabled — by attempting a representative CREATE MATERIALIZED VIEW against a crisis view under a synthetic non-breakglass test role + asserting it raises `insufficient_privilege`. CTAS / SELECT INTO copies of crisis views are blocked at DDL time AND verified blocked at preflight time.
7.1.a. **Phase 7.1.a — Synthetic preflight test role (R9 HIGH-1 addendum 2026-05-21):** create `crisis_ctas_preflight_test_role` with `NOLOGIN`; grant CREATE on `pg_temp` schema only; do NOT grant membership in any canonical reader/admin/wrapper-owner role. The role exists exclusively for §8.1 class K behavioral assertion (which uses `SET LOCAL ROLE crisis_ctas_preflight_test_role` to attempt a CREATE MATERIALIZED VIEW from a crisis view and verify the event trigger blocks it). Role is created at Phase 7.1.a alongside the event trigger so the behavioral preflight assertion always has its test principal available.
