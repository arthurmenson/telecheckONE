...
BEGIN
    ...
    BEGIN
        ...
        -- created at §8.2 Phase 7.1 (R8 HIGH-1 addendum). Preflight ASSERTS that the
        ...
        IF NOT FOUND THEN
            RAISE EXCEPTION 'crisis-view-ctas-provenance-trigger-missing: the canonical event trigger crisis_view_ctas_provenance_block (created at Phase 7.1) is missing OR disabled; CTAS / SELECT INTO copies of crisis views would not be caught by pg_depend-based class K; recreate the event trigger before cutover';
        END IF;
    END;
...
7.1. **Phase 7.1 — DDL event trigger for CTAS provenance block (R8 HIGH-1 addendum 2026-05-21):** create event trigger `crisis_view_ctas_provenance_block` on `ddl_command_start` for {`CREATE TABLE AS`, `SELECT INTO`, `CREATE MATERIALIZED VIEW`} commands. Trigger body inspects parsed command text via `pg_event_trigger_ddl_commands()` + `current_query()` and raises an exception if the command references either crisis view name or `crisis_event_*` base-table names AND the issuing role is not in the canonical break-glass-admin allowlist. The trigger is enabled before Phase 7 view creation runs and verified live by §8.1 class K secondary assertion. CTAS / SELECT INTO copies of crisis views are blocked at DDL time; this is the AUTHORITATIVE defense for the CTAS bypass class (pg_depend tracking does NOT exist for CTAS-produced tables). MV CREATE is also covered both here AND by pg_depend in class K (defense-in-depth).
