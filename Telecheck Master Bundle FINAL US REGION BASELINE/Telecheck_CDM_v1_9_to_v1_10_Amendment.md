...
BEGIN
    ...
    -- R14 HIGH-1 closure 2026-05-21: CATEGORICAL BLOCK — reject ALL CTAS / SELECT INTO /
    -- CREATE MATERIALIZED VIEW commands from non-breakglass roles regardless of statement
    -- content. v_query is preserved in the exception message for forensics but is NO
    -- LONGER used as authorization input.
    --
    -- Why categorical (R14 evolution of the closure):
    -- - v0.15 text-scan caught direct references but missed function indirection (R13 HIGH-1).
    -- - v0.16 added has_table_privilege() capability check on canonical crisis tables;
    --   caught non-SECDEF helper indirection (helper invokes view under caller privileges).
    -- - But SECURITY DEFINER wrapper indirection (R14 HIGH-1) defeats v0.16: a low-
    --   privilege caller with no direct SELECT on crisis tables can EXECUTE a SECDEF
    --   wrapper function that reads crisis data under the wrapper-owner's privilege
    --   and returns rows; the CTAS statement text only names the wrapper, and
    --   current_user (the CTAS issuer) has no crisis SELECT capability for
    --   has_table_privilege() to detect.
    -- - Resolving every referenced function and walking its body to detect SECDEF +
    --   crisis-domain access at trigger time is brittle (covers static SQL only;
    --   misses EXECUTE / dblink / nested SECDEF chains; introduces parser-version
    --   dependencies).
    -- - The categorical block is strictly stronger: no detection logic to bypass,
    --   no chain of indirection to chase. Non-breakglass roles cannot use these 3
    --   DDL commands at all in this database. Routine migration work uses standard
    --   DDL (CREATE TABLE + INSERT, not CTAS); analytics MVs that need CTAS-like
    --   shapes are owned by break-glass-admin and granted through normal grant
    --   matrix to their consumers.
    IF TRUE THEN  -- always evaluate breakglass check; categorical block
        ...
        IF NOT v_breakglass_allowed THEN
            -- R12 HIGH-1 + R14 HIGH-1 closure 2026-05-21: use a DEDICATED user-defined
            -- SQLSTATE so class K can distinguish "trigger blocked CTAS" from "PG normal
            -- view permission check rejected (insufficient_privilege)". The latter
            -- would fire if the test role lacks SELECT on the source view BEFORE
            -- the event trigger had a chance to evaluate. ERRCODE 'CR022' is a
            -- user-defined code (5-char) reserved for SI-022 / P-040 crisis
            -- provenance enforcement; class K asserts this exact SQLSTATE.
            -- R14 HIGH-1: message reflects categorical block discipline (no longer
            -- conditioned on crisis-domain reference in statement text).
            RAISE EXCEPTION
                'crisis-view-ctas-provenance-block: role % is not in the canonical break-glass-admin allowlist and attempted % — CTAS / SELECT INTO / CREATE MATERIALIZED VIEW is CATEGORICALLY rejected for non-breakglass roles in this database to prevent persistent crisis-data copies outside the security boundary (covers direct + helper-function + SECDEF-wrapper indirection paths uniformly). Query text (forensic): %',
