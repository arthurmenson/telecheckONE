...
BEGIN
    ...
    -- R13 HIGH-1 closure 2026-05-21: dual-trigger detection — block CTAS/MV/SELECT INTO
    -- when EITHER (a) current_query() text directly references a crisis-domain object,
    -- OR (b) the calling role can read any crisis-domain object (via direct grant or
    -- role-membership inheritance). Branch (b) catches function-indirection bypasses
    -- where a non-SECDEF helper function reads the crisis view and the CTAS reads
    -- the helper — the CTAS statement text doesn't contain crisis-domain names but
    -- the caller's effective privileges would let it persist crisis data outside the
    -- security boundary.
    IF v_query ~* '\m(crisis_event_current_state_v|crisis_event_patient_summary_v|crisis_event|crisis_event_lifecycle_transition|crisis_sweep_execution|notification_crisis_dispatch_ledger|notification_crisis_provider_attempt|notification_crisis_escalation_obligation)\M'
       ...
       -- canonical crisis-domain tables/views catches the indirection bypass class
       -- (the role would have to read at least one of these to construct the helper
       -- function whose result the CTAS persists).
