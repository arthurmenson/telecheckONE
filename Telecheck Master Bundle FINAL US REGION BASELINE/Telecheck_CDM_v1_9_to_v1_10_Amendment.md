...
BEGIN
    ...
    -- (K) R7 HIGH-2 closure 2026-05-21 — derived-persistent-copy rejection.
    -- Materialized views or other relations populated from the crisis views can
    -- persist tenant-wide crisis rows OUTSIDE the security_invoker / RLS / read-role
    -- boundary, then be granted to noncanonical roles without showing as offending
    -- SELECT grants on the original crisis views. Same impact as R1 HIGH-2 vulnerability
    -- but via persisted derivation instead of grant matrix.
    --
    ...
    BEGIN
        SELECT d.refobjid::regclass::text || ' (relkind=' || c2.relkind ||
               ', owner=' || pg_get_userbyid(c2.relowner)::text || ') depends on '
               || c.relname
          INTO v_offending
          FROM pg_class c
          JOIN pg_rewrite rw ON rw.ev_class = c.oid
          JOIN pg_depend d ON d.refobjid = rw.oid AND d.refclassid = 'pg_rewrite'::regclass
          JOIN pg_class c2 ON c2.oid = d.objid AND d.classid = 'pg_class'::regclass
         WHERE c.relname IN ('crisis_event_current_state_v', 'crisis_event_patient_summary_v')
           AND c2.relkind IN ('m', 'v', 'r', 'f')  -- materialized view, view, table, foreign table
           AND c2.relname NOT IN ('crisis_event_current_state_v', 'crisis_event_patient_summary_v')
           AND c2.relname <> ALL(v_canonical_view_dependents)
         LIMIT 1;
        IF v_offending IS NOT NULL THEN
            RAISE EXCEPTION 'crisis-view-derived-relation-violation: %; non-allowlisted persistent relation depends on a crisis view (could persist tenant-wide rows outside security boundary); allowlist update + ratifier sign-off required', v_offending;
        END IF;
    END;
