...
type AIWorkloadType =
  ...
  | "n/a";                           // SENTINEL — added v5.2 patch 2026-05-02 per Codex Round-7 Scope 1 HIGH-1 finding; scope broadened 2026-05-02 per Codex Round-8 Scope 1 HIGH finding. Reserved exclusively for use as the envelope-level value on **any I-012 clinician-only approval / confirmation audit record** where the human signer acted without any AI involvement and the I-012 closure rule still requires the field populated. The authoritative scope is **the full I-012 action-class set defined in AUDIT_EVENTS v5.2 §I-012 closure rule**: `prescribing.initiated`, `prescribing.approved`, `prescribing.declined`, `prescribing.modified`, `refill.approved`, `refill.declined`, `protocol_authorized_prescribing` (when human-only, hypothetical), `protocol_authorized_refill_renewal` (when human-only, hypothetical), `protocol_authorized_dispensing_release` (when human-only), and any future I-012-class action — wherever a human signer is the sole actor and no AI workload was upstream. Invalid for AIExecution / successful AI workload records and for any record where an AI workload was actually involved upstream. Per AUDIT_EVENTS v5.2 §I-012 closure rule clinician-only carve-out.
```
