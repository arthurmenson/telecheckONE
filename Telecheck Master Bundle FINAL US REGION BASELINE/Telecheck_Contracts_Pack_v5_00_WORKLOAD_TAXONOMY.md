...
`ai_workload_type` is a string enum carried on every `AIExecution` entity (per CDM v1.2) and every audit event with `actor_type = ai_workload` (per AUDIT_EVENTS v5.2). *(Field name `actor_type` corrected 2026-05-02 per Codex Round-9 Scope 1 MEDIUM-1 finding — was previously stated as `actor.type` (with dot), which doesn't match the audit envelope schema's flat `actor_type` field. The canonical envelope discriminator is `actor_type = ai_workload` per AUDIT_EVENTS v5.2 envelope schema; queries and validators MUST use that name.)*

**Audit-event nullability rule (v0.2 patch — Codex 2026-04-29 Finding 3):** For new v1.10 AI audit events, `ai_workload_type` and `autonomy_level` are **required** fields (not nullable). They are nullable only for: (a) legacy events backfilled from before v1.10 promotion; (b) non-AI events (where `actor_type ≠ ai_workload`); EXCEPT for I-012 action-class records, where `ai_workload_type` and `autonomy_level` are required regardless of `actor_type` per AUDIT_EVENTS v5.2 §I-012 closure rule (with `n/a` sentinel carve-out for clinician-only approvals). Reserved agentic-context fields (`agent_id`, `agent_version`, `tool_call_id`, `memory_read_set_id`, `memory_write_set_id`, `supervising_policy_id`, `knowledge_source_versions[]`) remain nullable across all events; they populate only when the corresponding capability activates.
...
type AIWorkloadType =
  ...
  | "n/a";                           // SENTINEL — added v5.2 patch 2026-05-02 per Codex Round-7 Scope 1 HIGH-1 finding; scope broadened 2026-05-02 per Codex Round-8 Scope 1 HIGH; authority delegated to AUDIT_EVENTS 2026-05-02 per Codex Round-9 Scope 1 HIGH-1. Reserved exclusively for use as the envelope-level value on **any I-012 clinician-only approval / confirmation audit record** where the human signer acted without any AI involvement and the I-012 closure rule still requires the field populated. **The authoritative I-012 action-class set is defined in AUDIT_EVENTS v5.2 §I-012 closure rule** (single source of truth — this comment intentionally does NOT enumerate the set; consult AUDIT_EVENTS to see which records carry the I-012 closure obligation). Invalid for AIExecution / successful AI workload records and for any record where an AI workload was actually involved upstream. Per AUDIT_EVENTS v5.2 §I-012 closure rule clinician-only carve-out.
```
