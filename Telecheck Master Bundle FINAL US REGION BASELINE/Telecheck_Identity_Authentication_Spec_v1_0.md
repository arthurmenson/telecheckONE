...
1. **Assumes the authContextPlugin has already verified JWT signature/expiry/audience + session liveness** (Codex SC6 PR #10 R29 round-29 HIGH closure 2026-05-18 — the prior procedure-side liveness-check wording split the revoked-session path across both Fastify plugin and SECURITY DEFINER procedure, creating duplicate/mis-owned audit writes and inconsistent 401/503 fail-closed behavior; per the round-28 plugin-owned writer-ownership model, all binding-path audit emission lives on the application side; `identity.session_liveness_check_failed` is owned by the plugin's Step 2 fail-closed path, NOT this procedure). The procedure receives JWT-derived input parameters (`p_account_id`, `p_tenant_id`, `p_role`, `p_admin_home_tenant_id`, `p_session_id`, `p_nonce`) that have ALREADY passed plugin-side liveness validation; the procedure does NOT re-verify liveness, does NOT emit `identity.session_liveness_check_failed`, and does NOT raise `UnauthenticatedError`. The procedure's only failure modes are PostgreSQL-level (permission denied; primary-key violation on UPSERT collision against an active row; etc.); any application-level authentication failure is the plugin's responsibility BEFORE this procedure is called.
...
4. Returns to the authContextPlugin caller (the procedure itself does NOT emit `identity.actor_context_bound` — emission is owned by the application-side authContextPlugin which calls into the audit-writer chain AFTER the procedure returns, per Codex R28 round-28 MEDIUM closure to align AUDIT_EVENTS catalog wording + Identity Spec § wiring + Test 7A on a single writer-ownership model). The plugin then emits `identity.actor_context_bound` Cat B audit via a **rollback-independent commit path** that survives request-transaction rollback. Implementation: the audit write is performed on a SEPARATE database connection that opens, writes, and commits the audit row BEFORE control returns to the request-transaction code path that may later roll back. Two concrete patterns are canonical (Codex SC6 PR #10 R2 round-3 HIGH finding closure 2026-05-18 — the previously-specified `pg_notify` path was rolled back to the drawing board because PostgreSQL's NOTIFY delivery is transactional and a NOTIFY issued inside the request transaction is suppressed on rollback):
   ...
   **Fail-closed semantics on audit-writer failure (Codex SC6 PR #10 R4 round-4 MEDIUM finding closure 2026-05-18):** if the rollback-independent audit commit fails (connection acquisition fails, INSERT raises, COMMIT fails, dblink_exec raises, etc.), the binding/procedure path MUST fail closed BEFORE any PHI-touching work proceeds. Concrete semantics by emission point:
   - **`identity.actor_context_bound` (application-side; durable-backstop-first fallback chain per Codex SC6 PR #10 R27 round-27 HIGH closure 2026-05-18 — the prior wording short-circuited to 503 on primary failure regardless of backstop health, contradicting Test 7A sub-scenario (ii) which requires primary failure + healthy backstop to commit to backstop + return the normal client envelope):** if the `auditWriterPool` commit fails AFTER the `bind_actor_context()` UPSERT succeeded, the authContextPlugin MUST attempt the dedicated audit-backstop commit (`auditBackstopPool`) BEFORE returning any error to the client. **If the backstop commit succeeds**, the request proceeds normally — the audit anchor is durably persisted on the backstop chain, the recovery_link checkpoint will bridge it back to primary on outage drain, and the client receives the normal authenticated-request envelope (200). **Only if BOTH primary AND backstop commits fail**, the plugin raises `AuditAnchorUnavailableError` → Fastify error-envelope plugin maps to **tenant-blind 503 per I-025** → the request transaction rolls back (the binding row is discarded by ROLLBACK; no orphan state); PHI-touching work in the route handler never executes because the plugin's error short-circuits the request before `done()` is called. This fallback semantics aligns with Test 7A sub-scenarios (i)/(ii)/(iv).
   ...
   **Hash-chain ordering across primary + backstop partitions (Codex round-6 R1 HIGH closure 2026-05-18):** events recorded in the backstop instance during a primary outage are NEVER back-inserted into the primary `identity_lifecycle` partition. The backstop's own `identity_lifecycle_backstop` partition is append-only with its own `previous_hash` + `sequence_number` discipline; primary continues its own chain unaffected during the outage. When primary connectivity restores, the `audit_backstop_ingest` worker drains the backstop chain into a quarantined holding area, then emits **variant-aware `identity.audit_recovery_link` checkpoint event(s)** (added v5.4 patch per Codex round-6 closure + refined per R17/R22/R23/R24/R25/R26 closures for multi-tenant outage windows; see AUDIT_EVENTS v5.4 amendment for the full per-variant payload contract). **For single-tenant outages** (backstop chain contains events for exactly one tenant or all events are `_PLATFORM_`-attributed): the worker emits ONE per-tenant checkpoint into the PRIMARY `identity_lifecycle` partition keyed by that tenant; the checkpoint carries scalar `backstop_partition_id` + `backstop_chain_head_audit_id` + `backstop_chain_tail_audit_id` + `outage_window_start` + `outage_window_end` + `recovered_event_count`. **For multi-tenant outages** (backstop chain spans multiple tenants): the worker emits ONE per-tenant checkpoint per distinct tenant (each into the primary partition keyed by that tenant, each with its own scalar backstop endpoints) PLUS ONE aggregate `_PLATFORM_` checkpoint (into the primary partition keyed by `_PLATFORM_`; aggregate carries NO scalar `backstop_partition_id`/head/tail because no single backstop partition represents the multi-tenant window — instead carries `per_tenant_checkpoints[]` manifest cross-referencing each per-tenant checkpoint's `recovery_link_audit_id`). Forensic walks across the outage period: single-tenant → primary chain → encounter per-tenant recovery_link → cross into the backstop chain segment → walk to backstop tail → return to primary; multi-tenant → either start from a tenant's primary chain → encounter that tenant's per-tenant recovery_link → cross into its backstop segment → return; or start from the aggregate `_PLATFORM_` checkpoint → walk the manifest array → resolve each per-tenant `recovery_link_audit_id` → cross into each per-tenant backstop segment. Append-only integrity holds in both chains; no rewrites; no out-of-order primary inserts. Operational invariant: only one outage window may be active per partition at a time (concurrent outages serialize on the backstop instance's connection-pool admission control).
...
7. **Fail-closed on audit-writer failure with durable backstop instance + hash-chain integrity + recovery_link emission resilience (Codex round-4 + round-5 + round-6 + round-7 + round-11 closures 2026-05-18; split into 7A (3 binding-path events) + 7B (recovery_link checkpoint) per round-11 MEDIUM closure to reflect the distinct writer + correlation model):**
   ...
   **Test 7B (`identity.audit_recovery_link` checkpoint — distinct writer / correlation / durability model per round-11 closure):** scenario covers ONLY the `audit_backstop_ingest` worker emitting the recovery_link checkpoint into the PRIMARY `identity_lifecycle` partition after backstop drain. The worker is NOT invoked from a request transaction, has NO client envelope, and MUST NOT write the recovery_link to the `identity_lifecycle_backstop` chain (the bridge always lands in primary; otherwise the bridge has no anchor for the next-outage forensic walk). Sub-scenarios: (i) **primary available at drain time** → recovery_link commits to primary `identity_lifecycle` at the end of the partition's current chain head; `previous_hash`/`sequence_number` extend in arrival order; resource_id depends on the checkpoint variant (per Codex R19 + R23 closures): **per-tenant checkpoint** (partition_key = concrete `account_tenant_id`) uses the 3-part composite `(backstop_partition_id, outage_window_start, partition_key)`; **aggregate `_PLATFORM_` checkpoint** (multi-tenant outage windows only) uses the distinct namespace `aggregate:_PLATFORM_:<outage_window_start>` (NO scalar backstop_partition_id; the aggregate references the per-tenant checkpoint set via its `per_tenant_checkpoints[]` manifest); backstop chain ranges + outage timestamps populated; worker logs success. (ii) **primary unavailable at recovery_link emission time** (drain ready, but primary `identity_lifecycle` partition's audit-writer surface still in outage) → worker retries with exponential backoff against primary; no write to backstop chain; worker advances retry queue. (iii) **primary recovers mid-retry** → recovery_link commits at end of retry queue against the then-current primary chain head (NOT back-inserted into the historical position; concurrent primary `identity_lifecycle` writes during the retry window are unaffected). (iv) **idempotency on resource_id collision** → a recovery_link already in primary for the same resource_id (per-variant: 3-part `(backstop_partition_id, outage_window_start, partition_key)` for per-tenant checkpoints; `aggregate:_PLATFORM_:<outage_window_start>` namespace for the aggregate checkpoint — per Codex R19 + R23 + R24 closures) causes the worker to no-op rather than emit a duplicate; multiple worker instances racing on the same outage window converge to one canonical checkpoint set (per-tenant + aggregate `_PLATFORM_`) via advisory-lock serialization + per-variant idempotency check (3-part composite for per-tenant; aggregate-namespace for `_PLATFORM_` aggregate — no uniform idempotency-on-3-part key for the aggregate path). Test 7B gate asserts: (a) recovery_link references valid backstop chain endpoints (chain head + tail + outage window) on per-tenant checkpoint variants; aggregate `_PLATFORM_` variant carries the `per_tenant_checkpoints[]` manifest with valid `recovery_link_audit_id` cross-references instead; (b) recovery_link lands in PRIMARY `identity_lifecycle` partition, never in `identity_lifecycle_backstop`; (c) advisory-lock topology serializes concurrent ingest attempts (only one drain proceeds per outage window); (d) idempotency-on-resource_id collision prevents duplicate emissions (per-tenant variant uses 3-part composite; aggregate variant uses `aggregate:_PLATFORM_:<outage_window_start>` namespace); (e) the worker never invokes `auditWriterPool` or `dblink_exec` to the procedure-side loopback (those are request-path writers; the recovery_link writer uses its own primary-audit connection that is independent of the request-path pools). **(f) Aggregate manifest exact-set assertion (Codex R23 round-23 HIGH-2 closure 2026-05-18):** for multi-tenant outage windows, assert that the aggregate `_PLATFORM_` checkpoint's `per_tenant_checkpoints[]` array is an EXACT-SET match against the emitted per-tenant recovery_link checkpoints — every emitted per-tenant checkpoint MUST appear exactly once in the aggregate's array (no missing tenants, no duplicate `partition_key`s, no stale `recovery_link_audit_id` cross-references that don't resolve to an actual per-tenant checkpoint row); the aggregate's `recovered_event_count` MUST equal the sum of per-tenant counts; the array's `partition_key` values MUST be distinct. Without this assertion an implementation could emit a malformed aggregate (missing tenant → forensic walks silently skip that tenant's backstop segment; duplicate tenant → forensic walks double-count) while passing the listed sub-assertions (a)-(e).
