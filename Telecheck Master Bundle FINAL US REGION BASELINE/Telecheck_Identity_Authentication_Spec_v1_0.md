...
4. Emits `identity.actor_context_bound` Cat B audit via the **rollback-durable audit-writer connection** (NOT via the request-bound transaction). Implementation: the procedure invokes `audit_emit_async(action, detail)` which posts the audit record to a process-local outbox channel via `pg_notify`; a dedicated audit-writer worker connection (separate pool, separate transaction lifecycle) drains the channel and commits the audit row to `audit_events` independently of the request transaction outcome. Rationale (Codex SC6 PR #10 R2 round-2 MEDIUM finding closure 2026-05-18): if the request transaction later aborts, the binding row is correctly discarded by ROLLBACK, but the `identity.actor_context_bound` forensic anchor MUST survive so investigators can correlate downstream `actor_context_unbound_rejected` or `session_liveness_check_failed` events to the original successful bind attempt. A same-transaction audit emit would lose this anchor on every rolled-back request. The same rollback-durable path is used for `identity.actor_context_unbound_rejected` (emitted by failing SECURITY DEFINER procedures whose enclosing transaction will roll back) and `identity.session_liveness_check_failed` (emitted before any binding row exists). The audit-writer worker connection holds I-027 attribution discipline by writing rows under its own `audit_writer_role` identity with the actor context derived from the audit `detail` payload, not from the audit-writer's own bound context.
