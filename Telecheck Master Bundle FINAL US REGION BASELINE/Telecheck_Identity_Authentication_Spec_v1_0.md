...
- **`telecheck_app_role` (LOGIN, request connections):** is a member of `bind_actor_context_role` (so `SET LOCAL ROLE bind_actor_context_role` succeeds for legitimate plugin invocations). Inherits ONLY `EXECUTE` on `bind_actor_context()` from that membership. Has SELECT on `_session_actor_context` ONLY via the helper functions (which themselves are `SECURITY DEFINER` and read from the table on the app role's behalf). Test #1 asserts the inheritance chain: `telecheck_app_role` cannot directly DML the table even after `SET LOCAL ROLE bind_actor_context_role` — only the procedure's `SECURITY DEFINER` runtime owner (`_session_actor_context_owner`) can.
