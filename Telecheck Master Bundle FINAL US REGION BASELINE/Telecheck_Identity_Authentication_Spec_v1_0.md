...
3. Calls `bind_actor_context()` via **role elevation on the request-scoped connection** — NOT under `telecheck_app_role` directly (that role has zero EXECUTE on `bind_actor_context()` per the GRANT gate; test #1 enforces this). The canonical pattern: `SET LOCAL ROLE bind_actor_context_role; CALL bind_actor_context(...); RESET ROLE;` all within the request transaction. `SET LOCAL ROLE` is scoped to the transaction and reverts automatically on commit/rollback; the explicit `RESET ROLE` after the CALL is defense-in-depth to guarantee the request handler executes under `telecheck_app_role`. The membership grant `GRANT bind_actor_context_role TO telecheck_app_role` is created in the SI-010 migration so the `SET LOCAL ROLE` succeeds; the EXECUTE grant on `bind_actor_context()` remains restricted to `bind_actor_context_role` only (so explicit role elevation is the only way the binding fires; the app role cannot invoke the procedure under its base identity). An alternative implementation pattern — dedicated `bindingPool` of connections authenticated directly as `bind_actor_context_role`, used only for the binding statement and then handed back — is also permitted and semantically equivalent; pick one of the two patterns consistently per service.
