...
4. Emits `identity.actor_context_bound` Cat B audit via a **rollback-independent commit path** that survives request-transaction rollback. Implementation: the audit write is performed on a SEPARATE database connection that opens, writes, and commits the audit row BEFORE control returns to the request-transaction code path that may later roll back. Two concrete patterns are canonical (Codex SC6 PR #10 R2 round-3 HIGH finding closure 2026-05-18 — the previously-specified `pg_notify` path was rolled back to the drawing board because PostgreSQL's NOTIFY delivery is transactional and a NOTIFY issued inside the request transaction is suppressed on rollback):
   ...
   **Fail-closed semantics on audit-writer failure (Codex SC6 PR #10 R4 round-4 MEDIUM finding closure 2026-05-18):** if the rollback-independent audit commit fails (connection acquisition fails, INSERT raises, COMMIT fails, dblink_exec raises, etc.), the binding/procedure path MUST fail closed BEFORE any PHI-touching work proceeds. Concrete semantics by emission point:
   ...
   - **`identity.session_liveness_check_failed` (application-side, pre-bind) — fail-closed with durable backstop, NOT a permitted audit gap (Codex SC6 PR #10 R5 round-5 HIGH closure 2026-05-18; the prior "log P0 alert and return 401 anyway" framing was rolled back because it created exactly the audit gap I-003 forbids — audit gaps are audit defects, and a mutable operational alert is not equivalent forensic evidence to an append-only audit row):** if the primary `auditWriterPool` commit fails, the plugin MUST fall through to a **durable backstop audit path** before returning the 401. The backstop is an append-only `audit_backstop.log` file on local disk opened with `O_APPEND | O_SYNC` semantics; each backstop write fsyncs the JSON-serialized audit envelope to disk before control returns. A separate `audit_backstop_ingest` worker continuously drains the backstop file into the primary `audit_events` table once primary connectivity is restored (the worker's ingest is idempotent on the envelope's `audit_id`; partial drains are safe). If even the local-disk backstop write fails (disk full, FS read-only, etc.), the plugin MUST raise `AuditAnchorUnavailableError` → tenant-blind **503** (NOT 401) — the rejection itself is suppressed at the API surface until the rejection can be auditably recorded. Patient-visible outcome under healthy primary path: 401 as expected. Under primary-path failure with healthy backstop: 401 as expected (backstop is durable; the audit chain stays complete after ingest catch-up). Under both-paths failure (rare; disk-failure-class incident): 503 + critical ops alert + ops break-glass triggers a synchronous restore of the primary or backstop write surface before the platform serves rejection traffic.
   - **`identity.actor_context_unbound_rejected` (procedure-side, via `dblink_exec`) — fail-closed with durable backstop, symmetric to the liveness-failed path:** if `dblink_exec` raises (loopback connection down, audit_writer_role grants revoked, etc.), the SECURITY DEFINER procedure MUST fall through to the same `audit_backstop.log` durable-disk path (via a `dblink_exec`-less SECURITY DEFINER backstop helper that writes to a local-FS-backed UNLOGGED table mirrored to disk via a 1-second flush trigger; OR via a server-side `COPY ... TO PROGRAM` invoking a fsync-wrapping shim). Whether the primary OR the backstop write succeeds, the procedure THEN raises the original `actor_context_unbound`/`request_nonce_unbound_or_expired` exception so the calling transaction rolls back (the audit row, primary or backstop, is on a separate transaction lifecycle so it survives the procedure's rollback regardless). If BOTH primary and backstop writes fail, the procedure raises the original exception WRAPPED with `caused_by audit_anchor_failure` AND the request-envelope plugin maps the wrapped exception to a tenant-blind **503** + critical ops alert (NOT the original procedure-level error envelope, because the rejection itself is unaudited and the platform cannot prove the rejection occurred from immutable audit history).
...
7. **Fail-closed on audit-writer failure with durable backstop (Codex round-4 + round-5 closures 2026-05-18):** simulate `auditWriterPool` failure (connection pool exhausted, INSERT raises, COMMIT raises) AND simulate `dblink_exec` failure (loopback connection down, `audit_writer_role` grants revoked) across all three identity-lifecycle events. Test in three sub-scenarios per event: (i) **primary path healthy** → audit row commits to `audit_events`, expected client envelope (200/401/procedure-error). (ii) **primary path fails, backstop healthy** → audit row persisted to `audit_backstop.log` via `O_APPEND|O_SYNC` fsync; subsequent `audit_backstop_ingest` drain replays the row idempotently into `audit_events`; client envelope unchanged from healthy-path expectation. (iii) **both primary and backstop fail** (disk full / FS read-only / `COPY ... TO PROGRAM` rejected) → client receives tenant-blind 503 + critical ops alert on the `audit_anchor_failure` channel; no `_session_actor_context` row remains; no PHI-touching mutation in the route handler commits; for procedure-side: the original procedure-level error is WRAPPED with `caused_by audit_anchor_failure` AND mapped to 503 (NOT to the original procedure-level envelope). The test gate asserts I-003 (audit completeness) holds across all sub-scenarios — there is no path through the gate where a rejection occurs without a durable audit trace (primary OR backstop).
