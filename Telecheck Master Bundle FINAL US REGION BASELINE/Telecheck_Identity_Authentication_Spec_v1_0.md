...
4. Emits `identity.actor_context_bound` Cat B audit via a **rollback-independent commit path** that survives request-transaction rollback. Implementation: the audit write is performed on a SEPARATE database connection that opens, writes, and commits the audit row BEFORE control returns to the request-transaction code path that may later roll back. Two concrete patterns are canonical (Codex SC6 PR #10 R2 round-3 HIGH finding closure 2026-05-18 — the previously-specified `pg_notify` path was rolled back to the drawing board because PostgreSQL's NOTIFY delivery is transactional and a NOTIFY issued inside the request transaction is suppressed on rollback):
   ...
   **Fail-closed semantics on audit-writer failure (Codex SC6 PR #10 R4 round-4 MEDIUM finding closure 2026-05-18):** if the rollback-independent audit commit fails (connection acquisition fails, INSERT raises, COMMIT fails, dblink_exec raises, etc.), the binding/procedure path MUST fail closed BEFORE any PHI-touching work proceeds. Concrete semantics by emission point:
   ...
   - **`identity.session_liveness_check_failed` (application-side, pre-bind):** if the `auditWriterPool` commit fails, the plugin MUST still raise `UnauthenticatedError` (the original fail-closed path) but logs a P0 alert (`audit_anchor_failure` log channel) indicating an unrecoverable audit gap on the rejection path. The patient-side outcome (401) is unchanged; the operational signal escalates to ops.
   - **`identity.actor_context_unbound_rejected` (procedure-side, via `dblink_exec`):** if `dblink_exec` raises (loopback connection down, audit_writer_role grants revoked, etc.), the SECURITY DEFINER procedure MUST raise the original `actor_context_unbound`/`request_nonce_unbound_or_expired` exception WRAPPED with a `caused_by audit_anchor_failure` annotation. The calling transaction rolls back regardless; the wrapped exception ensures the error envelope returned to the client and the ops-alert log channel both reflect that the rejection was unaudited (a P0 operational signal — rejections must always be auditable for forensic walks).
...
7. **Fail-closed on audit-writer failure (Codex round-4 closure 2026-05-18):** simulate `auditWriterPool` failure (connection pool exhausted, INSERT raises, COMMIT raises) AND simulate `dblink_exec` failure (loopback connection down, `audit_writer_role` grants revoked) across all three identity-lifecycle events. Assert: (a) the request transaction rolls back; (b) no `_session_actor_context` row remains; (c) no PHI-touching mutation in the route handler commits; (d) the appropriate error envelope reaches the client (`503` for `actor_context_bound` failure; `401` for `session_liveness_check_failed` failure; the original procedure-level error WRAPPED with `caused_by audit_anchor_failure` for `actor_context_unbound_rejected` failure); (e) the `audit_anchor_failure` log channel records a P0 alert in every failure case.
