...
**Incident response audit-path discipline (aligned with AUDIT_EVENTS v5.2 §5 export event family; MAY → MUST 2026-05-02 per Codex Round-6 Scope 2 HIGH-1 finding; enum expanded to 6-value 2026-05-02 per Codex Round-12 Scope 3 HIGH-1 + post-merge adversarial review HIGH-1):** When an incident triggers export invalidation, the audit chain MUST capture the failure transparently. Per AUDIT_EVENTS v5.2 §5: `research.export_completed` **MUST** emit with the violated state recorded in payload (e.g., `dsa_status_at_export = expired`, `permitted_data_domains_at_export` showing drift, `k_threshold_actual < k_min_required`, or `grant_artifact_validity_to_at_completion = <expired-timestamp>`); the event's `status` field carries `invalidated` to mark the failed completion AND `invalidation_reason` populated to one of the canonical 6-value enum (`dsa_inactive | k_anonymity_violation | permitted_domain_drift | consent_cohort_change | consent_revocation_mid_export | grant_artifact_invalidated`). Concurrently, the export pipeline **MUST** emit a `signal_enforcement_trigger` Category B audit event capturing the enforcement action (export artifact destruction; partner notification; engineering review trigger). The two records compose to give a complete audit trail of "what happened" + "what we did about it." Bare suppression of the completion event (no record at all) is forbidden — silent invalidation is an audit gap per I-003. Prior MAY framing is superseded.

**6-condition incident matrix (rewritten 2026-05-02 per Codex Round-4 Scope 2 HIGH-1 finding to mirror exactly the canonical I-029 reject-unless gate from AUDIT_EVENTS v5.2 / TYPES v5.2 / STATE_MACHINES v1.1 / OpenAPI v0.2; replaces prior 4-condition table that combined `consent_cohort_change` and per-patient active-consent failure into a single "consent revocation mid-export" row; expanded from 5-condition to 6-condition 2026-05-02 per Codex Round-12 Scope 3 HIGH-1 + post-merge adversarial review HIGH-1 finding adding the per-export grant-artifact gate):**
...
| 6. Grant artifact invalidation (added 2026-05-02 per Codex Round-12 Scope 3 HIGH-1) | The per-export grant artifact (PolicyAuthorization or named-equivalent per CCR_RUNTIME v5.2) at completion-time fails any of: nonexistence, expiry (`grant_artifact_validity_to < now`), ID/hash mismatch with the initiated export, OR signer-chain attestation invalidation (any signer rescinded the attestation between initiation and completion) | `grant_artifact_invalidated` | Per I-029 condition 6: `research.export_completed` MUST emit with `status = invalidated`, `invalidation_reason = grant_artifact_invalidated`; export artifact destroyed; `signal_enforcement_trigger` emitted; the failed grant artifact ID is captured in the audit payload for incident review; engineering MUST NOT auto-issue a replacement grant — recompute / re-export requires a fresh grant artifact attesting the current multi-party signer chain (including any new REC concurrence per `per_dsa_review_required` if the prior REC concurrence was rescinded). |
