...
Function is SECURITY INVOKER (no caller-identity manipulation needed; trigger executes under the transaction's effective privileges); locked search_path per P-034 R7 hardening; schema-qualified `public.forms_template_admin_review_lifecycle_transition` reference per P-038 R4 trigger-function hardening pattern. The append-only trigger above (declared just before this monotonic-ordering trigger) combined with this monotonic-ordering trigger means lifecycle rows are: (a) cannot be UPDATEd or DELETEd; (b) cannot be backdated; (c) cannot be future-dated beyond 5s clock-skew tolerance.
