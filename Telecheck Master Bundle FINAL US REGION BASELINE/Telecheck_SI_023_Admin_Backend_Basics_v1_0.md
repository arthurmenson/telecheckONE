...
```sql
CREATE OR REPLACE FUNCTION forms_template_admin_review_lifecycle_continuity_monotonic_ordering()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path = pg_catalog, public
...
BEGIN
    -- Reject future-dating beyond clock-skew tolerance (defense against caller
    -- manipulation that would let a later "real" transition land with an earlier
    -- transition_at; corrupts ORDER BY transition_at DESC, id DESC current-state
    -- derivation)
    ...
    -- Reject backdating: NEW.transition_at MUST be >= MAX(transition_at) for prior
    -- rows of the same (tenant_id, review_id)
...
ALTER FUNCTION forms_template_admin_review_lifecycle_continuity_monotonic_ordering() OWNER TO cdm_owner;

CREATE TRIGGER forms_template_admin_review_lifecycle_continuity_monotonic_ordering_trigger
    BEFORE INSERT ON forms_template_admin_review_lifecycle_transition
    FOR EACH ROW
    EXECUTE FUNCTION forms_template_admin_review_lifecycle_continuity_monotonic_ordering();

-- R13 HIGH-3 closure 2026-05-22: state-continuity invariant trigger.
-- Append-only trigger + monotonic-ordering trigger validate (a) immutability and
-- (b) temporal ordering, but neither validates that NEW.from_state equals the
-- current latest to_state for the review. The CHECK constraint validates the
-- (from_state, to_state, transition_reason) TRIPLE shape (only the 5 valid
-- combinations are accepted), but a wrapper-owner role could append (e.g.) a
-- 'pending_review → approved' tuple after an already-approved latest state →
-- legal triple shape + monotonic timestamp + matches CHECK + but corrupts
-- current-state derivation by inserting from a stale predecessor.
--
-- This trigger enforces: NEW.from_state MUST equal the current latest to_state
-- for the (tenant_id, review_id) pair, OR if no prior rows exist NEW.from_state
-- MUST be 'none' (initial_submission only).
CREATE OR REPLACE FUNCTION forms_template_admin_review_lifecycle_state_continuity()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path = pg_catalog, public
...
DECLARE
    ...
    v_lock_key BIGINT;
BEGIN
    -- R14 HIGH-1 + R15 HIGH-1 closure 2026-05-22: per-review xact-scoped advisory
    -- lock + READ COMMITTED isolation precondition. The advisory lock alone is
    -- not sufficient under REPEATABLE READ snapshot isolation: a tx that took
    -- its snapshot BEFORE the winner committed would wait on the lock + then
    -- read the OLD snapshot (not seeing the winner's row), allowing a stale
    -- successor insert to pass the continuity check. Under READ COMMITTED, each
    -- statement takes a fresh snapshot AT statement-start, so after the advisory
    -- lock releases, the latest-state SELECT below reads the committed winner.
    --
    -- This trigger therefore REQUIRES READ COMMITTED isolation (which is the
    -- canonical PostgreSQL default; SI-023 wrapper API contracts inherit this
    -- default). Explicit precondition check raises invalid_transaction_initiation
    -- if a caller has elevated isolation to REPEATABLE READ or SERIALIZABLE.
    ...
    IF v_latest_to_state IS NULL THEN
        -- No prior rows; NEW MUST be the initial transition (from_state='none')
        ...
        END IF;
    ELSE
        -- Prior rows exist; NEW.from_state MUST equal the latest to_state
...
ALTER FUNCTION forms_template_admin_review_lifecycle_state_continuity() OWNER TO cdm_owner;

CREATE TRIGGER forms_template_admin_review_lifecycle_state_continuity_trigger
    BEFORE INSERT ON forms_template_admin_review_lifecycle_transition
    FOR EACH ROW
    EXECUTE FUNCTION forms_template_admin_review_lifecycle_state_continuity();
