...
DECLARE
    ...
    v_lock_key BIGINT;
BEGIN
    -- R14 HIGH-1 closure 2026-05-22: per-review xact-scoped advisory lock so
    -- the trigger is a STANDALONE concurrency barrier (not just wrapper-discipline-
    -- dependent defense-in-depth). Without this lock, two concurrent INSERTs for
    -- the same review could both read the same latest state + both pass the
    -- continuity check + both insert successor rows from the same predecessor.
    -- pg_advisory_xact_lock(int8) acquires a per-review lock keyed by a hash of
    -- (tenant_id, review_id); auto-released at tx commit/rollback. Concurrent
    -- INSERTs for the same review block here; latest-state read happens AFTER
    -- the lock is held, so subsequent reads see the committed predecessor.
