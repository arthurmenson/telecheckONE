...
4. **3 OPTIONAL canonical views** (R15 HIGH-3 closure 2026-05-22 — corrected from stale "2 views ... granted to admin_basic_operator role only" v0.1 inventory that omitted the R1 HIGH-2 + R2 HIGH-1 admin_mode1_volume_health_v view AND incorrectly stated direct admin_basic_operator SELECT grants; canonical model is wrapper-only): `admin_crisis_operational_health_v` + `admin_consult_queue_health_v` + `admin_mode1_volume_health_v` (tenant-scoped via current_tenant_id_strict; all 3 with `security_invoker=true + security_barrier=true`; SELECT granted ONLY to the corresponding dashboard read-wrapper-owner role per §7 — admin_basic_operator does NOT have direct SELECT on any admin view; the SECDEF read-wrappers per Sub-decision 3.5 are the SOLE canonical read path per R1 HIGH-1 closure).
5. **6 SECURITY DEFINER procedures** (R14 HIGH-2 closure 2026-05-22 — corrected from v0.1's stale "4 procedures" count to the canonical 6 matching Sub-decision 5 + §8.1 Class O allowlist + §7 wrapper-owner enumeration; the v0.1 placeholder `record_admin_dashboard_query_execution` record-only wrapper is REMOVED because R1 HIGH-1's SECDEF read-wrappers (Sub-decision 3.5) already INSERT audit rows + emit Cat A audit co-transactionally — the record-only wrapper became redundant): **1 raw lifecycle writer** `record_forms_template_admin_review_transition()` + **3 dashboard read-wrappers** (`read_admin_crisis_operational_health` + `read_admin_consult_queue_health` + `read_admin_mode1_volume_health` per Sub-decision 3.5) + **2 template wrappers** (`submit_forms_template_for_admin_review` + `record_forms_template_admin_decision` per Sub-decision 4). Total: 6 SECDEF procedures owned by 6 distinct owner roles per §7.
...
The remaining 2 wrappers (`read_admin_consult_queue_health` + `read_admin_mode1_volume_health`) follow the identical shape against their respective views + audit `dashboard_name` payload values. **R15 HIGH-2 closure 2026-05-22 — canonical inventory is EXACTLY 3 SECDEF dashboard read-wrappers (no record-only wrapper); the v0.1 placeholder `record_admin_dashboard_query_execution` is REMOVED because the 3 read-wrappers already INSERT audit rows + emit Cat A audit co-transactionally per the FLOOR-020 fail-closed pattern — a separate record-only wrapper became redundant.** Total SECDEF procedures across SI-023: **6** (3 dashboard read-wrappers + 2 template wrappers per Sub-decision 4 + 1 raw lifecycle writer per Sub-decision 4.5) — matches §1 + §3 + Sub-decision 5 + §7 RBAC + §8.1 Class O exactly. **The wrapper-only canonical read path means class N audit-completeness assertion (Sub-decision 3) becomes a defensive check on database-integrity drift rather than a primary enforcement mechanism — the structural enforcement is the privilege boundary itself.**
...
BEGIN
    ...
    -- R11 HIGH-1 closure 2026-05-22: shared parent-template serialization point.
    -- Both submit_forms_template_for_admin_review and record_forms_template_admin_decision
    -- MUST participate in the SAME (tenant_id, forms_template_id) serialization lock to
    -- prevent submit-vs-decision races (e.g., submit reads stale "pending_review" latest
    -- state, raises already-in-flight; decision commits terminal right after — invariant
    -- preserved by chance via visibility timing rather than by design).
    --
    -- Lock ordering discipline (matches submit wrapper): acquire parent forms_template
    -- row lock FIRST, then the review row lock. The submit wrapper locks
    -- forms_template -> reads existing review under that lock; the decision wrapper
    -- here locks forms_template -> locks review under that same lock. Consistent
    -- (template, review) lock acquisition order prevents deadlock.
    --
    -- To know which forms_template row to lock, we need v_review_forms_template_id.
    -- Step 0: derive template_id from review without lock (forms_template_admin_review
    --   is append-only on the forms_template_id column so this read is stable).
    -- Step 1: acquire parent forms_template FOR UPDATE (SHARED SERIALIZATION POINT).
    -- Step 2: NOW acquire review row FOR UPDATE under the parent lock.
    ...
    -- Step 1: parent-template lock (SHARED with submit wrapper)
    PERFORM 1 FROM forms_template
     WHERE tenant_id = p_tenant_id AND id = v_review_forms_template_id
       FOR UPDATE;

    -- Step 2: review row lock (under parent lock)
    ...
    -- R2 MED-1 + R13 HIGH-2 closure 2026-05-22: INSERT idempotency-key row with
    -- explicit unique_violation handler. The pre-check above (SELECT ... INTO
    -- v_existing_decision) handles the serialized-already-committed case where the
    -- idempotency_key row already exists. But under CONCURRENT same-key calls
    -- (R13 HIGH-2), two transactions can both pass the pre-check (neither sees the
    -- other's uncommitted row), both reach this INSERT, and the loser collides on
    -- the UNIQUE constraint with 23505. Without an explicit handler, the loser
    -- surfaces 23505 to the caller instead of the canonical idempotent-success
    -- response promised by §4.NEW4. The handler catches unique_violation, re-reads
    -- the now-committed existing row, verifies the decision matches, and returns
    -- canonical idempotent success (or raises idempotency-key-decision-mismatch).
    BEGIN
    ...
    EXCEPTION
        WHEN unique_violation THEN
            -- Concurrent same-key INSERT won the race. Re-read existing row + verify
            -- decision matches; if so, this tx's transition is a logical duplicate of
            -- the winner's transition and should NOT be emitted twice. Rollback this
            -- tx's lifecycle transition + return canonical idempotent success.
            SELECT decision INTO v_existing_decision
              FROM admin_template_decision_idempotency_key
             WHERE tenant_id = p_tenant_id AND review_id = p_review_id
               AND idempotency_key = p_idempotency_key;
            IF v_existing_decision = p_decision THEN
                -- Logical duplicate of winning concurrent call; ROLLBACK this tx
                -- (RAISE with a sentinel that the outer call site catches as
                -- canonical idempotent success — or use a savepoint pattern).
                -- For simplicity at the wrapper boundary, RAISE EXCEPTION with
                -- explicit 'serialization_failure' so the application/HTTP layer
                -- catches it as "retry-safe" and the FIRST tx's response (which IS
                -- the canonical answer) propagates back to the client. NOTE: this
                -- relies on the canonical retry-safety pattern at the API layer.
                RAISE EXCEPTION 'admin-template-decision-concurrent-same-key-retry-safe: concurrent identical-key call already committed decision %; retry on the client side to receive canonical success response', v_existing_decision
                    USING ERRCODE = '40001';  -- serialization_failure
            ELSE
                RAISE EXCEPTION 'idempotency-key-decision-mismatch: concurrent call committed decision % but this request had decision %; not safe to retry', v_existing_decision, p_decision
                    USING ERRCODE = '40001';
            END IF;
    END;
...
### Sub-decision 4.5 — Raw lifecycle transition writer (R12 HIGH-1 closure 2026-05-22)

The canonical raw append-only lifecycle writer that both submit + decision wrappers call (per Sub-decision 4 wrapper bodies). Matches P-038 §3.1 + P-040 §3.1 raw-writer pattern: SECURITY DEFINER + locked search_path; EXECUTE granted to EXACTLY the 2 template wrapper-owner roles (anti-bypass per Sub-decision 5 + §8.1 class O).
...
BEGIN
    -- The CHECK constraint at §4.NEW3 enforces the 5 valid (from_state, to_state,
    -- transition_reason) triples; the monotonic-ordering trigger (also at §4.NEW3)
    -- enforces non-backdated transition_at. This raw writer is the SOLE INSERT path
    -- into forms_template_admin_review_lifecycle_transition; the 2 template wrapper
    -- owner roles (submit + decision) are the SOLE EXECUTE grantees; application
    -- roles do NOT hold EXECUTE on this raw writer (anti-bypass discipline per
    -- P-034 §3 + P-038 §3 + P-040 §3).
...
-- EXECUTE granted to EXACTLY the 2 template wrapper-owner roles per Sub-decision 5
-- + §8.1 class O. NO other roles receive EXECUTE on the raw writer.
...
-- R7 HIGH-1 + R8 HIGH-1 + R9 HIGH-1 + R10 HIGH-1 closure 2026-05-22: per-template
-- enforcement of one-active-review-per-(tenant_id, forms_template_id) is done in
-- 2 layers (R10 HIGH-1 closure removed the prior LAYER 3 partial UNIQUE INDEX
-- because the append-only lifecycle row design is fundamentally incompatible with
-- a "latest-row-only" UNIQUE constraint; historical `none -> pending_review` rows
-- stay in the index forever and would block the canonical revision cycle):
--
-- LAYER 1 (PRIMARY — R8 HIGH-1 + R11 HIGH-1): BOTH submit and decision wrappers
--   acquire `SELECT 1 FROM forms_template WHERE tenant_id = p_tenant_id AND
--   id = v_template_id FOR UPDATE` as the SHARED serialization point keyed by
--   (tenant_id, forms_template_id). Submit wrapper locks template at start;
--   decision wrapper locks template after deriving v_review_forms_template_id from
--   the review row (Step 0 read without lock; Step 1 parent FOR UPDATE; Step 2
--   review FOR UPDATE — consistent template→review acquisition order prevents
--   deadlock). All state-changing lifecycle writers thus serialize on the same
--   per-template lock; submit-vs-decision races are eliminated structurally.
...
--   Catches serialized-retry cases after LAYER 1 commits, AND catches direct
--   base-table INSERTs that bypass the wrapper.
--
-- LAYER 3 (REMOVED per R10 HIGH-1 closure): the partial UNIQUE INDEX
-- `forms_template_admin_review_one_active_per_template_uk` previously declared in
-- v0.9-v0.10 is removed. Future hardening via maintained current-state projection
-- table is a Track 6 follow-on item if needed.
--
-- BEFORE INSERT trigger on forms_template_admin_review (LAYER 2);
-- R13 HIGH-1 closure 2026-05-22: this function + trigger creation is MOVED OUT
-- of §4.NEW2 because the function references
-- public.forms_template_admin_review_lifecycle_transition which doesn't exist until
-- §4.NEW3 creates it. Phase 2 executes §4.NEW1 → NEW2 → NEW3 → NEW4 in order; placing
-- the function here in §4.NEW2 would attempt to CREATE FUNCTION before the referenced
-- table exists → migration fails. Function + trigger creation are relocated to §4.NEW3
-- below (immediately after CREATE TABLE forms_template_admin_review_lifecycle_transition).
--
-- For reference, the function + trigger DDL (now in §4.NEW3):

/* MOVED to §4.NEW3 per R13 HIGH-1 closure 2026-05-22:

[function + trigger definition moved to §4.NEW3 below — see "BEFORE INSERT trigger on
forms_template_admin_review for one-active-review-per-template enforcement"]
*/
...
-- R10 HIGH-1 closure 2026-05-22: the partial UNIQUE INDEX
-- `forms_template_admin_review_one_active_per_template_uk` (declared in v0.10 with
-- predicate `to_state IN ('pending_review', 'revision_requested')`) is REMOVED.
-- The append-only lifecycle row design is fundamentally incompatible with a
-- "latest-row-only" UNIQUE constraint: the initial `none -> pending_review` row stays
-- in the index forever; a later `pending_review -> revision_requested` insert for the
-- same review_id would ALSO match the predicate and violate the index, blocking the
-- canonical revision cycle (state-machine triples #4 + #5).
--
-- PostgreSQL partial UNIQUE indexes cannot express "latest row only" without a
-- maintained current-state projection. SI-023 v1.0 relies on the canonical 2-layer
-- enforcement architecture instead:
--
-- LAYER 1 (PRIMARY — R8 HIGH-1): submit wrapper acquires `SELECT 1 FROM forms_template
--   WHERE tenant_id = p_tenant_id AND id = p_template_id FOR UPDATE` at start;
--   serializes concurrent submits keyed by (tenant_id, forms_template_id).
-- LAYER 2 (DEFENSE-IN-DEPTH — R7 HIGH-1): BEFORE INSERT trigger
--   `enforce_one_active_review_per_template` on forms_template_admin_review rejects
--   creating a second review row for the same (tenant_id, forms_template_id) when an
--   existing review has a non-terminal latest state (LATERAL-derived); catches the
--   serialized-retry case after LAYER 1 commits, AND catches direct base-table INSERTs
--   that bypass the wrapper.
--
-- The R10 HIGH-1 closure REMOVES LAYER 3 (the partial UNIQUE INDEX) because it was
-- incompatible with the append-only row design. LAYER 1 + LAYER 2 together provide
-- the canonical enforcement; future hardening (e.g., a maintained current-state
-- projection table) is a Track 6 follow-on item if/when additional defense-in-depth is
-- needed beyond the SECDEF wrapper boundary.
```

**Monotonic-ordering invariant** (per P-038 R2 + P-040 §4.NEW2 pattern); R12 HIGH-2 closure 2026-05-22 — moved from prose into executable DDL:

```sql
-- R16 HIGH-1 closure 2026-05-22: merged monotonic-ordering + state-continuity into
-- a SINGLE trigger function so the advisory lock + isolation precondition cover
-- BOTH invariants under a single lock-held read window. The prior v0.15 design had
-- two separate triggers (monotonic + state-continuity); PostgreSQL fires same-event
-- BEFORE INSERT triggers in alphabetical order by trigger NAME — the monotonic
-- trigger fired FIRST (alphabetically) and read MAX(prior.transition_at) WITHOUT
-- the lock; only the state-continuity trigger (firing later alphabetically) took
-- the lock. A concurrent tx could pass the monotonic check on stale prior set,
-- wait on the advisory lock, then pass state continuity against the winner while
-- carrying an older transition_at → corrupts current-state derivation.
-- Merged into ONE trigger so both checks happen under the SAME lock + isolation
-- precondition.
...
BEGIN
    -- R15 HIGH-1: READ COMMITTED isolation precondition (covers all invariant checks below)
    ...
    -- R14 HIGH-1 + R16 HIGH-1: per-review xact-scoped advisory lock acquired BEFORE
    -- any prior-row reads (covers BOTH monotonic-ordering MAX read AND state-continuity
    -- latest-to_state read under the same lock window).
    v_lock_key := ('x' || substr(md5(NEW.tenant_id::text || ':' || NEW.review_id::text), 1, 16))::bit(64)::bigint;
    PERFORM pg_advisory_xact_lock(v_lock_key);

    -- Invariant 1: future-dating bounded by 5s clock-skew tolerance
    ...
    -- Invariant 2: backdating rejected (NEW.transition_at >= MAX(prior.transition_at))
    -- — READ under lock so the value is the committed predecessor's transition_at
    ...
    -- Invariant 3: state-continuity — NEW.from_state matches latest to_state under lock
...
-- R13 HIGH-3 + R14 HIGH-1 + R15 HIGH-1 + R16 HIGH-1 closure 2026-05-22:
-- state-continuity invariant trigger MERGED into the unified
-- forms_template_admin_review_lifecycle_invariants() function above. The 3 lifecycle
-- invariants (immutability via append-only trigger; monotonic-ordering; state-continuity)
-- are enforced by 2 triggers total: (a) the unified BEFORE INSERT invariants trigger
-- above which holds the advisory lock + READ COMMITTED precondition + does both
-- monotonic-ordering and state-continuity checks under that lock; (b) the
-- enforce_append_only trigger (declared earlier in §4.NEW3) which blocks UPDATE/DELETE.
-- Prior v0.14-v0.15 had 3 separate triggers; R16 HIGH-1 closure merged monotonic +
-- state-continuity into 1 unified trigger because PostgreSQL fires same-event triggers
-- in alphabetical order by trigger name (monotonic-ordering fired BEFORE state-continuity
-- + read MAX(prior.transition_at) WITHOUT the lock → stale prior set). Merged-function
-- design eliminates the inter-trigger ordering hazard structurally.

-- R13 HIGH-1 closure 2026-05-22 (relocated from §4.NEW2): the
-- enforce_one_active_review_per_template trigger function references
-- forms_template_admin_review_lifecycle_transition (above), so it's defined HERE
-- in §4.NEW3 after the table exists. BEFORE INSERT trigger ON forms_template_admin_review
-- (which is in §4.NEW2) is also created here in §4.NEW3 because PostgreSQL allows
-- attaching a trigger to a previously-created table once the function exists.
...
Function metadata: all 3 lifecycle-invariant triggers (append-only + monotonic-ordering + state-continuity) are SECURITY INVOKER (no caller-identity manipulation; trigger executes under transaction's effective privileges); all schema-qualified + locked search_path per P-034 R7 + P-038 R4 hardening pattern. **Together they enforce: (a) lifecycle rows are immutable (no UPDATE/DELETE); (b) temporally well-ordered (no backdating; future-dating bounded by 5s clock-skew); (c) state-continuous (NEW.from_state matches latest to_state OR is 'none' for initial submission).** The append-only trigger on forms_template_admin_review (in §4.NEW2) is augmented here in §4.NEW3 by enforce_one_active_review_per_template (now defined in correct DDL order after the lifecycle_transition table exists).
...
2. **Phase 2 — Tables + indexes + triggers (R10 HIGH-2 closure 2026-05-22 — consolidated; the §4 DDL blocks in v0.10 already include CREATE TRIGGER statements inline; a separate "Phase 4 — Triggers" would attempt to recreate them + fail with duplicate_object):** execute the §4.NEW1/NEW2/NEW3/NEW4 DDL blocks in order. Each block creates its table + RLS policies + append-only trigger + invariant triggers + indexes inline (CREATE TABLE → ALTER TABLE ENABLE ROW LEVEL SECURITY → CREATE POLICY → CREATE TRIGGER → CREATE INDEX). The 4 net-new tables (`admin_dashboard_query_execution` + `forms_template_admin_review` + `forms_template_admin_review_lifecycle_transition` + `admin_template_decision_idempotency_key`) are deployed together in this phase (admin_template_decision_idempotency_key dependency on forms_template_admin_review + lifecycle_transition dependency on forms_template_admin_review force same-phase deployment in order: NEW1 → NEW2 → NEW3 → NEW4). All triggers schema-qualified + locked search_path per P-034 R7 hardening pattern.
...
4. **Phase 4 — (REMOVED per R10 HIGH-2 closure 2026-05-22):** trigger creation is consolidated into Phase 2 (above). The §4 DDL blocks include all triggers inline; a separate Phase 4 would duplicate the CREATE TRIGGER statements + fail with duplicate_object errcode. Phase numbering is preserved (Phase 5 still follows Phase 4 slot conceptually) so the §8.2 → P-040 §8.2 phase-number cross-references remain valid; Phase 4 is intentionally an empty/skipped slot at v1.0.
...
7. **Phase 7 — Procedures (renamed from prior Phase 6 per R6 HIGH-1; R14 HIGH-2 closure 2026-05-22 — corrected count from "4 ... = 6" inconsistency to canonical 6):** deploy the **6 SECDEF procedures** (1 raw `record_forms_template_admin_review_transition` + 3 dashboard read-wrappers `read_admin_crisis_operational_health` + `read_admin_consult_queue_health` + `read_admin_mode1_volume_health` + 2 template wrappers `submit_forms_template_for_admin_review` + `record_forms_template_admin_decision`) per Sub-decision 3.5 + Sub-decision 4 + Sub-decision 4.5 wrapper bodies. Each verified SECURITY DEFINER + locked search_path; ownership set per §7; EXECUTE granted per §3 + §7 anti-bypass discipline. Since Phase 6 has already created the admin views + GRANT SELECT to the corresponding wrapper-owner role, CREATE FUNCTION validation finds the referenced relations + the wrapper bodies validate without dynamic-SQL workarounds.
...
**v0.17 DRAFT 2026-05-22 — R16 closure applied (1 HIGH):**
- **R16 HIGH-1 closed:** merged monotonic-ordering + state-continuity into unified `forms_template_admin_review_lifecycle_invariants()` trigger function so the advisory lock + READ COMMITTED isolation precondition cover BOTH invariants under a single lock-held read window. Eliminates inter-trigger ordering hazard from PostgreSQL alphabetical trigger firing order (M before S). 2 lifecycle triggers total: unified invariants + append-only.

**v0.16 DRAFT 2026-05-22 — R15 closures applied (3 HIGH):**
- **R15 HIGH-1 closed:** state-continuity trigger added READ COMMITTED isolation precondition check (raises invalid_transaction_initiation if caller has elevated isolation); advisory lock + READ COMMITTED together provide standalone concurrency safety.
- **R15 HIGH-2 closed:** deleted lingering "record_admin_dashboard_query_execution retained for cross-slice callers" sentence from Sub-decision 3.5; canonical inventory = 3 dashboard read-wrappers + 2 template wrappers + 1 raw writer = 6 SECDEF procedures across SI-023.
- **R15 HIGH-3 closed:** §1 in-scope rewritten from "2 OPTIONAL canonical views ... granted to admin_basic_operator role only" → "3 OPTIONAL canonical views ... SELECT granted ONLY to corresponding dashboard read-wrapper-owner role per §7; admin_basic_operator does NOT have direct SELECT".

**v0.15 DRAFT 2026-05-22 — R14 closures applied (2 HIGH):**
- **R14 HIGH-1 closed:** state-continuity trigger now acquires `pg_advisory_xact_lock(int8)` keyed by hash of (tenant_id, review_id) at trigger entry; concurrent INSERTs for the same review serialize at the lock; latest-state read happens AFTER lock held. Trigger is now standalone concurrency-safe.
- **R14 HIGH-2 closed:** reconciled SECDEF procedure inventory to canonical 6 (1 raw + 3 dashboard read-wrappers + 2 template wrappers); removed redundant `record_admin_dashboard_query_execution` (the 3 read-wrappers already emit audit co-transactionally post-R1 HIGH-1). §1 + §3.5 + Sub-decision 5 + §7 + §8.1 + §8.2 Phase 7 all enumerate 6.

**v0.14 DRAFT 2026-05-22 — R13 closures applied (3 HIGH):**
- **R13 HIGH-1 closed:** moved enforce_one_active_review_per_template function + trigger DDL from §4.NEW2 to §4.NEW3 (after CREATE TABLE forms_template_admin_review_lifecycle_transition); fixes Phase 2 ordering violation.
- **R13 HIGH-2 closed:** wrapped decision wrapper idempotency_key INSERT in EXCEPTION block catching unique_violation; re-reads existing row + verifies decision; raises retry-safe serialization_failure on match OR mismatch-error on different decision. Concurrent same-key races now resolve to canonical idempotent success.
- **R13 HIGH-3 closed:** added forms_template_admin_review_lifecycle_state_continuity BEFORE INSERT trigger to §4.NEW3; rejects NEW.from_state ≠ current latest to_state. 3 lifecycle-invariant triggers (append-only + monotonic-ordering + state-continuity) now together guarantee immutability + temporal ordering + state continuity.

**v0.13 DRAFT 2026-05-22 — R12 closures applied (2 HIGH):**
- **R12 HIGH-1 closed:** added executable DDL for record_forms_template_admin_review_transition() raw writer in NEW Sub-decision 4.5 — SECURITY DEFINER + locked search_path + ownership + REVOKE PUBLIC + GRANT EXECUTE to 2 template wrapper-owner roles. Matches P-038 §3.1 + P-040 §3.1 raw-writer pattern.
- **R12 HIGH-2 closed:** moved monotonic-ordering trigger from prose into executable DDL in §4.NEW3 — function + BEFORE INSERT trigger; rejects backdating + future-dating beyond 5s clock-skew with explicit errcodes; schema-qualified + locked search_path per P-038 R4 hardening.

**v0.12 DRAFT 2026-05-22 — R11 closure applied (1 HIGH):**
- **R11 HIGH-1 closed:** decision wrapper now participates in the shared parent-template FOR UPDATE serialization point (Step 0 read template_id from review without lock; Step 1 parent forms_template FOR UPDATE matching submit wrapper's acquisition order; Step 2 review row FOR UPDATE under parent lock). Both wrappers serialize on the same per-template lock; submit-vs-decision races eliminated structurally. §4.NEW2 LAYER 1 prose updated.

**v0.11 DRAFT 2026-05-22 — R10 closures applied (2 HIGH):**
- **R10 HIGH-1 closed:** REMOVED partial UNIQUE INDEX `forms_template_admin_review_one_active_per_template_uk` — append-only lifecycle row design is fundamentally incompatible with "latest-row-only" UNIQUE constraint; historical rows would block canonical revision cycle. Reduced to 2-layer enforcement (LAYER 1 parent-row FOR UPDATE + LAYER 2 BEFORE INSERT trigger). §4.NEW2/NEW3 prose + index DDL updated.
- **R10 HIGH-2 closed:** consolidated trigger creation into Phase 2 (where §4 DDL blocks naturally execute); Phase 4 removed (empty slot; preserves P-040 §8.2 phase-number alignment). Phase 2 prose updated with natural execution order + NEW1→NEW2→NEW3→NEW4 dependency chain.
