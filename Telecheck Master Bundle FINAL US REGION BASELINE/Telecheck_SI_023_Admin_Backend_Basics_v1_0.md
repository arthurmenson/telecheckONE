...
DECLARE
    v_row_count INTEGER;
BEGIN
    -- 1. Verify caller authorization (R3 HIGH-1 closure 2026-05-22: identical defect class
    --    to P-040 R17 — under SECURITY DEFINER, `current_user` returns the function OWNER
    --    (read_admin_crisis_operational_health_wrapper_owner), NOT the invoking role.
    --    A pg_has_role(current_user, 'admin_basic_operator', 'MEMBER') check would either
    --    (a) brick every wrapper call (if owner is not a member) or (b) be meaningless
    --    (if owner is made a member to satisfy the check). Fix: use `session_user` which
    --    is unaffected by SECURITY DEFINER and reflects the actual session-authenticated
    --    role; combine with SI-024.1 verify_session_jwt_and_extract_claims() for tenant scope.
    --
    --    The EXECUTE grant on the wrapper is the PRIMARY authorization boundary (only
    --    admin_basic_operator role members hold EXECUTE per §7); the session_user check
    --    below is DEFENSE-IN-DEPTH catching accidental over-grants.
    PERFORM 1 FROM verify_session_jwt_and_extract_claims() vc
    WHERE vc.verified_tenant_id = p_tenant_id
      AND pg_has_role(session_user, 'admin_basic_operator', 'MEMBER');
    IF NOT FOUND THEN
        RAISE EXCEPTION 'admin-dashboard-unauthorized: session_user % is not admin_basic_operator for tenant % OR JWT tenant binding mismatch', session_user, p_tenant_id
            USING ERRCODE = '42501';
    END IF;
