...
| `dual_control_approval` | **2 (deterministic; SEPARATE patient-chain emission for clinical approvals)** | `tenant_id` of the approving authority | **Revised v0.2 per Codex R1 HIGH-1 closure + R3 HIGH-1 closure:** v0.1 marked this "tier 1 or 2 by instance" — non-deterministic. Resolution: the `dual_control_approval` audit event records the governance action (the act of approval), which is ALWAYS tier 2 (tenant-scoped governance attribution). When the approved action is patient-affecting (e.g., approving a clinical decision for patient X), a SEPARATE Cat A patient-scope audit event (the action being approved, e.g., `prescribing.protocol_authorization_granted` for patient X) is ALSO emitted tier 1 in patient X's chain. **The two records are NOT cross-referenced via any canonical envelope field in SI-018's scope (R3 HIGH-1 closure 2026-05-19: v0.2's earlier wording used `linked_events[]` for cross-reference, but the canonical AUDIT_EVENTS envelope defines `linked_events[]` as `["<domain_event_id>"]` — audit→audit linkage via that field would overload the canonical schema; SI-018 does not amend the envelope schema).** Forensic walks for dual-emitted events use the canonical-pattern of timestamp proximity + same actor_id + same tenant_id + same correlation context in the action details — sufficient for forensic-walk-by-inference, the same pattern already used for other coupled audit events across the v1.10 catalog. A future SI may add an explicit audit→audit linkage field (e.g., `linked_audit_ids[]`) if implementation experience shows the inference-based linkage is insufficient; SI-018 narrowly does NOT introduce this. |
| `fake_med_flag_raised` / `_resolved` | **2 (deterministic; SEPARATE patient-chain emission when patient-traceable)** | `tenant_id` of the pharmacy operation | **Revised v0.2 per Codex R1 HIGH-1 closure + R3 HIGH-1 closure:** same pattern as `dual_control_approval`. The fake-med flag event is governance-class (Cat B); always tier 2 with `tenant_id`. When the flag is traceable to a specific patient's order, a SEPARATE Cat C patient-event (`medication_request.flagged_for_fake_med` or equivalent) is ALSO emitted tier 1 in the patient's chain. **No cross-reference via `linked_events[]` (R3 HIGH-1 closure — same reasoning as `dual_control_approval` above).** Forensic walks use canonical timestamp+actor+tenant+detail-context inference. |
...
| `signal_enforcement_trigger` | **2 (deterministic; SEPARATE patient-chain emission when signal references specific patient)** | `tenant_id` of the tenant where signal triggered | **Revised v0.2 per Codex R1 HIGH-1 closure + R3 HIGH-1 closure + R4 prose-scrub 2026-05-19:** v0.1 marked this "Variable" — non-deterministic. Resolution: the `signal_enforcement_trigger` audit event records the enforcement action (governance-class); always tier 2 with `tenant_id`. When the signal references a specific patient (e.g., crisis-detection signal for patient X), a SEPARATE Cat A patient-event (e.g., `crisis.escalation_destination_resolved` per SI-013) is ALSO emitted tier 1 in patient X's chain. **No cross-reference via `linked_events[]` (R3 HIGH-1 closure — same reasoning as `dual_control_approval` above; SI-018 does NOT amend the envelope schema; forensic walks use timestamp+actor+tenant+detail-context inference).** The detail-payload `signal_type` field discriminates the signal class but does NOT determine the partition tier (which is always 2 for this action ID). |
...
**Every row is now deterministic.** Future Cat B events MUST be annotated with their deterministic tier at ratification time. If an event has dual scope (governance-class AND patient-affecting), the pattern is: emit a Cat B tier-2 audit for the governance action + a SEPARATE Cat A or Cat C tier-1 audit for the patient effect. The two audit records are NOT cross-referenced via any canonical envelope linkage field (per R3 HIGH-1 closure 2026-05-19 — `linked_events[]` is canonically domain-event IDs, not audit-event IDs; SI-018 does NOT amend the envelope schema). Forensic walks use canonical timestamp+actor+tenant+detail-context inference, the same pattern already used for other coupled audit events across the v1.10 catalog. A future SI may add an explicit audit→audit linkage field (e.g., `linked_audit_ids[]`) if implementation experience shows inference-based linkage is insufficient; SI-018 narrowly does NOT introduce this. This is the canonical dual-emission pattern; no per-instance variable-tier ratification is permitted.
...
**New paragraph added to §Cross-partition checkpoint (line 297 area; refined v0.2 R3 closure 2026-05-19 per Codex R3 HIGH-2 to remove ambiguity in checkpoint input format):**

> The 24-hour checkpoint hashes across **all** active partitions across both tiers. The checkpoint input is a deterministic ordered sequence of `(tier, partition_key, latest_record_hash)` tuples where `tier` is the integer `1` (for tier-1 patient partitions) or `2` (for tier-2 tenant partitions); `partition_key` is the raw value of `target_patient_id` (tier 1) or `tenant_id` (tier 2) — NOT a prefixed string; `latest_record_hash` is the SHA-256 record hash of the most recent record in that partition. The tuples are sorted first by `tier` ascending (so all tier-1 tuples sort before all tier-2 tuples), then by `partition_key` lexicographically within tier. The `sorted_concat` operation at line 299 of the canonical AUDIT_EVENTS file concatenates the tuples in this canonical sort order, with a single canonical separator (`|` ASCII byte 0x7C) between tuple fields and a different canonical separator (`\n` ASCII byte 0x0A) between tuples. The integer-tier discriminator ensures deterministic sorting regardless of partition_key value collisions across tiers (e.g., if `target_patient_id = "X"` and `tenant_id = "X"` both exist as raw strings, they sort to distinct positions because tier 1 < tier 2). This ensures the global integrity anchor covers governance-event chains alongside patient-event chains without checkpoint-format ambiguity.
...
### Open Question 2: ~~Cross-partition checkpoint deterministic-sort prefix ordering~~ CLOSED via R3 HIGH-2 closure 2026-05-19

The v0.2 original phrasing of this question relied on prefix-string sorting of `hash_chain.partition` values. Codex R3 HIGH-2 correctly flagged that `hash_chain.partition` stores raw patient_id or raw tenant_id (per SI-018's own design), not prefixed strings — making the prefix-sort rule under-specified. **Closed v0.2 R3 closure:** Sub-decision 4's checkpoint specification now uses an integer-tier discriminator as the primary sort key (tier 1 < tier 2) + raw partition_key as the secondary sort key, eliminating any ambiguity. See revised Sub-decision 4 text above. This open question is no longer open.
...
Sub-decision 3 establishes that dual-scope events (`dual_control_approval`, `fake_med_flag_*`, `signal_enforcement_trigger` when patient-referencing) emit a Cat B tier-2 governance audit AND a SEPARATE Cat A/C tier-1 patient audit. Per the v0.2 R3 closure (HIGH-1), the two records are NOT cross-referenced via any canonical envelope linkage field — forensic walks use timestamp+actor+tenant+detail-context inference (the canonical pattern already used for coupled events). Should this dual-emission pattern be documented as a canonical pattern in the AUDIT_EVENTS file (e.g., a new §"Dual-emission pattern for governance+patient events" section), or is the per-event annotation in the Sub-decision 3 table sufficient?

**Recommendation:** document the canonical pattern in AUDIT_EVENTS as a new section (post-§Category B), with explicit note that the pattern emits two independent audit rows linked by forensic-walk inference (timestamp+actor+tenant+detail-context), NOT by canonical envelope linkage field. This makes the pattern reusable for future Cat B events without re-establishing it per-event. Small addition; +1 paragraph. A future SI may add explicit `linked_audit_ids[]` envelope field if implementation experience shows inference-based linkage is insufficient — but SI-018 narrowly does NOT amend the canonical envelope schema.
...
4. **Mixed-tier checkpoint test (refined v0.2 R3 closure 2026-05-19):** emit one event of each tier (one tier-1 + one tier-2), run the 24-hour checkpoint, assert checkpoint input is the canonical sequence of `(tier, partition_key, latest_record_hash)` tuples with tier-1 tuples before tier-2 tuples (integer-tier primary sort) and partition_key lexicographic within tier (secondary sort); separators `|` between fields + `\n` between tuples; checkpoint hash is reproducibly computable from this canonical input. Tamper any tuple → checkpoint hash changes.
...
8. **Dual-emission test (added v0.2; refined v0.2 R3 closure 2026-05-19):** emit a `dual_control_approval` event with `tenant_id = T` (Cat B tier-2 governance audit) AND a paired `prescribing.protocol_authorization_granted` event for patient X in tenant T (Cat A tier-1 patient audit); assert both rows exist with correct tier assignments; assert tenant T's chain extends by 1 (governance event) AND patient X's chain extends by 1 (patient event); assert the two events share `tenant_id = T` + similar `timestamp` (within ~1 second) + same `actor_id` — sufficient for forensic-walk-by-inference (R3 HIGH-1 closure: no canonical cross-reference field is used; SI-018 does NOT amend the envelope schema to add audit→audit linkage; future SI may add `linked_audit_ids[]` if implementation needs it).
