...
Audit-event emission moves from inside-procedure to application-layer immediately after procedure-success return. Important: domain-event (`consult_events`) atomicity stays inside procedure (per the unchanged "atomic UPDATE + paired `consult_events` INSERT" step of the 11-step validation); audit-event (`audit_events`) emission moves to application layer. SI-018 partition tier is **P1 (patient-bound)** because `target_patient_id` IS NOT NULL.
...
### Open Question 4: `rotate_consult_clinician_decision_kms()` audit event partition tier (P1 or P2)

`rotate_consult_clinician_decision_kms()` is a KMS-rotation lifecycle event. If it rotates the key for ONE specific patient's `clinician_decision_rationale_encrypted` envelope (single-patient scope), the audit event is patient-bound (P1). If it batch-rotates across multiple patients' envelopes (e.g., during a tenant-wide KMS key version rollover), the audit event is tenant-governance (P2). Both modes may exist; SI-018's partition rule is per-event, not per-procedure.

**Recommendation:** Per-event partition decision at the application-layer audit emission call site. The procedure's parameters should NOT pre-bind the partition tier; the application code emitting the audit event decides P1 vs P2 based on whether the rotation was scoped to a single patient or batch. Document this expectation in the implementation IMPL-readiness gate.
