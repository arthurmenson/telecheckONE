...
4. **`rotate_consult_clinician_decision_kms()` audit-event SI-018 partition tier is NORMATIVELY BOUND TO ROTATION SCOPE (closes Codex R1 HIGH-1 on PR #18):** SI-018's two-tier partition rule is per-event, so without a binding contract, an implementation could emit a tenant-wide P2 event for a single-patient rotation or a P1 event without `target_patient_id` for a batch rotation. The supersession therefore makes the partition rule normative for the KMS rotation call path:
   - **Single-patient rationale-envelope rotation** (rotation targets exactly one Consult row's `clinician_decision_rationale_encrypted` envelope; `p_rotation_scope = 'single_patient'`): audit_events record lives in **P1 (patient-bound)** with `target_patient_id` = the Consult's target_patient_id; `chain_partition_key = SHA-256("GENESIS:PATIENT:<target_patient_id>")`; Cat A.
   - **Batch / tenant-governance rotation** (rotation targets all rationale envelopes for a tenant during a KMS key-version rollover or compliance rotation; `p_rotation_scope = 'batch_tenant'`): audit_events record lives in **P2 (tenant-governance)** with `target_patient_id IS NULL`; `chain_partition_key = SHA-256("GENESIS:TENANT:<tenant_id>")`; Cat B governance.
   - **Application audit-emission call site contract:** the caller (application layer issuing the audit INSERT immediately after `rotate_consult_clinician_decision_kms()` returns success) MUST derive `partition_key` from the rotation scope (the procedure parameter or invocation context) BEFORE constructing the audit envelope. Mixed-scope or ambiguous rotations are not permitted at this surface; if a future use case requires multi-patient-but-not-tenant-wide scope, it requires a separate normative amendment.
...
### Sub-decision 4: Normatively bind `rotate_consult_clinician_decision_kms()` audit-event partition tier to rotation scope

P1 (patient-bound) for `p_rotation_scope = 'single_patient'`; P2 (tenant-governance) for `p_rotation_scope = 'batch_tenant'`. Application audit-emission call site MUST derive partition_key from rotation scope before constructing the audit envelope. Mixed-scope rotations not permitted at this surface (would require separate normative amendment).
