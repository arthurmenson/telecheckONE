...
- Re-target the four downstream SECURITY DEFINER procedures (SI-005's two at P-021; SI-008's at P-018; SI-009's at P-019) onto the canonical model. **[SUPERSEDED 2026-05-19 by PR #11 round-1 finding closure + engineering review answer same-day:** the original sentence "Their own ratifications do NOT depend on SI-010 — only the SI-010 source's narrative claim of 'unblocks four procedures' implied dependency" was WRONG. The procedures' OWN ratified ledger entries codify SI-010 dependencies (P-018 line 640; P-019 lines 575–576; P-021 line 491). Corrected re-targeting action: amend each procedure's actor-identity-source sub-decisions via lockstep supersession entries **P-018a (PR #12), P-019a (PR #13), P-021a (PR #14)** — each a single narrow ratifier sub-decision moving the actor-identity-source from SI-010's `current_actor_*()` helpers reading the rejected `_session_actor_context` table to application-supplied parameters from middleware-resolved JWT-verified context, with application-layer audit emission in the same application-managed transaction per engineering review's unanimous NO answer (autonomous-Claude analysis subject to actual-human sign-off at implementation downstream gate; documented at `Telecheck_v1_10_PRD_Update/Engineering-Review-Request-I-003-Atomic-Audit-Inside-vs-Application-Layer-Audit-2026-05-19.md`). The procedures' ratified row shapes / KMS envelopes / atomic-UPDATE patterns / rejection-code enums / three-tier-durability framework survive the supersessions intact; only the actor-identity-source sub-decisions change. See Promotion Ledger P-023a CORRECTION block + §8 of this Decision Memo for the authoritative current state.**] Each procedure can accept actor identity as an application-supplied parameter sourced from the canonical middleware-resolved actor context, with the application bearing the trust-boundary responsibility (same as every RPC the platform issues).
