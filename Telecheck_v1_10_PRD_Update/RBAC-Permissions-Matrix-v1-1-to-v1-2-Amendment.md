...
|---|---|---|
| Inv-1 (R2 HIGH closure: covers full 6-signer set; stakeholder-role-level separation) | No single `human_id` holds signer identities for more than one STAKEHOLDER ROLE across the full set: `{hsm_signer_cto, hsm_signer_cto_alt}` ∪ `{hsm_signer_compliance_officer, hsm_signer_compliance_officer_alt}` ∪ `{hsm_signer_incident_commander, hsm_signer_incident_commander_alt}` — i.e., a human may hold primary+alternate within ONE stakeholder role bucket (CTO, CO, or IC) but never identities spanning two or more buckets. The approval-broker (canonical 3-of-3 OR degraded 2-of-3 mode) MUST validate that the accepted signatures map to (a) two distinct `human_id` values AND (b) two distinct stakeholder-role buckets — both conditions required | Static rule TLC-RBAC-001 (verifies full 6-signer set membership); grant-time DB constraint computing stakeholder-role-bucket per signer + UNIQUE per (human_id, stakeholder_role_bucket) prevents cross-bucket grants; IAM AssumeRole trust-policy condition rejecting AssumeRole when source-and-target principals resolve to same human_id; approval-broker explicit validation: `distinct_human_ids(sigs) AND distinct_stakeholder_buckets(sigs)` before issuing STS credentials |
