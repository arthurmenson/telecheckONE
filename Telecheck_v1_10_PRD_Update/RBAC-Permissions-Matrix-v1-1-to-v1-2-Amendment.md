...
**Version:** 0.1 DRAFT
**Status:** RATIFIER-READY-WITH-KNOWN-OQs (R4 prose-consistency close + canonical vs degraded mode validation split 2026-05-19); Sprint 18 of autonomous 24h-loop work plan
**Codex iteration trajectory:** R1 (4 HIGH + 2 MED) → R2 (1 HIGH + 1 MED) → R3 (1 HIGH Inv-1 constraint inversion) → R4 (1 HIGH broker validation mode ambiguity). All 9 findings closed inline; 0 architectural-judgment items closed inline.
...
|---|---|---|
| Inv-1 (R2 HIGH + R3 HIGH closure: covers full 6-signer set; stakeholder-role-level separation; correct grant-time enforcement) | No single `human_id` holds signer identities for more than one STAKEHOLDER ROLE across the full set: `{hsm_signer_cto, hsm_signer_cto_alt}` ∪ `{hsm_signer_compliance_officer, hsm_signer_compliance_officer_alt}` ∪ `{hsm_signer_incident_commander, hsm_signer_incident_commander_alt}` — a human may hold primary+alternate within ONE stakeholder-role bucket (CTO, CO, or IC) but never identities spanning two or more buckets. The approval-broker (canonical 3-of-3 OR degraded 2-of-3 mode) MUST validate that the accepted signatures map to (a) two distinct `human_id` values AND (b) two distinct stakeholder-role buckets — both conditions required | Static rule TLC-RBAC-001 (verifies full 6-signer set membership); **canonical grant-time invariant: for each human_id, `COUNT(DISTINCT stakeholder_role_bucket) <= 1` across all their active HSM signer bindings**. Implemented as a normalized `hsm_signer_binding(human_id, iam_principal_arn, stakeholder_role_bucket, granted_at, revoked_at)` table + a BEFORE INSERT trigger that rejects new bindings where the candidate `human_id` already has an active binding in a DIFFERENT `stakeholder_role_bucket` (allows same-bucket primary+alternate; rejects cross-bucket grants). IAM AssumeRole trust-policy condition rejecting AssumeRole when source-and-target principals resolve to same human_id; **approval-broker explicit validation, per mode (R4 HIGH closure: split canonical vs degraded thresholds):** (a) canonical 3-of-3 mode: EXACTLY 3 valid signatures, covering ALL 3 distinct `stakeholder_role_bucket` values {CTO, CO, IC} (one signature per bucket), with 3 distinct `human_id` values; (b) degraded 2-of-3 mode (only allowed after the 30-min incident threshold predicate fires): at least 2 valid signatures covering at least 2 distinct `stakeholder_role_bucket` values with at least 2 distinct `human_id` values. The broker SQL/code checks the predicate `IF emergency_mode_active THEN 2-of-3 validation ELSE 3-of-3 validation`. |

**Positive + negative test examples (R3 HIGH closure):**

- POSITIVE: human Alice holds both `hsm_signer_cto` (primary) AND `hsm_signer_cto_alt` (alternate) — ALLOWED; both in same `stakeholder_role_bucket='CTO'`; COUNT(DISTINCT bucket) = 1.
- NEGATIVE: human Alice holds `hsm_signer_cto` AND `hsm_signer_compliance_officer` — REJECTED at grant-time; would create cross-bucket binding for human Alice; trigger raises `TLC_RBAC_INV1_VIOLATION`.
- NEGATIVE: human Alice holds `hsm_signer_cto_alt` AND `hsm_signer_compliance_officer_alt` — REJECTED at grant-time (alternates are still bucket-scoped; alternate-only does NOT defeat the separation).
- POSITIVE: human Alice holds `hsm_signer_cto` (primary) AND THEN her primary binding is revoked AND THEN she is granted `hsm_signer_compliance_officer` — ALLOWED; at the time of the new grant, COUNT(DISTINCT bucket) over ACTIVE bindings = 1.

**Mode-specific approval-broker validation examples (R4 HIGH closure):**

- **Canonical 3-of-3 POSITIVE:** Alice (CTO bucket primary) + Bob (CO bucket alternate) + Carol (IC bucket primary) — 3 signatures, 3 distinct human_ids, 3 distinct buckets {CTO, CO, IC}. Broker issues STS credentials.
- **Canonical 3-of-3 NEGATIVE:** Alice (CTO primary) + Bob (CO primary) — only 2 signatures; canonical mode requires 3. Broker REJECTS even though degraded thresholds would pass.
- **Canonical 3-of-3 NEGATIVE:** Alice (CTO primary) + Alice (CTO alternate) + Carol (IC primary) — 2 distinct human_ids only; canonical requires 3 distinct. Broker REJECTS (also fails Inv-1 since Alice cannot hold both CTO primary + alternate AND CTO alternate is a separate binding — but assuming Alice does hold both within CTO bucket, this still fails 3-of-3 distinctness).
- **Degraded 2-of-3 POSITIVE (after 30-min predicate fires):** Alice (CTO primary) + Bob (CO primary) — 2 signatures, 2 distinct human_ids, 2 distinct buckets {CTO, CO}. Broker issues STS credentials with 2-hour cap (per Sprint 13 §7.1 emergency expiry).
- **Degraded 2-of-3 NEGATIVE:** Alice (CTO primary) + Bob (CTO alternate) — 2 distinct human_ids BUT only 1 distinct bucket {CTO}. Broker REJECTS (covers <2 buckets).
- **Degraded 2-of-3 NEGATIVE (before 30-min predicate):** before the emergency-mode predicate fires (incident <30 min old), even a valid degraded-pattern signature set is rejected by the broker's mode-gate; the canonical 3-of-3 requirement holds until the predicate activates.
...
| Inv-5 (R2 MED closure: explicit pairwise distinct-human constraints) | `ai_mode2_l4_authorized` scope cannot be self-granted; the granting principals MUST be one CTO + one Compliance Officer (dual-control); the receiving principal MUST be a clinician or scheduled_job; the two grantors AND the grantee MUST be three distinct humans | Grant-procedure verification: (1) grant procedure inserts row with explicit `granting_human_id_1` + `granting_human_id_2` + `granted_human_id` columns; (2) DB constraints `CHECK (granting_human_id_1 <> granting_human_id_2)` AND `CHECK (granting_human_id_1 <> granted_human_id)` AND `CHECK (granting_human_id_2 <> granted_human_id)` — three pairwise distinct-human constraints; (3) procedure verifies `granting_human_id_1` resolves to canonical CTO role + `granting_human_id_2` resolves to canonical Compliance Officer role (or vice versa; order-independent); (4) procedure verifies `granted_human_id` resolves to clinician or scheduled_job role only |
...
| Round | Findings | Status |
|---|---|---|
| R2 | HIGH 2-of-3 degraded mode could collapse to one human via alternate signer role overlap (Inv-1 covered only primary signer identities); MED Inv-5 chained inequality SQL non-enforceable | Both closed inline |
| R3 | HIGH R2's Inv-1 grant-time constraint `UNIQUE per (human_id, stakeholder_role_bucket)` was logically inverted — it ALLOWED cross-bucket grants AND FORBADE primary+alternate in same bucket | Closed inline: canonical invariant `COUNT(DISTINCT stakeholder_role_bucket) <= 1 per human_id` enforced via BEFORE INSERT trigger on normalized `hsm_signer_binding` table; positive + negative test examples added |
| R4 | HIGH R2's broker validation `distinct_human_ids AND distinct_buckets` was the 2-of-3 threshold applied to BOTH modes; canonical 3-of-3 path accepted 2-bucket signatures | Closed inline: mode-specific split — canonical 3-of-3 requires EXACTLY 3 valid signatures, ALL 3 distinct buckets {CTO, CO, IC}, 3 distinct human_ids; degraded 2-of-3 (only after 30-min predicate) accepts 2 signatures, ≥2 distinct buckets, ≥2 distinct human_ids; 6 worked test examples added (3 canonical + 3 degraded) |

**Status at R4 close:** RATIFIER-READY-WITH-KNOWN-OQs. Sprint 18 closes at R4 prose-consistency clean with mode-specific quorum thresholds.

**R2 closure pattern recap:**
- HIGH: Inv-1 rewritten to cover the full 6-signer set (primary + alternate per stakeholder role); stakeholder-role-level separation prevents a human from holding signer identities for more than one of {CTO, CO, IC} buckets; approval-broker explicitly validates `distinct_human_ids(sigs) AND distinct_stakeholder_buckets(sigs)` before issuing STS credentials in both 3-of-3 and 2-of-3 modes.
- MED: Inv-5 rewritten with explicit 3 pairwise distinct-human CHECK constraints; grant procedure additionally verifies grantor roles are exactly {CTO, CO} (order-independent) + grantee role is clinician or scheduled_job.
