...
**Version:** 0.3 (audit-trail artifact; superseded by canonical content port 2026-05-19)
**Status:** **RATIFIED 2026-05-19** per Evans's specific per-item chat-message ratification + canonical content port lockstep commit (branch `spec/canonical-content-port-si018-si017-supersessions-2026-05-19` HEAD `82c61a0`, Codex R8 APPROVE). Canonical content lives in `Telecheck_Promotion_Ledger.md` entry **P-027** (SI-017 ratification: Phase 2 F-3 + Sub-decision 4.5 mismatch path). AUDIT_EVENTS v5.5 carries the 2 new action IDs (`identity.session_liveness_check_failed` Cat B + `identity.session_jwt_tenant_id_mismatch` Cat A). SI-017-OQ-MISMATCH resolved via A2+B2+C Decision Memo at `Telecheck_v1_10_PRD_Update/Decision-Memo-SI-017-OQ-MISMATCH-A2-B2-C-Adopted-2026-05-19.md`. This source file is the ratifier-input + audit-trail artifact; the canonical content port is implementation-authoritative.
