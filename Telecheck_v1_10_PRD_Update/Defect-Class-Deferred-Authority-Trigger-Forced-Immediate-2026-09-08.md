...
**Open item 2026-09-08 (Codex R1 on PR #306, HIGH, inherited by the merged copies):** the COMMIT-outcome classifier treats any SQLSTATE-with-severity rejection of an issued COMMIT as a definite rollback. PostgreSQL may send an ErrorResponse while the client awaits ReadyForQuery, so `CommandComplete(COMMIT)` followed by `FATAL 57P01` (backend termination) rejects the COMMIT promise although the transaction committed — a committed write can surface as a 500. Fixed in the shared primitive on #306 (only severity `ERROR` during COMMIT establishes rollback; no SQLSTATE, class 08, FATAL, PANIC → `unconfirmed()` / PT503). The module-internal copies in crisis-response (#302/#304, plus the caller-side `definiteRollback` in `admitPatientCareInput`), async-consult (#303) and consent (#305) carry the flaw until the consolidation refactor migrates them onto the primitive — scheduled immediately after #306 and the identity PR merge.

**Status 2026-09-08 (later):** all five sites fixed and merged — crisis #302 (`c17c727`) + #304 (`0a969ab`), async-consult #303 (`112f9aa`), consent #305 (`7b8d66c`), forms-intake #306 (`6ed2a53`, introduces the shared `src/lib/commit-authority-transaction.ts` primitive), identity #308 (`148d6ed`; #307 closed by GitHub when its stacked base was deleted, rebased content-identical). Remaining: the inherited FATAL/PANIC-after-COMMIT classifier open item in the three merged copies — consolidation refactor PR open (`refactor/commit-authority-consolidation`), Codex R1 in progress.

**CLOSED 2026-09-08:** consolidation refactor merged as PR #309 (`ec4da00`) — crisis-response, async-consult and consent are adapters over `src/lib/commit-authority-transaction.ts`; the inherited FATAL/PANIC-after-COMMIT classifier flaw is closed everywhere, and the primitive's unknown-outcome errors carry a symbol discriminator (`isCommitUnconfirmed`) so a server-raised PT503 is never mistaken for an unconfirmed COMMIT. All five sites fixed, one lifecycle, one classifier. Open follow-ups: identity-role PostgreSQL regression in the shape of `crisis-admission-commit-authority.test.ts`; thin the async-consult / consent adapter-level test files to adapter-specific cases.
