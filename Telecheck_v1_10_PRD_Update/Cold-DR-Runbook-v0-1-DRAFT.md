...
- **Durability under partition (R5 HIGH-1 closure; quorum semantics now partition-aware):** the ACK channel operates in two modes with explicit partition behavior. **Normal operation (BOTH regions reachable and healthy):** each ACK write requires QUORUM across both regions = the device's "ACK received" signal fires only after BOTH regions (W=2-of-2) have accepted the write. **Partition operation (a declared regional outage is active OR the channel detects cross-region replication is broken):** the channel falls back to W=1 — a single-region write produces a PROVISIONAL ACK. **The device treats a provisional ACK as locally-still-pending** for the purpose of its own queue state (i.e., the device does not delete the local signal entry from its retry queue), but the server-side ACK log records the partition-degraded write with a `partition_degraded=true` flag + the region that accepted it. **All `partition_degraded=true` entries are first-class pending obligations** at Step 14.5 (enumerated in the inventory below); they ARE NOT counted as platform-acknowledged emissions for the purpose of marking failover complete until either (a) the second region accepts the write via replication backfill (promoting them to quorum-ACKed), OR (b) the device's first-reconnect upload supplies the matching local-signal entry, allowing the platform to reconcile + finalize. (Closes R5 HIGH on 2-of-2-quorum-vs-availability contradiction inline; the 2-of-2 quorum holds in normal operation, the W=1 + partition-degraded flag holds in DR. The R4 HIGH-1 concern that single-region writes would create false device ACKs is preserved — the device still treats provisional ACKs as locally-pending.)
...
- **DR topology guarantee (partition-aware per R5 HIGH-1):** because the ACK channel writes to a multi-region store separate from the primary RDS, the ACK channel remains writeable from AT LEAST ONE region during a single-region outage (the surviving region accepts provisional ACKs flagged `partition_degraded=true`). ACKs from devices during T+0 to T+90 are durably recorded in the surviving region; quorum is restored when the failed region recovers + replication backfill promotes the partition-degraded entries to quorum-ACKed. **The runbook does NOT claim 2-of-2 quorum is available during a single-region outage** — partition operation explicitly falls back to W=1 with the partition-degraded provenance flag, and Step 14.5 + the offline-device path account for the partition-degraded set as first-class pending obligations.
...
# (a) Server-side inventory (THREE-STATE per R5 MED-1 closure refining R4 HIGH-2):
#     (a.i) i019_enqueue_ack_log entries acknowledged with QUORUM during the read-only window (partition_degraded=false) — authoritative for emissions the platform durably recorded under normal quorum.
#     (a.ii) i019_enqueue_ack_log entries flagged partition_degraded=true (single-region/provisional ACKs accepted under partition operation) — these are PENDING OBLIGATIONS until reconciliation per (a.iii). Enumerated in the inventory; not counted as platform-acknowledged until reconciled.
#     (a.iii) For each reachable device, on first successful write after cutover, the device uploads its local queue's pending-obligation list — every entry the device emitted that did not transition to a confirmed device-side ACK (quorum-confirmed via replication backfill OR explicit server confirmation). This includes (1) signals with no server-side ACK at all (channel was unreachable from device) and (2) signals with only partition-degraded ACKs from (a.ii) that the device has not seen promoted to quorum-confirmed. This upload is REQUIRED for the device to advance past first-reconnect; failure BLOCKS subsequent device writes per the canonical I-019 reconciliation contract.
#     The expected gating inventory = UNION of (a.i) + (a.ii) + (a.iii). Step 14.5 drain/order/dedup/isolation checks apply to the union. A signal in (a.ii) is RECONCILED when either the second-region replication backfill promotes it to quorum-ACKed (moves to (a.i)) OR the device's (a.iii) upload includes it (allowing finalization).
...
**Offline-device exception path (R5 MED-1 closure refining R2 MED-1 + R3 MED-1; three-state per-device obligation model):**

For each unreachable device (no successful write within 4h of cutover) the platform opens a per-device unresolved-obligation record. The record is keyed `(device_id, tenant_id)` and enumerates per-signal obligations in three states matching the inventory model in Step 14.5 (a):

- **Quorum-ACKed (state-Q):** for signals where the device reached the ACK channel under normal quorum operation pre-DR, the `i019_enqueue_ack_log` row is authoritative; the platform writes a `i019_pending_replay` row per `(device_id, local_signal_id)` noting deferred replay.
- **Partition-degraded (state-P):** for signals where the device reached the ACK channel under partition operation, the partition_degraded=true row in `i019_enqueue_ack_log` is informational only; the platform writes a `i019_pending_replay` row per `(device_id, local_signal_id)` AND records the partition_degraded provenance. These are NOT counted as platform-acknowledged until reconciliation matches them to the device's first-reconnect upload OR the second-region replication backfill promotes them.
- **No-ACK (state-N):** for signals where the device EMITTED but the ACK channel was unreachable (no row in `i019_enqueue_ack_log` at all), the platform cannot enumerate per-signal obligations without the device's first-reconnect upload. The per-device unresolved-obligation record carries `per_signal_inventory_known=false` until first reconnect. **At Step 16 mark-complete, the failover-completion report MUST state explicitly: "per-signal inventory for N offline devices in tenant_X is unknowable until first reconnect; reconciliation continues asynchronously."** This is an honest gap acknowledgment, not a suppression.

A background reconciliation job runs every 60 min: it drains state-Q entries as offline devices reconnect, promotes state-P entries to confirmed-ACKed (or pending-obligation) on reconnect, and opens state-N per-signal records on each offline device's first reconnect.

**Tenant risk reporting:** for tenants with >5% offline-device cohort relative to total active devices, a tenant-level Cat B `audit_retention.i019_offline_cohort_warning` audit event is emitted; tenant operator surfaces a per-tenant offline-cohort dashboard for clinical review. The dashboard MUST disclose the state-N device count separately (state-N has unknown per-signal exposure until reconnect; state-Q + state-P have known per-signal counts).
