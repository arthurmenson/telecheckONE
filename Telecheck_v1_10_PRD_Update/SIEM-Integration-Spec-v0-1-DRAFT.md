...
6. **Manifest immutability (concrete enforcement; R2 HIGH-2 + R3 HIGH-1 + R3 MED-1 closures).** Per-tier manifests are stored with the following enforced controls, with independence-of-verification + first-write acceptance controls per R3:
   ...
   **Independent transparency anchor (R3 HIGH-1 closure; eliminates circular tamper-detection):**
   ...
   - **Crash-safe two-phase signing protocol (R4 HIGH-2 closure; prevents permanently-unsigned manifest after PUT-then-fail):**
     ...
     - **Crash recovery / partial-failure handling:** if Phase 2 fails after Phase 1 succeeded — i.e., transparency log has a `manifest_intent` entry but no corresponding `manifest_persisted` Cat B audit event — a recovery job (separate from writer/signer/verifier) detects this within 5 min, re-attempts the PUT idempotently (S3 PUT of the same bytes returns existing version + 200 if `If-None-Match: *` finds the key, OR creates the version if not yet written). Idempotency holds because the manifest digest + signature are deterministic functions of the manifest content.
