...
**Post-P-044 canonical-versions reconciliation 2026-05-25 (Evans-authorised unilateral ratification; cockpit-surfaced drift between this table and file-header reality):**

Master PRD **v1.10** · Artifact Registry **v2.29** · System Architecture v1.2 · CDM **v1.4** · State Machines **v1.2** · OpenAPI v0.2 · RBAC v1.1 · Contracts Pack **v5.4** (whole family normalised: every file's header bumped to v5.4 to declare the family-canonical version; per-file amendment-cycle provenance preserved in Promotion Ledger P-044 only — file bodies untouched by this normalisation) · Engineering Handoff v1.3 · Operational Readiness v1.5 · Ghana Playbook v1.2 · ADR Set v1.0 + Addendums 016–019, 020–025, 026 + **ADR-027 Country-Conditional DTC Marketing** + **ADR-028 Research Data Partnership Posture A** + **ADR-029 AI Workload Taxonomy** (all 3 Accepted at v1.10 promotion) · DIC **v1.1 Canonical for development** (Patient mock v7 binding visual reference per Evans Option B 2026-04-28 fold-in) · Forms/Intake v2.1 · Pharmacy + Refill v2.1 · Admin Backend v1.1 · all other slice PRDs v1.0.

**P-044 reconciliation:** the cockpit's SpecCorpus screen (PR #14 — header auto-discovery + max-across-family) surfaced four canonical artifacts whose file headers had drifted past this table without it being re-flowed: CDM v1.2 → v1.4 (header), State Machines v1.1 → v1.2 (header), Contracts Pack v5.2 → v5.4 (max across the family; family normalised in P-044), Artifact Registry v2.10 → v2.29 (header). Other 3 artifacts (Master PRD, OpenAPI, RBAC) already in sync with their headers. Filename versions (v1_2, v1_1, v5_00, v2_10) are preserved as historical naming-convention anchors — the canonical version is the header-declared one. See Promotion Ledger P-044.
