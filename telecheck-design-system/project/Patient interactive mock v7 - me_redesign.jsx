    ...
    <>
      ...
      <div className="me-cards">
        ...
        <div className="me-grid">
          ...
          {/* Data & audit — opens immutable hash-chained log (ADR-013) */}
          <div className="me-mini" onClick={() => window.__tcSheet && window.__tcSheet("audit")}>
            <div className="me-mini-top">
              <div className="me-mini-ico" style={{ background:"#f0fdf4" }}>
                <Ic2 n="lock" s={14} sw={2} c="#166534"/>
              </div>
              <span style={{ fontSize:8.5, fontWeight:700, color:"#166534", background:"#dcfce7", padding:"2px 6px", borderRadius:9999 }}>IMMUTABLE</span>
            </div>
            <div>
              <div className="me-mini-section">Audit log</div>
              <div className="me-mini-val" style={{ fontSize:15 }}>24</div>
              <div className="me-mini-lbl">Events · hash-chained</div>
            </div>
          </div>
        </div>

        {/* PRD: Trust & safety (§7.3 · §11.7 #12 · §11.7 #14 · §20.4) */}
        <div className="section-h" style={{padding:"6px 0 4px"}}><span>Trust & safety</span></div>
        <div style={{display:"grid", gap:8}}>
          <div className="lc" onClick={() => window.__tcSheet && window.__tcSheet("ae")}>
            <div className="lc-ic warn"><Ic2 n="flag" s={18}/></div>
            <div><div className="lc-t">Report a side effect</div><div className="lc-s">Forwarded to FDA Ghana within 24h · pharmacovigilance</div></div>
            <div className="lc-chev"><Ic2 n="chev" s={16}/></div>
          </div>
          <div className="lc" onClick={() => window.__tcSheet && window.__tcSheet("fakemed")}>
            <div className="lc-ic warn"><Ic2 n="scan" s={18}/></div>
            <div><div className="lc-t">Last fake-med scan · advisory</div><div className="lc-s">2 of 3 checks passed · Maxmart Pharmacy</div></div>
            <div className="lc-chev"><Ic2 n="chev" s={16}/></div>
          </div>
          <div className="lc" onClick={() => window.__tcSheet && window.__tcSheet("tenant")}>
            <div className="lc-ic info"><Ic2 n="globe" s={18}/></div>
            <div><div className="lc-t">Tenant context · Telecheck-Ghana 🇬🇭</div><div className="lc-s">ADR-023 isolation · ADR-024 country profile · per-tenant KMS</div></div>
            <div className="lc-chev"><Ic2 n="chev" s={16}/></div>
          </div>
          <div className="lc" onClick={() => window.__tcSheet && window.__tcSheet("crisis")} style={{background:"#fef2f2", borderColor:"rgba(127,29,29,.2)"}}>
            <div className="lc-ic" style={{background:"#7f1d1d", color:"#fff"}}><Ic2 n="sos" s={18} c="#fff"/></div>
            <div><div className="lc-t">Crisis · 112 · Mental Health Authority</div><div className="lc-s">FLOOR-021 · always-on · in chat · in community</div></div>
            <div className="lc-chev"><Ic2 n="chev" s={16}/></div>
          </div>
        </div>
      </div>
