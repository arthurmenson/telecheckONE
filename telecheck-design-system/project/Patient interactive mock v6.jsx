...
const Ic2 = ({ n, s = 22, c = "currentColor", sw = 1.8 }) => {
  const p = {
    ...
    fork:<><path d="M8 3v6m0 0a3 3 0 0 0 3 3h0a3 3 0 0 0 3-3V3M11 12v9M16 3v4a2 2 0 0 1-2 2h0v12"/></>,
...
const TopBar = ({ delegate, onAccount, onNotifs, onEmergency, onAI, onFAB, notifCount = 3 }) => (
  <div className="topbar">
    <div className="tb-brand" onClick={onFAB || onAccount} style={{ cursor: "pointer" }}>
      <img className="tb-logo-img" src="assets/logo/telecheck-logo.svg" alt="Telecheck" style={{ height: 22, width: "auto", display: "block", flexShrink: 0 }}/>
    ...
    <div className="tb-ctx-av" onClick={onAccount} title={delegate === "k" ? "Viewing Kofi" : delegate === "abena" ? "Viewing Abena" : "Ama Mensah"} style={{ position: "relative" }}>
      {delegate === "k" ? "KO" : delegate === "abena" ? "AB" : "AM"}
      <div onClick={e => { e.stopPropagation(); onEmergency(); }} style={{ position: "absolute", bottom: -2, right: -2, width: 14, height: 14, borderRadius: "50%", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 1px 3px rgba(0,0,0,0.15)", cursor: "pointer" }}>
        <Ic2 n="sos" s={9} sw={2.5} c="var(--danger)"/>
      </div>
    </div>
  </div>
);
